...
public class PrettifyWebView extends NestedWebView {
  ...
  @Override
  public boolean onInterceptTouchEvent(final MotionEvent p) {
    return true;
  }
  ...
  private void initView(final @Nullable AttributeSet attrs) {
    if (isInEditMode())
      return;
    if (attrs != null) {
      TypedArray tp = getContext().obtainStyledAttributes(
          attrs, R.styleable.PrettifyWebView);
      try {
  ...
  public void setOnContentChangedListener(
      final @NonNull OnContentChangedListener onContentChangedListener) {
    this.onContentChangedListener = onContentChangedListener;
  }

  public void setThemeSource(final @NonNull String source,
  ...
  public void setSource(final @NonNull String source, final boolean wrap) {
    if (!InputHelper.isEmpty(source)) {
      ...
      if (!wrap)
        settings.setDisplayZoomControls(false);
      String page = PrettifyHelper.generateContent(
          source, AppHelper.isNightMode(getResources()), wrap);
      loadCode(page);
    }
  }
  ...
  private void loadCode(final String page) {
    post(()
             -> loadDataWithBaseURL("file:///android_asset/highlight/", page,
  ...
  public void scrollToLine(final @NonNull String url) {
    ...
    if (lineNo != null && lineNo.length > 1) {
      loadUrl("javascript:scrollToLineNumber('" + lineNo[0] + "', '" +
              lineNo[1] + "')");
    } else if (lineNo != null) {
  ...
  public void setGithubContentWithReplace(final @NonNull String source,
    ...
    addJavascriptInterface(new MarkDownInterceptorInterface(this, false),
                           "Android");
    String page = GithubHelper.generateContent(
        getContext(), source, baseUrl, AppHelper.isNightMode(getResources()),
        false, replace);
    post(()
             -> loadDataWithBaseURL("file:///android_asset/md/", page,
                                    "text/html", "utf-8", null));
  }

  public void setGithubContent(final @NonNull String source,
                               final @Nullable String baseUrl,
                               final boolean toggleNestScrolling) {
    setGithubContent(source, baseUrl, toggleNestScrolling, true);
  }

  public void setWikiContent(final @NonNull String source,
                             final @Nullable String baseUrl) {
    addJavascriptInterface(new MarkDownInterceptorInterface(this, true),
                           "Android");
    String page = GithubHelper.generateContent(
        getContext(), source, baseUrl, AppHelper.isNightMode(getResources()),
        AppHelper.isNightMode(getResources()), true);
    post(()
             -> loadDataWithBaseURL("file:///android_asset/md/", page,
                                    "text/html", "utf-8", null));
  }
  ...
  public void setGithubContent(final @NonNull String source,
                               final @Nullable String baseUrl,
                               final boolean toggleNestScrolling,
                               final boolean enableBridge) {
    if (enableBridge)
      addJavascriptInterface(
          new MarkDownInterceptorInterface(this, toggleNestScrolling),
          "Android");
    String page = GithubHelper.generateContent(
        getContext(), source, baseUrl, AppHelper.isNightMode(getResources()),
  ...
  private void startActivity(final @Nullable Uri url) {
    ...
    if (MarkDownProvider.isImage(url.toString())) {
      ...
      if (lastSegment != null || url.toString().startsWith("#") ||
