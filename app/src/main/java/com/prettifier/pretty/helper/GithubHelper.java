...
public class GithubHelper {
  ...
  generateContent(final @NonNull Context context, final @NonNull String source,
  ...
  @NonNull
  private static String parseReadme(final @NonNull String source,
                                    final @NonNull String baseUrl,
                                    final boolean isWiki) {
    ...
    return getParsedHtml(source, owner, repoName,
                         !isWiki ? builder.toString() : baseUrl, baseLinkUrl,
                         isWiki);
  }

  @NonNull
  private static String
  getParsedHtml(final @NonNull String source, final String owner,
                final String repoName, final String builder,
                final String baseLinkUrl, final boolean isWiki) {
  ...
  @NonNull
  private static String getLinkBaseUrl(final @NonNull String baseUrl) {
    NameParser nameParser = new NameParser(baseUrl);
    String owner = nameParser.getUsername();
    String repoName = nameParser.getName();
    Uri uri = Uri.parse(baseUrl);
    ...
    builder.append("https://")
        ...
        .append(repoName)
        .append("/");
    boolean containsMaster =
        paths.size() > 3 && paths.get(2).equalsIgnoreCase("blob");
    if (!containsMaster) {
  ...
  @NonNull
  private static String mergeContent(final @NonNull Context context,
  ...
  @NonNull
  private static String getStyle(final boolean dark) {
    return dark ? "./github_dark.css" : "./github.css";
  }

  @NonNull
  private static String getCodeStyle(final @NonNull Context context,
                                     final boolean isDark) {
    if (!isDark)
      return "";
    String primaryColor = getCodeBackgroundColor(context);
    String accentColor =
        "#" + Integer.toHexString(ViewHelper.getAccentColor(context))
                  .substring(2)
                  .toUpperCase();
    return "<style>\n"
        + "body .highlight pre, body pre {\n"
        + "background-color: " + primaryColor + " !important;\n" +
        (PrefGetter.getThemeType(context) == PrefGetter.AMLOD
             ? "border: solid 1px " + accentColor + " !important;\n"
             : "") +
        "}\n"
        + "</style>";
  }
  ...
  @NonNull
  private static String getCodeBackgroundColor(final @NonNull Context context) {
    @PrefGetter.ThemeType int themeType = PrefGetter.getThemeType();
    if (themeType == PrefGetter.BLUISH) {
      return "#" + Integer.toHexString(ViewHelper.getPrimaryDarkColor(context))
                       .substring(2)
                       .toUpperCase();
    }
    return "#" + Integer.toHexString(ViewHelper.getPrimaryColor(context))
                     .substring(2)
                     .toUpperCase();
  }
}
