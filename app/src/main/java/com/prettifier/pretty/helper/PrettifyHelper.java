...
public class PrettifyHelper {
    ...
    @NonNull private static String getHtmlContent(final @NonNull String css, final @NonNull String text, final boolean wrap, final boolean isDark) {
        return "<!DOCTYPE html>\n"
               + "<html>\n"
               + "<head>\n"
               + "    <meta charset=\"utf-8\">\n"
    ...
    @NonNull public static String generateContent(final @NonNull String source, final String theme) {
        return getHtmlContent(theme, getFormattedSource(source), false, false);
    }

    @NonNull public static String generateContent(final @NonNull String source, final boolean isDark, final boolean wrap) {
        return getHtmlContent(getStyle(isDark), getFormattedSource(source), wrap, isDark);
    }

    @NonNull private static String getFormattedSource(final @NonNull String source) {
        return source.replaceAll("<", "&lt;")
               .replaceAll(">", "&gt;");
    }

    @NonNull private static String getStyle(final boolean isDark) {
