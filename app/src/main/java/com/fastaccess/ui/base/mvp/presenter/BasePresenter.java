...
public class BasePresenter<V extends BaseMvp.FAView>
    extends TiPresenter<V> implements BaseMvp.FAPresenter {
  @com.evernote.android.state.State boolean enterprise;

  private boolean apiCalled;
  private final RxTiPresenterDisposableHandler subscriptionHandler =
  ...
  @Override
  public void onRestoreInstanceState(final Bundle outState) {
    if (outState != null)
      StateSaver.restoreInstanceState(this, outState);
  }

  @Override
  public void manageDisposable(final @Nullable Disposable... disposables) {
    if (disposables != null) {
  ...
  @Override
  public <T> void manageObservable(final @Nullable Observable<T> observable) {
    if (observable != null) {
  ...
  @Override
  public void manageViewDisposable(final @Nullable Disposable... disposables) {
    if (disposables != null) {
      if (isViewAttached()) {
  ...
  public void onError(final @NonNull Throwable throwable) {
    ...
    GitHubErrorResponse errorResponse =
        RestProvider.getErrorResponse(throwable);
    if (errorResponse != null && errorResponse.getMessage() != null) {
      sendToView(v -> v.showErrorMessage(errorResponse.getMessage()));
    } else {
      sendToView(v
                 -> v.showMessage(R.string.error,
                                  getPrettifiedErrorMessage(throwable)));
    }
  }

  @Override
  public <T> void makeRestCall(final @NonNull Observable<T> observable,
                               final @NonNull Consumer<T> onNext) {
  ...
  @StringRes
  private int getPrettifiedErrorMessage(final @Nullable Throwable throwable) {
    ...
    manageObservable(RestProvider.gitHubStatus()
                         ...
                         .doOnNext(gitHubStatusModel -> {
                           ...
                           String indicatorStatus =
                               status != null ? status.getIndicator() : null;
                           if (!InputHelper.isEmpty(description) &&
                               !"none".equalsIgnoreCase(indicatorStatus)) {
                             sendToView(
                                 v
                                 -> v.showErrorMessage("Github Status:(" +
                                                       indicatorStatus + ")\n" +
                                                       description));
                           }
                         }));
