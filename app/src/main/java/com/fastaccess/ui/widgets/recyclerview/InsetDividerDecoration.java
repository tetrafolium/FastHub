...
/**
 * A decoration which draws a horizontal divider between {@link
 * RecyclerView.ViewHolder}s of a given type; with a left inset. this class
 * adopted from Plaid
 */
class InsetDividerDecoration extends RecyclerView.ItemDecoration {
  ...
  InsetDividerDecoration(final int divider, final int leftInset,
  ...
  @Override
  public void onDrawOver(final @NonNull Canvas canvas,
    ...
    for (int i = 0; i < childCount; i++) {
      ...
      if (!(viewHolder instanceof ProgressBarViewHolder)) {
        boolean canDivide =
            toDivide == null || viewHolder.getClass() == toDivide;
        if (canDivide) {
          ...
          if (position != (state.getItemCount() - 1)) {
            lines[i * 4] =
                inset == 0 ? inset : inset + lm.getDecoratedLeft(child);
            lines[(i * 4) + 2] = lm.getDecoratedRight(child);
            int y = lm.getDecoratedBottom(child) +
                    (int)child.getTranslationY() - height;
            lines[(i * 4) + 1] = y;
            lines[(i * 4) + 3] = y;
            hasDividers = true;
          }
