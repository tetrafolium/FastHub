...
public class RecyclerViewFastScroller extends FrameLayout {
  ...
  public boolean onTouchEvent(final @NonNull MotionEvent event) {
    ...
    case MotionEvent.ACTION_DOWN:
      if (event.getX() < (scrollerView.getX() - scrollerView.getPaddingStart()))
        return false;
  ...
  private void initScrollHeight() {
    if (recyclerView.computeVerticalScrollOffset() == 0) {
      this.recyclerView.getViewTreeObserver().addOnPreDrawListener(
          new ViewTreeObserver.OnPreDrawListener() {
            @Override
            public boolean onPreDraw() {
              RecyclerViewFastScroller.this.recyclerView.getViewTreeObserver()
  ...
  protected void iniHeight() {
    if (scrollerView.isSelected())
      return;
    int verticalScrollOffset = RecyclerViewFastScroller.this.recyclerView
                                   .computeVerticalScrollOffset();
    int verticalScrollRange =
        RecyclerViewFastScroller.this.computeVerticalScrollRange();
    float proportion =
  ...
  private void setRecyclerViewPosition(final float y) {
    ...
    if (recyclerView != null) {
      ...
      if (scrollerView.getY() == 0) {
        proportion = 0f;
      } else if (scrollerView.getY() + scrollerView.getHeight() >=
                 height - TRACK_SNAP_RANGE) {
        proportion = 1f;
      } else {
        proportion = y / (float)height;
      }
      int targetPos =
          getValueInRange(itemCount - 1, (int)(proportion * (float)itemCount));
  ...
  private void setScrollerHeight(final float y) {
    ...
    scrollerView.setY(
        getValueInRange(height - handleHeight, (int)(y - handleHeight / 2)));
  }
  ...
  private final RecyclerView.OnScrollListener onScrollListener =
      new RecyclerView.OnScrollListener() {
        ...
        public void onScrolled(final RecyclerView recyclerView, final int dx,
          ...
          float proportion = (float)verticalScrollOffset /
                             ((float)verticalScrollRange - height);
          setScrollerHeight(height * proportion);
        }
      };
  ...
  protected void hideShow() {
    if (recyclerView != null && recyclerView.getAdapter() != null) {
      setVisibility(recyclerView.getAdapter().getItemCount() > 10 ? VISIBLE
                                                                  : GONE);
    } else {
      setVisibility(GONE);
    }
