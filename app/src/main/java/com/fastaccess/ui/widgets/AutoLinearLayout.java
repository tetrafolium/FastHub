...
public class AutoLinearLayout extends FrameLayout {
  ...
  private void init(final Context context, final AttributeSet attrs,
    ...
    TypedArray a = context.obtainStyledAttributes(
        attrs, R.styleable.AutoLinearLayout, defStyleAttr, defStyleRes);
    try {
      mOrientation =
          a.getInt(R.styleable.AutoLinearLayout_auto_orientation, HORIZONTAL);
  ...
  private void measureHorizontal(final int widthMeasureSpec,
                                 final int heightMeasureSpec) {
  ...
  private void measureVertical(final int widthMeasureSpec,
                               final int heightMeasureSpec) {
    ...
    setMeasuredDimension(
        resolveSize(totalWidth + getPaddingRight() + getPaddingLeft(),
                    widthMeasureSpec),
        resolveSize(hSize, heightMeasureSpec));
  }

  /**
   * Arranges the children in columns. Takes care about child margin, padding,
   * gravity and child layout gravity.
   *
   * @param left
   *         parent left
   * @param top
   ...
   *         parent bottom
   */
  private void layoutVertical(final int left, final int top, final int right,
                              final int bottom) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (child != null && child.getVisibility() != View.GONE) {
        ...
        if (childTop + childHeight + lp.topMargin + lp.bottomMargin >
            height + getPaddingTop()) {
          // before change column update positions if the gravity is present
          updateChildPositionVertical(height, totalVertical, column,
                                      maxChildWidth);
  ...
  /**
   * Arranges the children in rows. Takes care about child margin, padding,
   * gravity and child layout gravity. Analog to vertical.
   *
   * @param left
   *         parent left
   * @param top
   ...
   *         parent bottom
   */
  private void layoutHorizontal(final int left, final int top, final int right,
                                final int bottom) {
    final int count = getChildCount();
    if (count == 0)
      return;
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (child != null && child.getVisibility() != View.GONE) {
        ...
        if (childLeft + childWidth + lp.leftMargin + lp.rightMargin >
          ...
          updateChildPositionHorizontal(width, totalHorizontal, row,
                                        maxChildHeight);
   ...
   */
  private void updateChildPositionVertical(final int parentHeight,
                                           final int totalSize,
                                           final int column,
                                           final int maxChildWidth) {
    for (int i = 0; i < mListPositions.size(); i++) {
      ...
      if (mOrientation == VERTICAL && pos.position == column) {
        LayoutParams lp = (LayoutParams)child.getLayoutParams();
        int size = maxChildWidth - child.getMeasuredWidth() - lp.leftMargin -
                   lp.rightMargin;
   ...
   */
  private void updateChildPositionHorizontal(final int parentWidth,
                                             final int totalSize, final int row,
                                             final int maxChildHeight) {
    for (int i = 0; i < mListPositions.size(); i++) {
      ViewPosition pos = mListPositions.get(i);
      final View child = getChildAt(i);
      ...
      if (mOrientation == HORIZONTAL && pos.position == row) {
        LayoutParams lp = (LayoutParams)child.getLayoutParams();
        int size = maxChildHeight - child.getMeasuredHeight() - lp.topMargin -
                   lp.bottomMargin;
  ...
  private void updateLeftPositionByGravity(final ViewPosition pos,
                                           final int size, final int gravity) {
  ...
  private void updateTopPositionByGravity(final ViewPosition pos,
                                          final int size, final int gravity) {
  ...
  private void layout(final View child, final ViewPosition pos) {
    ...
    if (mOrientation == HORIZONTAL)
      child.layout(pos.left, pos.top + lp.topMargin,
                   pos.left + child.getMeasuredWidth(),
                   pos.top + child.getMeasuredHeight() + lp.topMargin);
    else
      child.layout(pos.left + lp.leftMargin, pos.top,
                   pos.left + child.getMeasuredWidth() + lp.leftMargin,
  ...
  public void setHorizontalGravity(final int horizontalGravity) {
    final int gravity =
        horizontalGravity & GravityCompat.RELATIVE_HORIZONTAL_GRAVITY_MASK;
    if ((mGravity & Gravity.RELATIVE_HORIZONTAL_GRAVITY_MASK) != gravity) {
      mGravity = (mGravity & ~GravityCompat.RELATIVE_HORIZONTAL_GRAVITY_MASK) |
                 gravity;
      requestLayout();
    }
  }
