...
public class LabelSpan extends ReplacementSpan {
  ...
  private int measureWidth(final Paint paint, final CharSequence text,
  ...
  private void setupFontMetrics(final CharSequence text, final int start,
                                final int end, final FontMetricsInt fm,
                                final Paint p) {
    txtPaint.set(p);
    final CharacterStyle[] otherSpans =
        ((Spanned)text).getSpans(start, end, CharacterStyle.class);
  ...
  public void draw(final @NonNull Canvas canvas, final CharSequence text,
    ...
    if (bgWidth == maxWidth) {
      text = TextUtils.ellipsize(text.subSequence(start, end).toString(),
                                 txtPaint, bgWidth - 2 * paddingW,
    ...
    if (color != Color.TRANSPARENT)
      txtPaint.setColor(ViewHelper.generateTextColor(color));
