...
public class ColorPickerPreference
    extends Preference implements ColorPicker.OnChooseColorListener {
  ...
  protected void onClick() {
    ...
    String title =
        String.format("Accent Color: (Currently: %s)", getSelectedColorName());
    ...
    TextView title_tv =
        colorPicker.getDialogViewLayout().findViewById(R.id.title);
    title_tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 14);
    colorPicker.getPositiveButton().setTextColor(
        ViewHelper.getPrimaryTextColor(getContext()));
    colorPicker.getNegativeButton().setTextColor(
        ViewHelper.getPrimaryTextColor(getContext()));
  ...
  public void onBindViewHolder(PreferenceViewHolder holder) {
    ...
    colorButton.getBackground().setColorFilter(getSelectedColor(),
                                               PorterDuff.Mode.SRC_IN);
  }

  private int getSelectedColor() {
    TypedArray colorTypedArray =
        getContext().getResources().obtainTypedArray(R.array.theme_colors_hex);
    String[] colorNames =
        getContext().getResources().getStringArray(R.array.theme_colors);
    ...
    for (int i = 0; i < colorNames.length; i++) {
      preferenceValueToColor.put(
          PrefGetter.getThemeColor(getContext().getResources(), colorNames[i]),
          colors.get(i));
    }
  ...
  public void onChooseColor(int position, int color) {
    ...
    // color);
    persistString(getContext().getResources().getStringArray(
        R.array.theme_colors)[position]);
