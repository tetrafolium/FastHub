...
public class GitHubContributionsView extends View {
  ...
  private void initAttributes(final TypedArray attributes) {
    baseColor = attributes.getColor(
        R.styleable.GitHubContributionsView_baseColor, baseColor);
    baseEmptyColor = attributes.getColor(
        R.styleable.GitHubContributionsView_baseEmptyColor, baseEmptyColor);
    ...
    textColor = attributes.getColor(
        R.styleable.GitHubContributionsView_textColor, textColor);
    displayMonth = attributes.getBoolean(
        R.styleable.GitHubContributionsView_displayMonth, displayMonth);
    lastWeeks = attributes.getInt(R.styleable.GitHubContributionsView_lastWeeks,
                                  lastWeeks);
    if (attributes.getString(R.styleable.GitHubContributionsView_username) !=
        null) {
      username =
          attributes.getString(R.styleable.GitHubContributionsView_username);
      if (!isInEditMode()) {
  ...
  public void setLastWeeks(final int lastWeeks) {
    if (lastWeeks >= 2 && lastWeeks <= 53) {
      ...
      throw new RuntimeException(
          "The last weeks should be a number between 2 and 53");
    }
  }
  ...
  public Bitmap drawOnCanvas(final List<ContributionsDay> contributionsFilter,
                             final List<ContributionsDay> contributions) {
    if ((contributionsFilter == null || contributions == null) ||
        (contributionsFilter.isEmpty() || contributions.isEmpty())) {
      return null;
    }
    if (bitmap == null) {
      ...
      int horizontalBlockNumber = getHorizontalBlockNumber(
          contributionsFilter.size(), verticalBlockNumber);
      ...
      int height =
          (int)((blockWidth + spaceWidth) * 7 + topMargin + monthTextHeight);
      bitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
      Canvas canvas1 = new Canvas(bitmap);
      // Background
      blockPaint.setColor(backgroundBaseColor);
      canvas1.drawRect(0, (topMargin + monthTextHeight), width,
      ...
      int currentWeekDay = DatesUtils.getWeekDayFromDate(
          contributions.get(0).year, contributions.get(0).month,
          contributions.get(0).day);
      float x = 0;
      float y = (currentWeekDay - 7) % 7 * (blockWidth + spaceWidth) +
                (topMargin + monthTextHeight);
      for (ContributionsDay day : contributionsFilter) {
        blockPaint.setColor(ColorsUtils.calculateLevelColor(
            baseColor, baseEmptyColor, day.level));
  ...
  private void drawPlaceholder(final Canvas canvas) {
    ...
    // Background
    blockPaint.setColor(backgroundBaseColor);
    canvas.drawRect(0, (topMargin + monthTextHeight), width,
                    height + monthTextHeight, blockPaint);
    ...
    for (int i = 1; i < (lastWeeks * 7) + 1; i++) {
      ...
      blockPaint.setColor(
          ColorsUtils.calculateLevelColor(baseColor, baseEmptyColor, 0));
  ...
  public List<ContributionsDay>
  getLastContributions(final List<ContributionsDay> contributions) {
    int lastWeekDays = contributions.size() % 7;
    int lastDays =
        (lastWeekDays > 0) ? lastWeekDays + (lastWeeks - 1) * 7 : lastWeeks * 7;
    return contributions.subList(contributions.size() - lastDays,
                                 contributions.size());
  }
}
