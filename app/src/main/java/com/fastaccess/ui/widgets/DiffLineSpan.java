...
public class DiffLineSpan
    extends MetricAffectingSpan implements LineBackgroundSpan {
  private Rect rect = new Rect();
  private final int color;
  public static Pattern HUNK_TITLE = Pattern.compile(
      "^.*-([0-9]+)(?:,([0-9]+))? \\+([0-9]+)(?:,([0-9]+))?.*$");

  private DiffLineSpan(final int color) { this.color = color; }
  ...
  getSpannable(final @Nullable String text,
               final @ColorInt int patchAdditionColor,
               final @ColorInt int patchDeletionColor,
               final @ColorInt int patchRefColor) {
    return getSpannable(text, patchAdditionColor, patchDeletionColor,
                        patchRefColor, false);
  }
  ...
  getSpannable(final @Nullable String text,
               final @ColorInt int patchAdditionColor,
               final @ColorInt int patchDeletionColor,
               final @ColorInt int patchRefColor, final boolean truncate) {
      ...
      if (split.length > 0) {
        ...
        for (int i = 0; i < lines; i++) {
          ...
          if (color != Color.TRANSPARENT) {
            ...
            spannableDiff.setSpan(span, 0, token.length(),
        ...
        if (index != -1) {
          builder.insert(
              builder.length() - 1,
              SpannableBuilder.builder().append(ContextCompat.getDrawable(
                  App.getInstance(), R.drawable.ic_newline)));
        }
      }
    }
