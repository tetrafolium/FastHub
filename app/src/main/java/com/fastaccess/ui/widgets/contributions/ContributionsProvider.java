...
public class ContributionsProvider {
  ...
  @NonNull
  public List<ContributionsDay> getContributions(final
    ...
    while (true) {
      ...
      if (fillPos == -1)
        break;
      int level = 0;
      String levelString = string.substring(fillPos + FILL_STRING.length(),
      ...
      String dataString =
          string.substring(dataPos + DATA_STRING.length(), dataEndPos);
      int data = Integer.valueOf(dataString);
      String dateString = string.substring(datePos + DATE_STRING.length(),
      ...
      contributions.add(new ContributionsDay(
          ...
          Integer.valueOf(dateString.substring(8, 10)), level, data));
    }
