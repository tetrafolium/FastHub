...
public class FeedsViewHolder extends BaseViewHolder<Event> {
  ...
  public FeedsViewHolder(final @NonNull View itemView,
                         final @Nullable BaseRecyclerAdapter adapter) {
    super(itemView, adapter);
    this.resources = itemView.getResources();
  }

  public static View getView(final @NonNull ViewGroup viewGroup,
  ...
  @Override
  public void bind(final @NonNull Event eventsModel) {
  ...
  private void appendOrganizationEvent(final SpannableBuilder spannableBuilder,
                                       final Event eventsModel) {
    spannableBuilder
        .bold(eventsModel.getPayload().getAction().replaceAll("_", ""))
        .append(" ")
        .append(eventsModel.getPayload().getInvitation() != null
                    ? eventsModel.getPayload().getInvitation().getLogin() + " "
                    : "")
        .append(eventsModel.getPayload().getOrganization().getLogin());
  }

  private void appendProjectCardEvent(final SpannableBuilder spannableBuilder,
                                      final Event eventsModel,
  ...
  private void appendOrgBlockEvent(final SpannableBuilder spannableBuilder,
                                   final Event eventsModel) {
    spannableBuilder.bold(eventsModel.getPayload().getAction())
        .append(" ")
  ...
  private void appendForkApplyEvent(final SpannableBuilder spannableBuilder,
                                    final Event eventsModel) {
    spannableBuilder.bold(eventsModel.getPayload().getHead())
        .append(" ")
        .append(eventsModel.getPayload().getBefore())
        .append(" ")
        .append(eventsModel.getRepo() != null
                    ? "in " + eventsModel.getRepo().getName()
                    : "");
  }

  private void appendReleaseEvent(final SpannableBuilder spannableBuilder,
                                  final Event eventsModel) {
  ...
  private void appendDeleteEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
  ...
  private void appendTeamEvent(final SpannableBuilder spannableBuilder,
                               final Event eventsModel) {
    ...
    spannableBuilder.bold("added")
        ...
        .append(user != null ? user.getLogin()
                             : eventsModel.getRepo().getName())
        .append(" ")
        .bold("in")
        .append(" ")
        .append(teamsModel.getName() != null ? teamsModel.getName()
                                             : teamsModel.getSlug());
  }

  private void appendPushEvent(final SpannableBuilder spannableBuilder,
                               final Event eventsModel) {
    ...
    if (size > 0) {
      if (size != 1)
        spanCommits.append(String.valueOf(eventsModel.getPayload().getSize()))
            .append(" new commits")
            .append("\n");
      else
      ...
      for (GitCommitModel commit : commits) {
        ...
        spanCommits.url(sha)
            .append(" ")
            .append(commit.getMessage() != null
                        ? commit.getMessage().replaceAll("\\r?\\n|\\r", " ")
                        : "")
            .append("\n");
        appended++;
        if (appended == max)
  ...
  private void
  appendPullRequestReviewCommentEvent(final SpannableBuilder spannableBuilder,
                                      final Event eventsModel) {
    ...
    if (comment.getBody() != null) {
      MarkDownProvider.stripMdText(
          description, comment.getBody().replaceAll("\\r?\\n|\\r", " "));
      description.setVisibility(View.VISIBLE);
    } else {
      description.setText("");
      description.setVisibility(View.GONE);
  ...
  private void appendPullRequestEvent(final SpannableBuilder spannableBuilder,
                                      final Event eventsModel) {
    ...
    if ("opened".equals(action) || "closed".equals(action)) {
      if (issue.getTitle() != null) {
        MarkDownProvider.stripMdText(
            description, issue.getTitle().replaceAll("\\r?\\n|\\r", " "));
        description.setVisibility(View.VISIBLE);
      } else {
        description.setText("");
        description.setVisibility(View.GONE);
  ...
  private void appendPublicEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
    String action = "public";
    if (eventsModel.getPayload() != null &&
        "privatized".equalsIgnoreCase(eventsModel.getPayload().getAction())) {
      action = "private";
    }
  ...
  private void appendMemberEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
  ...
  private void appendIssueEvent(final SpannableBuilder spannableBuilder,
                                final Event eventsModel) {
    Issue issue = eventsModel.getPayload().getIssue();
    boolean isLabel = "label".equals(eventsModel.getPayload().getAction());
    LabelModel label =
        isLabel ? issue.getLabels() != null && !issue.getLabels().isEmpty()
                      ? issue.getLabels().get(issue.getLabels().size() - 1)
                      : null
                : null;
    spannableBuilder
        .bold(isLabel && label != null ? ("Labeled " + label.getName())
                                       : eventsModel.getPayload().getAction())
        .append(" ")
        .bold("issue")
        .append(" ")
        .append(eventsModel.getRepo().getName())
  ...
  private void appendIssueCommentEvent(final SpannableBuilder spannableBuilder,
                                       final Event eventsModel) {
    Comment comment = eventsModel.getPayload().getComment();
    Issue issue = eventsModel.getPayload().getIssue();
  ...
  private void appendGollumEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
  ...
  private void appendGistEvent(final SpannableBuilder spannableBuilder,
                               final Event eventsModel) {
    String action = eventsModel.getPayload().getAction();
    action = "create".equals(action)
                 ? "created"
                 : "update".equals(action) ? "updated" : action;
    spannableBuilder.bold(action)
        .append(" ")
  ...
  private void appendForkEvent(final SpannableBuilder spannableBuilder,
                               final Event eventsModel) {
    spannableBuilder.bold("forked").append(" ").append(
        eventsModel.getRepo().getName());
  }

  private void appendFollowEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
  ...
  private void appendDownloadEvent(final SpannableBuilder spannableBuilder,
                                   final Event eventsModel) {
    spannableBuilder.bold("uploaded a file")
        .append(" ")
        .append(eventsModel.getPayload().getDownload() != null
                    ? eventsModel.getPayload().getDownload().getName()
                    : "")
        .append(" ")
        .append("to")
        .append(" ")
  ...
  private void appendCreateEvent(final SpannableBuilder spannableBuilder,
                                 final Event eventsModel) {
    PayloadModel payloadModel = eventsModel.getPayload();
    String refType = payloadModel.getRefType();
    spannableBuilder.bold("created")
        .append(" ")
        .append(refType)
        .append(" ")
        .append(!"repository".equalsIgnoreCase(refType)
                    ? payloadModel.getRef() + " "
                    : "")
        .bold("at")
        .append(" ")
    ...
    if (payloadModel.getDescription() != null) {
      MarkDownProvider.stripMdText(
          description,
          payloadModel.getDescription().replaceAll("\\r?\\n|\\r", " "));
      description.setVisibility(View.VISIBLE);
    } else {
      description.setText("");
      description.setVisibility(View.GONE);
  ...
  private void appendWatch(final SpannableBuilder spannableBuilder,
                           final EventsType type, final Event eventsModel) {
    spannableBuilder.bold(resources.getString(type.getType()).toLowerCase())
        .append(" ")
        .append(eventsModel.getRepo().getName());
  }

  private void appendCommitComment(final SpannableBuilder spannableBuilder,
                                   final Event eventsModel) {
    Comment comment = eventsModel.getPayload().getCommitComment() == null
                          ? eventsModel.getPayload().getComment()
                          : eventsModel.getPayload().getCommitComment();
    String commitId = comment != null && comment.getCommitId() != null &&
                              comment.getCommitId().length() > 10
                          ? comment.getCommitId().substring(0, 10)
                          : null;
    spannableBuilder.bold("commented")
        .append(" ")
        .bold("on")
  ...
  private void appendActor(final @NonNull Event eventsModel,
                           final SpannableBuilder spannableBuilder) {
  ...
  private void appendAvatar(final @NonNull Event eventsModel) {
    if (avatar != null) {
      if (eventsModel.getActor() != null) {
        avatar.setUrl(
            eventsModel.getActor().getAvatarUrl(),
            eventsModel.getActor().getLogin(),
