...
public class TopicsAdapter
    extends BaseRecyclerAdapter<String, SimpleViewHolder<String>,
                                BaseViewHolder.OnItemClickListener<String>> {
  private boolean isLightTheme = true;
  @ColorInt private int cardBackground;

  public TopicsAdapter(final @NonNull List<String> data) { super(data); }

  @Override
  protected SimpleViewHolder<String> viewHolder(final ViewGroup parent,
                                                final int viewType) {
    ...
    return new SimpleViewHolder<>(
        BaseViewHolder.getView(parent, R.layout.topics_row_item), null);
  }

  @Override
  protected void onBindView(final SimpleViewHolder<String> holder,
                            final int position) {
    if (isLightTheme) {
    ...
    holder.itemView.setOnClickListener((view) -> {
      Intent intent = new Intent(new Intent(
          App.getInstance().getApplicationContext(), SearchActivity.class));
