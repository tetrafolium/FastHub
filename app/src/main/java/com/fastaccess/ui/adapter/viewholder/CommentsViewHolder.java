...
public class CommentsViewHolder extends BaseViewHolder<Comment> {
  ...
  private CommentsViewHolder(@NonNull View itemView,
  ...
  public static CommentsViewHolder
  newInstance(@NonNull ViewGroup viewGroup,
              @Nullable BaseRecyclerAdapter adapter) {
    return new CommentsViewHolder(
        getView(viewGroup, R.layout.no_emojies_comments_row_item), adapter,
        viewGroup);
  }

  @Override
  public void bind(@NonNull Comment commentsModel) {
    if (commentsModel.getUser() != null) {
      avatar.setUrl(
          commentsModel.getUser().getAvatarUrl(),
          commentsModel.getUser().getLogin(),
          commentsModel.getUser().isOrganizationType(),
          LinkParserHelper.isEnterprise(commentsModel.getUser().getHtmlUrl()));
    } else {
