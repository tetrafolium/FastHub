...
public class GistsViewHolder extends BaseViewHolder<Gist> {
  ...
  private GistsViewHolder(@NonNull View itemView,
  ...
  public static GistsViewHolder
  newInstance(@NonNull ViewGroup viewGroup,
              @Nullable BaseRecyclerAdapter adapter, boolean isFromProfile) {
    if (!isFromProfile) {
      return new GistsViewHolder(getView(viewGroup, R.layout.feeds_row_item),
                                 adapter, false);
    } else {
      return new GistsViewHolder(
          getView(viewGroup, R.layout.feeds_row_no_image_item), adapter, true);
    }
  }
