...
public class IssuesTimelineAdapter extends BaseRecyclerAdapter<
    TimelineModel, BaseViewHolder,
  ...
  public IssuesTimelineAdapter(
      final @NonNull List<TimelineModel> data, final OnToggleView onToggleView,
      final boolean showEmojies, final ReactionsCallback reactionsCallback,
      final boolean isMerged, final ReviewCommentCallback reviewCommentCallback,
  ...
  public IssuesTimelineAdapter(final @NonNull List<TimelineModel> data,
                               final OnToggleView onToggleView,
                               final boolean showEmojies,
                               final ReactionsCallback reactionsCallback,
                               final String repoOwner, final String poster) {
  ...
  @Override
  protected BaseViewHolder viewHolder(final ViewGroup parent,
                                      final int viewType) {
    if (viewType == 0) {
      return new UnknownTypeViewHolder(
          BaseViewHolder.getView(parent, R.layout.unknown_row_item));
    } else if (viewType == TimelineModel.HEADER) {
      return IssueDetailsViewHolder.newInstance(
          parent, this, onToggleView, reactionsCallback, repoOwner, poster);
      ...
      return GroupedReviewsViewHolder.newInstance(
          parent, this, onToggleView, reactionsCallback, reviewCommentCallback,
          repoOwner, poster);
    } else if (viewType == TimelineModel.COMMIT_COMMENTS) {
      return CommitThreadViewHolder.Companion.newInstance(parent, this,
                                                          onToggleView);
    } else if (viewType == TimelineModel.STATUS) {
      return PullStatusViewHolder.newInstance(parent);
    }
    return TimelineCommentsViewHolder.newInstance(
        parent, this, onToggleView, showEmojies, reactionsCallback, repoOwner,
        poster);
  }
  ...
  public int getItemViewType(final int position) {
    ...
    return timelineModel != null ? timelineModel.getType()
