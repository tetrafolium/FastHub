...
public class NotificationsViewHolder
  ...
  private NotificationsViewHolder(@NonNull View itemView,
                                  ...
                                  boolean showUnreadState) {
    super(itemView, adapter);
    markAsRead.setOnClickListener(this);
    this.showUnreadState = showUnreadState;
  }

  public static NotificationsViewHolder
  newInstance(@NonNull ViewGroup viewGroup,
              @Nullable BaseRecyclerAdapter adapter, boolean showUnreadState) {
    return new NotificationsViewHolder(
        getView(viewGroup, R.layout.notifications_row_item), adapter,
        showUnreadState);
  }
  ...
  public void bind(@NonNull GroupedNotificationModel model) {
    ...
    if (thread != null && thread.getSubject() != null) {
      ...
      if (thread.getSubject().getType() != null) {
        notificationType.setImageResource(
            thread.getSubject().getType().getDrawableRes());
        notificationType.setContentDescription(
            thread.getSubject().getType().name());
      } else {
      ...
      if (showUnreadState) {
        ...
        ((CardView)itemView)
            .setCardBackgroundColor(thread.isUnread() ? color : cardBackground);
      } else {
