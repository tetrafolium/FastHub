...
public class GroupedReviewsViewHolder extends BaseViewHolder<TimelineModel>
    implements BaseViewHolder.OnItemClickListener<ReviewCommentModel> {
  ...
  private GroupedReviewsViewHolder(
      @NonNull View itemView, ViewGroup viewGroup,
      @Nullable BaseRecyclerAdapter adapter, @NonNull OnToggleView onToggleView,
      @NonNull ReactionsCallback reactionsCallback,
      @NonNull PullRequestTimelineMvp
    ...
    patchAdditionColor =
        ViewHelper.getPatchAdditionColor(itemView.getContext());
    patchDeletionColor =
        ViewHelper.getPatchDeletionColor(itemView.getContext());
  ...
  public static GroupedReviewsViewHolder
  newInstance(ViewGroup viewGroup, BaseRecyclerAdapter adapter,
              ...
              @NonNull PullRequestTimelineMvp
                  .ReviewCommentCallback reviewCommentCallback,
              String repoOwner, String poster) {
    return new GroupedReviewsViewHolder(
        getView(viewGroup, R.layout.grouped_review_timeline_row_item),
  ...
  public void bind(@NonNull TimelineModel model) {
    ...
    if (groupedReviewModel.getComments() == null ||
        groupedReviewModel.getComments().isEmpty()) {
      ...
      nestedRecyclerView.setAdapter(new ReviewCommentsAdapter(
          groupedReviewModel.getComments(), this, onToggleView,
  ...
  @Override
  public void onItemClick(int position, View v, ReviewCommentModel item) {
  ...
  @Override
  public void onItemLongClick(int position, View v, ReviewCommentModel item) {
    if (reviewCommentCallback != null) {
      reviewCommentCallback.onLongClick(getAdapterPosition(), position, v,
                                        item);
    }
  }
  ...
  private long getId() { return getAdapterPosition(); }

  private void setPatchText(@NonNull String text) {
    patch.setText(DiffLineSpan.getSpannable(
        text, patchAdditionColor, patchDeletionColor, patchRefColor, true));
  }
}
