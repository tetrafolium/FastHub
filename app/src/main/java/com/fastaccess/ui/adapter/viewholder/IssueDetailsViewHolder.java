...
public class IssueDetailsViewHolder extends BaseViewHolder<TimelineModel> {
  ...
  private IssueDetailsViewHolder(final @NonNull View itemView,
  ...
  public static IssueDetailsViewHolder
  newInstance(final @NonNull ViewGroup viewGroup,
    ...
    return new IssueDetailsViewHolder(
  ...
  private void addReactionCount(final View v) {
    if (adapter != null) {
      ...
      boolean isReacted =
          ...
          reactionsCallback.isPreviouslyReacted(number, v.getId());
      boolean isCallingApi = reactionsCallback != null &&
                             reactionsCallback.isCallingApi(number, v.getId());
      switch (v.getId()) {
      case R.id.heart:
      ...
      case R.id.thumbsUpReaction:
        reactionsModel.setPlusOne(!isReacted ? reactionsModel.getPlusOne() + 1
  ...
  private void bind(final @NonNull Issue issueModel) {
    setup(issueModel.getUser(), issueModel.getBodyHtml(),
          issueModel.getReactions());
  ...
  private void bind(final @NonNull PullRequest pullRequest) {
    setup(pullRequest.getUser(), pullRequest.getBodyHtml(),
          pullRequest.getReactions());
    setupDate(pullRequest.getCreatedAt(), pullRequest.getUpdatedAt());
    setupLabels(pullRequest.getLabels());
  }

  private void setup(final User user, final String description,
    ...
    if (!InputHelper.isEmpty(description)) {
      HtmlHelper.htmlIntoTextView(
          comment, description,
          viewGroup.getWidth() - ViewHelper.dpToPx(itemView.getContext(), 24));
    } else {
      comment.setText(R.string.no_description_provided);
    }
  }

  private void setupDate(final @NonNull Date createdDate,
                         final @NonNull Date updated) {
  ...
  private void appendEmojies(final ReactionsModel reaction) {
    CommentsHelper.appendEmojies(
        ...
        hooray, hurrayReaction, sad, sadReaction, laugh, laughReaction, heart,
