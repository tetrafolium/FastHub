...
public class IssueTimelineViewHolder extends BaseViewHolder<TimelineModel> {
  ...
  private IssueTimelineViewHolder(final @NonNull View itemView,
  ...
  public static IssueTimelineViewHolder
  newInstance(final ViewGroup viewGroup, final BaseRecyclerAdapter adapter,
              final boolean isMerged) {
    return new IssueTimelineViewHolder(
        getView(viewGroup, R.layout.issue_timeline_row_item), adapter,
        isMerged);
  }
  ...
  public void bind(final @NonNull TimelineModel timelineModel) {
    ...
    if (issueEventModel.getAssignee() != null &&
      ...
      avatarLayout.setUrl(
          issueEventModel.getAssigner().getAvatarUrl(),
          issueEventModel.getAssigner().getLogin(), false,
          LinkParserHelper.isEnterprise(issueEventModel.getUrl()));
    } else {
      if (event != IssueEventType.committed) {
        avatarLayout.setVisibility(View.VISIBLE);
        if (issueEventModel.getActor() != null) {
          avatarLayout.setUrl(
              issueEventModel.getActor().getAvatarUrl(),
              issueEventModel.getActor().getLogin(), false,
              LinkParserHelper.isEnterprise(issueEventModel.getUrl()));
        } else if (issueEventModel.getAuthor() != null) {
          avatarLayout.setUrl(
              issueEventModel.getAuthor().getAvatarUrl(),
              issueEventModel.getAuthor().getLogin(), false,
              LinkParserHelper.isEnterprise(issueEventModel.getUrl()));
        }
      } else {
    ...
    if (event != null) {
      stateText.setText(TimelineProvider.getStyledEvents(
          issueEventModel, itemView.getContext(), isMerged));
    } else {
      stateText.setText("");
  ...
  protected void onViewIsDetaching() {
    DrawableGetter drawableGetter =
        (DrawableGetter)stateText.getTag(R.id.drawable_callback);
