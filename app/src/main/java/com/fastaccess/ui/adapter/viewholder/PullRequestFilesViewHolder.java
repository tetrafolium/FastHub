...
public class PullRequestFilesViewHolder
    extends BaseViewHolder<CommitFileChanges>
    implements BaseViewHolder.OnItemClickListener<CommitLinesModel> {
  ...
  private PullRequestFilesViewHolder(
  ...
  public static PullRequestFilesViewHolder newInstance(
      ViewGroup viewGroup, BaseRecyclerAdapter adapter,
      @NonNull OnToggleView onToggleView,
      @Nullable PullRequestFilesMvp.OnPatchClickListener onPatchClickListener) {
    return new PullRequestFilesViewHolder(
        getView(viewGroup, R.layout.pullrequest_file_row_item), viewGroup,
        adapter, onToggleView, onPatchClickListener);
  }

  @Override
  public void bind(@NonNull CommitFileChanges commitFileChanges) {
      ...
      if (adapter != null) {
        ...
        if (model.getLinesModel() != null && !model.getLinesModel().isEmpty()) {
          if (model.getLinesModel().size() <= 100) {
            patch.setAdapter(
                new CommitLinesAdapter(model.getLinesModel(), this));
  ...
  @Override
  public void onItemClick(int position, View v, CommitLinesModel item) {
    if (onPatchClickListener != null && adapter != null) {
      int groupPosition = getAdapterPosition();
      CommitFileChanges commitFileChanges =
          (CommitFileChanges)adapter.getItem(groupPosition);
      onPatchClickListener.onPatchClicked(
          groupPosition, position, v, commitFileChanges.getCommitFileModel(),
          item);
    }
  }
  ...
  public void onItemLongClick(int position, View v, CommitLinesModel item) {
    ...
    CommitFileChanges commitFileChanges =
        (CommitFileChanges)adapter.getItem(groupPosition);
    int lineNo =
        item.getLeftLineNo() > 0 ? item.getLeftLineNo() : item.getRightLineNo();
    String url =
        commitFileChanges.getCommitFileModel().getBlobUrl() + "#L" + lineNo;
