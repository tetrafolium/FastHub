...
public class IssuesViewHolder extends BaseViewHolder<Issue> {
  ...
  private IssuesViewHolder(final @NonNull View itemView,
  ...
  public static IssuesViewHolder newInstance(final ViewGroup viewGroup,
  ...
  public static IssuesViewHolder newInstance(final ViewGroup viewGroup,
                                             ...
                                             final boolean showState) {
    if (withAvatar) {
      return new IssuesViewHolder(getView(viewGroup, R.layout.issue_row_item),
                                  adapter, true, showRepoName, showState);
    } else {
      return new IssuesViewHolder(
          getView(viewGroup, R.layout.issue_no_image_row_item), adapter, false,
          showRepoName, showState);
    }
  }

  @Override
  public void bind(final @NonNull Issue issueModel) {
    title.setText(issueModel.getTitle());
    if (issueModel.getState() != null) {
      CharSequence data = ParseDateFormat.getTimeAgo(
          issueModel.getState() == IssueState.open ? issueModel.getCreatedAt()
                                                   : issueModel.getClosedAt());
      SpannableBuilder builder = SpannableBuilder.builder();
      if (showRepoName) {
        PullsIssuesParser parser =
            PullsIssuesParser.getForIssue(issueModel.getHtmlUrl());
        if (parser != null)
      ...
      details.setText(
          builder
              .append(itemView.getResources()
                          .getString(issueModel.getState().getStatus())
                          .toLowerCase())
              .append(" ")
              .append(data));
    ...
    if (withAvatar && avatarLayout != null) {
      avatarLayout.setUrl(
          issueModel.getUser().getAvatarUrl(), issueModel.getUser().getLogin(),
          false,
