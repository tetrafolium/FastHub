...
public class ReposViewHolder extends BaseViewHolder<Repo> {
  ...
  private ReposViewHolder(final @NonNull View itemView,
                          final @Nullable BaseRecyclerAdapter adapter,
                          final boolean isStarred, final boolean withImage) {
    super(itemView, adapter);
    this.isStarred = isStarred;
    this.withImage = withImage;
  }

  public static ReposViewHolder newInstance(final ViewGroup viewGroup,
                                            final BaseRecyclerAdapter adapter,
                                            final boolean isStarred,
                                            final boolean withImage) {
    if (withImage) {
      return new ReposViewHolder(getView(viewGroup, R.layout.repos_row_item),
                                 adapter, isStarred, true);
    } else {
      return new ReposViewHolder(
          getView(viewGroup, R.layout.repos_row_no_image_item), adapter,
          isStarred, false);
    }
  }

  @Override
  public void bind(final @NonNull Repo repo) {
    ...
    if (withImage) {
      String avatar =
          repo.getOwner() != null ? repo.getOwner().getAvatarUrl() : null;
      String login =
          repo.getOwner() != null ? repo.getOwner().getLogin() : null;
      boolean isOrg =
          repo.getOwner() != null && repo.getOwner().isOrganizationType();
      if (avatarLayout != null) {
        avatarLayout.setVisibility(View.VISIBLE);
        avatarLayout.setUrl(avatar, login, isOrg,
                            LinkParserHelper.isEnterprise(repo.getHtmlUrl()));
      }
    }
    long repoSize =
        repo.getSize() > 0 ? (repo.getSize() * 1000) : repo.getSize();
    ...
    if (!InputHelper.isEmpty(repo.getLanguage())) {
      ...
      language.setTextColor(ColorsProvider.getColorAsColor(
          repo.getLanguage(), language.getContext()));
