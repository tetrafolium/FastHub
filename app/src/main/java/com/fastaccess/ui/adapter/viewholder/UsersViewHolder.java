...
public class UsersViewHolder extends BaseViewHolder<User> {
  ...
  public static UsersViewHolder
  newInstance(@NonNull ViewGroup parent, @Nullable BaseRecyclerAdapter adapter,
              boolean isFilter) {
    return new UsersViewHolder(
        getView(parent, isFilter ? R.layout.users_small_row_item
                                 : R.layout.feeds_row_item),
        adapter, isFilter);
  }
  ...
  public void bind(@NonNull User user, boolean isContributor) {
    ...
    if (isContributor) {
      date.setText(String.format(
          "%s (%s)", date.getResources().getString(R.string.commits),
