...
public class RepoFilesViewHolder extends BaseViewHolder<RepoFile> {
  ...
  private RepoFilesViewHolder(final @NonNull View itemView,
  ...
  public static RepoFilesViewHolder
  newInstance(final ViewGroup viewGroup, final BaseRecyclerAdapter adapter) {
    return new RepoFilesViewHolder(
        getView(viewGroup, R.layout.repo_files_row_item), adapter);
  }

  @Override
  public void bind(final @NonNull RepoFile filesModel) {
    contentTypeImage.setContentDescription(
        String.format("%s %s", filesModel.getName(), file));
    ...
    if (filesModel.getType() != null && filesModel.getType().getIcon() != 0) {
      ...
      if (filesModel.getType() == FilesType.file) {
        size.setText(
            Formatter.formatFileSize(size.getContext(), filesModel.getSize()));
