  ...
  public void onClick(final View v) {
    if (onSelectAssignee != null) {
      ...
      onSelectAssignee.onToggleSelection(
          position, !onSelectAssignee.isAssigneeSelected(position));
    } else {
      super.onClick(v);
    }
  }

  private AssigneesViewHolder(
      final @NonNull View itemView,
      final @Nullable AssigneesAdapter.OnSelectAssignee onSelectAssignee,
      final @NonNull BaseRecyclerAdapter adapter) {
    super(itemView, adapter);
    this.onSelectAssignee = onSelectAssignee;
  }

  public static AssigneesViewHolder
  newInstance(final @NonNull ViewGroup viewGroup,
              final
              @Nullable AssigneesAdapter.OnSelectAssignee onSelectAssignee,
              final @NonNull BaseRecyclerAdapter adapter) {
    return new AssigneesViewHolder(getView(viewGroup, R.layout.feeds_row_item),
                                   onSelectAssignee, adapter);
  }
  ...
  public void bind(final @NonNull User user) {
    ...
    if (onSelectAssignee != null) {
      itemView.setBackgroundColor(
          onSelectAssignee.isAssigneeSelected(getAdapterPosition())
              ? lightGray
              : ViewHelper.getWindowBackground(itemView.getContext()));
    }
  }
}
