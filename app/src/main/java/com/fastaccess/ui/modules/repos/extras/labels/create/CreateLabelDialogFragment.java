...
public class CreateLabelDialogFragment
    extends BaseDialogFragment<CreateLabelMvp.View, CreateLabelPresenter>
    implements CreateLabelMvp.View {
  ...
  public static CreateLabelDialogFragment
  ...
  @Override
  public void onSuccessfullyCreated(final @NonNull LabelModel labelModel1) {
    hideProgress();
    if (callback != null)
      callback.onLabelAdded(labelModel1);
    dismiss();
  }

  @Override
  public void onColorSelected(final @NonNull String color) {
  ...
  protected void onFragmentCreated(final @NonNull View view,
    ...
    recycler.setAdapter(new LabelColorsAdapter(
        Arrays.asList(getResources().getStringArray(R.array.label_colors)),
        getPresenter()));
    ...
    toolbar.setOnMenuItemClickListener(item -> {
      ...
      description.setError(emptyColor ? getString(R.string.required_field)
                                      : null);
      name.setError(emptyName ? getString(R.string.required_field) : null);
      if (!emptyColor && !emptyName) {
        getPresenter().onSubmitLabel(InputHelper.toString(name),
                                     InputHelper.toString(description), repo,
                                     login);
      }
      return true;
    });
