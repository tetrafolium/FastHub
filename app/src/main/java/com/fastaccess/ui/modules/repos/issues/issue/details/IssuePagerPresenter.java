...
class IssuePagerPresenter extends BasePresenter<IssuePagerMvp.View>
    implements IssuePagerMvp.Presenter {
  ...
  @Override
  public void onWorkOffline(final long issueNumber,
                            final @NonNull String repoId,
                            final @NonNull String login) {
    if (issueModel == null) {
      manageDisposable(RxHelper
                           .getObservable(Issue.getIssueByNumber(
                               (int)issueNumber, repoId, login))
  ...
  public boolean isOwner() {
    ...
    PullsIssuesParser parser =
        PullsIssuesParser.getForIssue(getIssue().getHtmlUrl());
  ...
  public void onOpenCloseIssue() {
    ...
    if (currentIssue != null) {
      IssueRequestModel requestModel =
          IssueRequestModel.clone(currentIssue, true);
      manageDisposable(
          RxHelper
              .getObservable(
                  RestProvider.getIssueService(isEnterprise())
                      .editIssue(login, repoId, issueNumber, requestModel))
              .doOnSubscribe(
                ...
                if (issue != null) {
                  sendToView(view
                             -> view.showSuccessIssueActionMsg(
                                 currentIssue.getState() == IssueState.open));
  ...
  @Override
  public void onLockUnlockIssue(final String reason) {
    Issue currentIssue = getIssue();
    if (currentIssue == null)
      return;
    ...
    Observable<Response<Boolean>> observable = RxHelper.getObservable(
        model == null ? issueService.unlockIssue(login, repoId, number)
  ...
  public void onPutMilestones(final @NonNull MilestoneModel milestone) {
    ...
    IssueRequestModel issueRequestModel =
        IssueRequestModel.clone(issueModel, false);
    makeRestCall(
        RestProvider.getIssueService(isEnterprise())
  ...
  public void onPutAssignees(final @NonNull ArrayList<User> users) {
    ...
    assigneesRequestModel.setAssignees(
        assignees.isEmpty()
            ? Stream.of(issueModel.getAssignees()).map(User::getLogin).toList()
            : assignees);
    makeRestCall(
        !assignees.isEmpty() ? RestProvider.getIssueService(isEnterprise())
  ...
  @Override
  public void onUpdateIssue(final @NonNull Issue issue) {
  ...
  private void getIssueFromApi() {
    ...
    if (loginUser == null)
      return;
    makeRestCall(RxHelper.getObservable(Observable.zip(
                     RestProvider.getIssueService(isEnterprise())
                         .getIssue(login, repoId, issueNumber),
                     RestProvider.getRepoService(isEnterprise())
                         .isCollaborator(login, repoId, loginUser.getLogin()),
  ...
  private void updateTimeline(final IssuePagerMvp.View view,
