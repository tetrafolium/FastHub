...
public class RepoPagerActivity
    extends BaseActivity<RepoPagerMvp.View, RepoPagerPresenter>
  ...
  public static void startRepoPager(final @NonNull Context context,
                                    final @NonNull NameParser nameParser) {
    if (!InputHelper.isEmpty(nameParser.getName()) &&
        !InputHelper.isEmpty(nameParser.getUsername())) {
  ...
  createIntent(final @NonNull Context context, final @NonNull String repoId,
               final @NonNull String login,
  ...
  void onFabClicked() {
    if (navType == RepoPagerMvp.ISSUES) {
      ...
      RepoPullRequestPagerFragment pullRequestPagerView =
          (RepoPullRequestPagerFragment)AppHelper.getFragmentByTag(
              getSupportFragmentManager(), RepoPullRequestPagerFragment.TAG);
      if (pullRequestPagerView != null) {
        FilterIssuesActivity.startActivity(
            this, getPresenter().login(), getPresenter().repoId(), false,
  ...
  void onTitleClick() {
    ...
    if (repoModel != null && !InputHelper.isEmpty(repoModel.getDescription())) {
      MessageDialogView
          .newInstance(repoModel.getFullName(), repoModel.getDescription(),
                       false, true)
  ...
  void onTagsClick() {
    if (topicsList.getAdapter().getItemCount() > 0) {
      ...
      topicsList.setVisibility(topicsList.getVisibility() == View.VISIBLE
                                   ? View.GONE
                                   : View.VISIBLE);
    }
  }
  ...
  void
  onClick(final View view) {
    switch (view.getId()) {
    case R.id.forkRepoLayout:
      MessageDialogView
          .newInstance(getString(R.string.fork),
                       String.format("%s %s/%s?", getString(R.string.fork),
                                     login, repoId),
                       Bundler.start()
    ...
    case R.id.starRepoLayout:
      if (!InputHelper.isEmpty(getPresenter().login()) &&
          !InputHelper.isEmpty(getPresenter().repoId())) {
    ...
    case R.id.wikiLayout:
      ActivityHelper.startReveal(
          this, WikiActivity.Companion.getWiki(this, repoId, login),
          wikiLayout);
      break;
    case R.id.licenseLayout:
      if (getPresenter().getRepo() != null) {
        LicenseModel licenseModel = getPresenter().getRepo().getLicense();
        String license = !InputHelper.isEmpty(licenseModel.getSpdxId())
                             ? licenseModel.getSpdxId()
                             : licenseModel.getName();
        RepoLicenseBottomSheet.Companion
            .newInstance(getPresenter().login(), getPresenter().repoId(),
                         license)
  ...
  boolean onLongClick(final View view) {
    switch (view.getId()) {
    case R.id.forkRepoLayout:
      RepoMiscDialogFragment.show(getSupportFragmentManager(), login, repoId,
                                  RepoMiscMVp.FORKS);
      return true;
    case R.id.starRepoLayout:
      RepoMiscDialogFragment.show(getSupportFragmentManager(), login, repoId,
  ...
  void onSortIssues(final boolean isChecked) {
    RepoIssuesPagerFragment pagerView =
        (RepoIssuesPagerFragment)AppHelper.getFragmentByTag(
            getSupportFragmentManager(), RepoIssuesPagerFragment.TAG);
    if (pagerView != null) {
  ...
  @Override
  public void
  onNavigationChanged(final @RepoPagerMvp.RepoNavigationType int navType) {
  ...
  public void onFinishActivity() {
    // do nothing here, github might return 404 if even the repo don't have
    // anything but issues.
  }
  ...
  public void onInitRepo() {
    ...
    detailsIcon.setVisibility(InputHelper.isEmpty(repoModel.getDescription())
                                  ? View.GONE
                                  : View.VISIBLE);
    language.setVisibility(InputHelper.isEmpty(repoModel.getLanguage())
                               ? View.GONE
                               : View.VISIBLE);
    if (!InputHelper.isEmpty(repoModel.getLanguage())) {
      language.setText(repoModel.getLanguage());
      language.setTextColor(ColorsProvider.getColorAsColor(
          repoModel.getLanguage(), language.getContext()));
    }
    ...
    if (repoModel.getOwner() != null) {
      avatarLayout.setUrl(
          repoModel.getOwner().getAvatarUrl(), repoModel.getOwner().getLogin(),
      ...
      avatarLayout.setUrl(
          repoModel.getOrganization().getAvatarUrl(),
          repoModel.getOrganization().getLogin(), true,
          LinkParserHelper.isEnterprise(repoModel.getHtmlUrl()));
    }
    long repoSize = repoModel.getSize() > 0 ? (repoModel.getSize() * 1000)
                                            : repoModel.getSize();
    date.setText(
    ...
    if (repoModel.getLicense() != null) {
      ...
      license.setText(!InputHelper.isEmpty(licenseModel.getSpdxId())
                          ? licenseModel.getSpdxId()
                          : licenseModel.getName());
    }
  ...
  @Override
  public void onRepoWatched(final boolean isWatched) {
    watchRepoImage.tintDrawableColor(isWatched ? accentColor : iconColor);
    onEnableDisableWatch(true);
  }

  @Override
  public void onRepoStarred(final boolean isStarred) {
    starRepoImage.setImageResource(isStarred ? R.drawable.ic_star_filled
                                             : R.drawable.ic_star);
  ...
  @Override
  public void onRepoForked(final boolean isForked) {
    forkRepoImage.tintDrawableColor(isForked ? accentColor : iconColor);
    onEnableDisableFork(true);
  }

  @Override
  public void onRepoPinned(final boolean isPinned) {
    pinImage.setImageResource(isPinned ? R.drawable.ic_pin_filled
                                       : R.drawable.ic_pin);
    pinLayout.setEnabled(true);
  }

  @Override
  public void onEnableDisableWatch(final boolean isEnabled) {
    watchRepoLayout.setEnabled(isEnabled);
  }

  @Override
  public void onEnableDisableStar(final boolean isEnabled) {
    starRepoLayout.setEnabled(isEnabled);
  }

  @Override
  public void onEnableDisableFork(final boolean isEnabled) {
    forkRepoLayout.setEnabled(isEnabled);
  }

  @Override
  public void onChangeWatchedCount(final boolean isWatched) {
    long count = InputHelper.toLong(watchRepo);
    watchRepo.setText(numberFormat.format(
        isWatched ? (count + 1) : (count > 0 ? (count - 1) : 0)));
    updatePinnedRepo();
  }

  @Override
  public void onChangeStarCount(final boolean isStarred) {
    long count = InputHelper.toLong(starRepo);
    starRepo.setText(numberFormat.format(
        isStarred ? (count + 1) : (count > 0 ? (count - 1) : 0)));
    updatePinnedRepo();
  }

  @Override
  public void onChangeForkCount(final boolean isForked) {
    long count = InputHelper.toLong(forkRepo);
    forkRepo.setText(numberFormat.format(
        isForked ? (count + 1) : (count > 0 ? (count - 1) : 0)));
    updatePinnedRepo();
  }
  ...
  public void openUserProfile() {
    UserPagerActivity.startActivity(this, Login.getUser().getLogin(), false,
                                    PrefGetter.isEnterprise(), -1);
  }
  ...
  public boolean onPrepareOptionsMenu(final Menu menu) {
    Repo repoModel = getPresenter().getRepo();
    if (repoModel != null && repoModel.isFork() &&
        repoModel.getParent() != null) {
    ...
    if (menu.findItem(R.id.deleteRepo) != null)
      menu.findItem(R.id.deleteRepo)
          .setVisible(false); // removing delete permission.
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    if (item.getItemId() == android.R.id.home) {
      ...
      if (getPresenter().getRepo() != null)
        ActivityHelper.startCustomTab(this,
                                      getPresenter().getRepo().getHtmlUrl());
      return true;
      ...
      if (getPresenter().getRepo() != null &&
          getPresenter().getRepo().getParent() != null) {
      ...
      MessageDialogView
          .newInstance(getString(R.string.delete_repo),
                       getString(R.string.delete_repo_warning),
                       Bundler.start()
  ...
  public void onBackPressed() {
    if (navType == RepoPagerMvp.CODE) {
      RepoCodePagerFragment codePagerView =
          (RepoCodePagerFragment)AppHelper.getFragmentByTag(
              getSupportFragmentManager(), RepoCodePagerFragment.TAG);
  ...
  public void onSearchSelected() {
    ...
    FilterIssuesActivity.startActivity(this, getPresenter().login(),
                                       getPresenter().repoId(), true, isOpen,
                                       isEnterprise());
  }
  ...
  private void hideFilterLayout() {
    AnimHelper.mimicFabVisibility(
        false, filterLayout,
        new FloatingActionButton.OnVisibilityChangedListener() {
          @Override
          public void onHidden(final FloatingActionButton actionButton) {
            fab.show();
          }
        });
  }
  ...
  private void updatePinnedRepo() {
    getPresenter().updatePinned((int)InputHelper.toLong(forkRepo),
                                (int)InputHelper.toLong(starRepo),
