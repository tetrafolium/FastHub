...
class RepoReleasesPresenter extends BasePresenter<RepoReleasesMvp.View>
    implements RepoReleasesMvp.Presenter {
  ...
  public void onFragmentCreated(final @NonNull Bundle bundle) {
    ...
    if (!InputHelper.isEmpty(tag)) {
      manageObservable(RestProvider.getRepoService(isEnterprise())
                           .getTagRelease(login, repoId, tag)
                           .doOnNext(release -> {
                             if (release != null) {
  ...
  public void onWorkOffline() {
    if (releases.isEmpty()) {
      manageDisposable(
          RxHelper.getSingle(Release.get(repoId, login))
              .subscribe(releasesModels
                         -> sendToView(
                             view -> view.onNotifyAdapter(releasesModels, 1))));
    } else {
      sendToView(RepoReleasesMvp.View::hideProgress);
    }
  }
