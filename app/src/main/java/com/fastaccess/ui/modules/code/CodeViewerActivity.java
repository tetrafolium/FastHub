...
public class CodeViewerActivity extends BaseActivity {
  ...
  public static void startActivity(final @NonNull Context context,
    ...
    if (!InputHelper.isEmpty(url)) {
      Intent intent =
          ActivityHelper.editBundle(createIntent(context, url, htmlUrl),
  ...
  protected void onCreate(final @Nullable Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      ...
      url = Objects.requireNonNull(bundle.getString(BundleConstant.EXTRA),
                                   "Url is null");
      htmlUrl = bundle.getString(BundleConstant.EXTRA_TWO);
      getSupportFragmentManager()
          .beginTransaction()
          .replace(R.id.container, ViewerFragment.newInstance(url, htmlUrl),
                   ViewerFragment.TAG)
          .commit();
    }
    ...
    setTitle(title);
    if (toolbar != null)
      toolbar.setSubtitle(MimeTypeMap.getFileExtensionFromUrl(url));
    setTaskName(title);
  }
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    ...
    if (item.getItemId() == R.id.viewAsCode) {
      ViewerFragment viewerFragment =
          (ViewerFragment)AppHelper.getFragmentByTag(
              getSupportFragmentManager(), ViewerFragment.TAG);
