...
class RepoPullRequestPresenter extends BasePresenter<RepoPullRequestMvp.View>
    implements RepoPullRequestMvp.Presenter {
  ...
  public boolean onCallApi(int page, @Nullable IssueState parameter) {
    ...
    makeRestCall(RestProvider.getPullRequestService(isEnterprise())
                     .getPullRequests(login, repoId, parameter.name(), page),
                 response -> {
                   lastPage = response.getLast();
                   if (getCurrentPage() == 1) {
                     manageDisposable(
                         PullRequest.save(response.getItems(), login, repoId));
                   }
                   sendToView(
                       view -> view.onNotifyAdapter(response.getItems(), page));
                 });
    return true;
  }
  ...
  public void onWorkOffline() {
    if (pullRequests.isEmpty()) {
      manageDisposable(
          RxHelper
              .getSingle(PullRequest.getPullRequests(repoId, login, issueState))
  ...
  @Override
  public void onItemClick(int position, View v, PullRequest item) {
    PullsIssuesParser parser =
        PullsIssuesParser.getForPullRequest(item.getHtmlUrl());
  ...
  @Override
  public void onItemLongClick(int position, View v, PullRequest item) {
    if (getView() != null)
      getView().onShowPullRequestPopup(item);
  }
}
