...
public class NotificationActivity
    extends BaseActivity implements OnNotificationChangedListener {

  @BindView(R.id.tabs) TabLayout tabs;
  @BindView(R.id.notificationContainer) ViewPagerView pager;
  ...
  @Override
  public void
  onNotificationChanged(final @NonNull GroupedNotificationModel notification,
                        final int index) {
    if (pager != null && pager.getAdapter() != null) {
      if (index == 0) {
        UnreadNotificationsFragment fragment =
            (UnreadNotificationsFragment)pager.getAdapter().instantiateItem(
                pager, 0);
        fragment.onNotifyNotificationChanged(notification);
      } else {
        AllNotificationsFragment fragment =
            (AllNotificationsFragment)pager.getAdapter().instantiateItem(pager,
                                                                         1);
        fragment.onNotifyNotificationChanged(notification);
      }
    }
