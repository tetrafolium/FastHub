...
public class RepoFilesFragment
    extends BaseFragment<RepoFilesMvp.View, RepoFilesPresenter>
    implements RepoFilesMvp.View {
  ...
  @Override
  public void onItemClicked(@NonNull RepoFile model) {
    ...
    if (model.getType() == FilesType.dir) {
      ...
      if (model.getSize() == 0 && InputHelper.isEmpty(model.getDownloadUrl()) &&
          !InputHelper.isEmpty(model.getGitUrl())) {
        RepoFilesActivity.startActivity(getContext(),
                                        model.getGitUrl().replace("trees/", ""),
                                        isEnterprise());
      } else {
        String url = InputHelper.isEmpty(model.getDownloadUrl())
                         ? model.getUrl()
                         : model.getDownloadUrl();
        if (InputHelper.isEmpty(url))
          return;
        if (model.getSize() > FileHelper.ONE_MB &&
            !MarkDownProvider.isImage(url)) {
          MessageDialogView
              .newInstance(
                  getString(R.string.big_file),
                  getString(R.string.big_file_description), false, true,
                  Bundler.start()
  ...
  @Override
  public void onMenuClicked(int position, @NonNull RepoFile item, View v) {
    if (login == null) {
      login = Login.getUser();
    }
    if (refresh.isRefreshing())
    ...
    popup.getMenu()
        .findItem(R.id.download)
        .setVisible(item.getType() == FilesType.file);
    boolean canOpen = canOpen(item);
    popup.getMenu()
        .findItem(R.id.editFile)
        .setVisible(isOwner && item.getType() == FilesType.file && canOpen);
    popup.getMenu()
        .findItem(R.id.deleteFile)
        .setVisible(isOwner && item.getType() == FilesType.file);
    ...
    popup.setOnMenuItemClickListener(item1 -> {
      ...
      case R.id.download:
        ...
        break;
      case R.id.copy:
        AppHelper.copyToClipboard(v.getContext(),
      ...
      case R.id.editFile:
        if (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked()) {
          if (canOpen) {
            EditRepoFileModel fileModel = new EditRepoFileModel(
                getPresenter().login, getPresenter().repoId, item.getPath(),
                getPresenter().ref, item.getSha(), item.getDownloadUrl(),
                item.getName(), true);
            EditRepoFileActivity.Companion.startForResult(this, fileModel,
                                                          isEnterprise());
          }
        } else {
      ...
      case R.id.deleteFile:
        if (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked()) {
          DeleteFileBottomSheetFragment.Companion
              .newInstance(position, item.getName())
              .show(getChildFragmentManager(),
                    DeleteFileBottomSheetFragment.class.getSimpleName());
        } else {
          PremiumActivity.Companion.startActivity(getContext());
        }
        break;
  ...
  @Override
  public void onSetData(@NonNull String login, @NonNull String repoId,
                        @NonNull String path, @NonNull String ref,
  ...
  @Override
  public void onUpdateTab(@Nullable RepoFile toAppend) {
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (resultCode == Activity.RESULT_OK &&
        requestCode == EditRepoFileActivity.EDIT_RQ) {
      onRefresh();
    }
  }

  @Override
  public void onDelete(@NonNull String message, int position) {
    getPresenter().onDeleteFile(message, adapter.getItem(position),
                                getParent() != null ? getParent().getRef()
                                                    : "master");
  }
  ...
  private boolean canOpen(@NonNull RepoFile item) {
    return item.getDownloadUrl() != null &&
        !MarkDownProvider.isImage(item.getDownloadUrl()) &&
