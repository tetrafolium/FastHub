...
class RepoIssuesPresenter extends BasePresenter<RepoIssuesMvp.View>
    implements RepoIssuesMvp.Presenter {
  ...
  public boolean onCallApi(final int page,
                           final @Nullable IssueState parameter) {
    if (parameter == null) {
    ...
    makeRestCall(
        RestProvider.getIssueService(isEnterprise())
            ...
            .flatMap(issues -> {
              ...
              List<Issue> filtered =
                  Stream.of(issues.getItems())
                      .filter(issue -> issue.getPullRequest() == null)
                      .toList();
              if (filtered != null) {
                if (filtered.size() < 10 && issues.getNext() > 1) {
                  setCurrentPage(getCurrentPage() + 1);
                  return grabMoreIssues(filtered, parameter.name(), finalSortBy,
                                        getCurrentPage());
                }
  ...
  @Override
  public void onFragmentCreated(final @NonNull Bundle bundle,
  ...
  public void onWorkOffline() {
    if (issues.isEmpty()) {
      manageDisposable(
          RxHelper.getSingle(Issue.getIssues(repoId, login, issueState))
  ...
  @Override
  public void onSetSortBy(final boolean isLastUpdated) {
            ...
            .getObservable(RestProvider.getIssueService(isEnterprise())
                               .getIssuesWithCount(
                                   RepoQueryProvider.getIssuesPullRequestQuery(
                                       login, repoId, issueState, false),
                                   1))
            .subscribe(pullRequestPageable
                       -> sendToView(view
                                     -> view.onUpdateCount(
                                         pullRequestPageable.getTotalCount())),
                       Throwable::printStackTrace));
  }

  private Observable<List<Issue>>
  grabMoreIssues(final @NonNull List<Issue> issues, final @NonNull String state,
                 final @NonNull String sortBy, final int page) {
    return RestProvider.getIssueService(isEnterprise())
        .getRepositoryIssues(login, repoId, state, sortBy, page)
        .flatMap(issuePageable -> {
          if (issuePageable != null) {
            lastPage = issuePageable.getLast();
            List<Issue> filtered =
                Stream.of(issuePageable.getItems())
                    .filter(issue -> issue.getPullRequest() == null)
                    .toList();
            if (filtered != null) {
