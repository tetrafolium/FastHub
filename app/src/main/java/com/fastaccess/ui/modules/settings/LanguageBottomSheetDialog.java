...
public class LanguageBottomSheetDialog extends BaseBottomSheetDialog {
  ...
  public static final String TAG =
      LanguageBottomSheetDialog.class.getSimpleName();
  ...
  public void onViewCreated(final @NonNull View view,
    ...
    String[] values =
        getResources().getStringArray(R.array.languages_array_values);
    List<AppLanguageModel> languageModels =
        Stream.of(getResources().getStringArray(R.array.languages_array))
    ...
    for (int i = 0; i < languageModels.size(); i++) {
      ...
      if (model.getValue().equalsIgnoreCase(language))
        radioGroup.check(i);
    }
    radioGroup.setOnCheckedChangeListener((group, checkedId) -> {
      ...
      if (!tag.equalsIgnoreCase(language)) {
        ...
        if (listener != null)
          listener.onLanguageChanged(this::dismiss);
      }
    });
  }
}
