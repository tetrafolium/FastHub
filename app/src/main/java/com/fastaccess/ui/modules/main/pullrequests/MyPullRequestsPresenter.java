...
public class MyPullRequestsPresenter
    extends BasePresenter<MyPullRequestsMvp.View>
    implements MyPullRequestsMvp.Presenter {
  ...
  MyPullRequestsPresenter() { setEnterprise(PrefGetter.isEnterprise()); }
  ...
  @Override
  public void onSetPullType(@NonNull MyIssuesType issuesType) {
  ...
  public boolean onCallApi(int page, @Nullable IssueState parameter) {
    ...
    makeRestCall(
        RestProvider.getPullRequestService(isEnterprise())
            .getPullsWithCount(getUrl(parameter), page),
        response -> {
  ...
  @NonNull
  private String getUrl(@NonNull IssueState parameter) {
    switch (issuesType) {
    case CREATED:
      return RepoQueryProvider.getMyIssuesPullRequestQuery(login, parameter,
                                                           true);
    case ASSIGNED:
