...
public class SearchActivity
    extends BaseActivity<SearchMvp.View, SearchPresenter>
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    searchEditText.setOnItemClickListener(
        (parent, view, position,
         id) -> getPresenter().onSearchClicked(pager, searchEditText));
  ...
  @Override
  public void onNotifyAdapter(final @Nullable SearchHistory query) {
    if (query == null)
  ...
  @Override
  public void onSetCount(final int count, final int index) {
    ...
    if (index == 0) {
      textView.setText(String.format("%s(%s)", getString(R.string.repos),
                                     numberFormat.format(count)));
    } else if (index == 1) {
      textView.setText(String.format("%s(%s)", getString(R.string.users),
                                     numberFormat.format(count)));
    } else if (index == 2) {
      textView.setText(String.format("%s(%s)", getString(R.string.issues),
                                     numberFormat.format(count)));
    } else if (index == 3) {
      textView.setText(String.format("%s(%s)", getString(R.string.code),
                                     numberFormat.format(count)));
    }
  }
