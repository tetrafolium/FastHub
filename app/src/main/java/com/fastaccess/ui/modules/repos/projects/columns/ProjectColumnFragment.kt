...
class ProjectColumnFragment : BaseFragment<ProjectColumnMvp.View, ProjectColumnPresenter>(), ProjectColumnMvp.View {
    ...
    override fun onCreatedOrEdited(text: String, isCard: Boolean, position: Int) {
        ...
        if (!isCard) {
            ...
            getColumn()?.let { presenter.onEditOrDeleteColumn(text, it) }
        } else {
            if (position == -1) {
                getColumn()?.let { presenter.createCard(text, it.id) }
            } else {
        ...
        if (isOk) {
            if (bundle != null) {
                if (bundle.containsKey(BundleConstant.ID)) {
                    ...
                    getColumn()?.let { presenter.onEditOrDeleteColumn(null, it) }
                }
            } else {
                getColumn()?.let { presenter.onEditOrDeleteColumn(null, it) }
            }
    ...
    override fun deleteColumn() {
        getColumn()?.let { pageCallback?.onDeletePage(it) }
