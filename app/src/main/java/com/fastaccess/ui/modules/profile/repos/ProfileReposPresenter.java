...
class ProfileReposPresenter extends BasePresenter<ProfileReposMvp.View>
    implements ProfileReposMvp.Presenter {
  ...
  public boolean onCallApi(final int page, final @Nullable String parameter) {
    ...
    makeRestCall(
        isProfile ? RestProvider.getUserService(isEnterprise())
                        .getRepos(filterOptions.getQueryMap(), page)
                  : RestProvider.getUserService(isEnterprise())
                        .getRepos(parameter, filterOptions.getQueryMap(), page),
        repoModelPageable -> {
          lastPage = repoModelPageable.getLast();
          if (getCurrentPage() == 1) {
            manageDisposable(
                Repo.saveMyRepos(repoModelPageable.getItems(), parameter));
          }
          sendToView(
              view -> view.onNotifyAdapter(repoModelPageable.getItems(), page));
        });
    return true;
  }
  ...
  public void onWorkOffline(final @NonNull String login) {
    if (repos.isEmpty()) {
      manageDisposable(
          RxHelper.getObservable(Repo.getMyRepos(login).toObservable())
              .subscribe(
                  repoModels
  ...
  @Override
  public void onItemClick(final int position, final View v, final Repo item) {
  ...
  FilterOptionsModel getFilterOptions() { return filterOptions; }
}
