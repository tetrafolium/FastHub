...
class RepoCommitsPresenter extends BasePresenter<RepoCommitsMvp.View>
    implements RepoCommitsMvp.Presenter {
  ...
  public boolean onCallApi(final int page, final @Nullable Object parameter) {
    ...
    if (repoId == null || login == null)
      return false;
    Observable<Pageable<Commit>> observable =
        InputHelper.isEmpty(path)
            ? RestProvider.getRepoService(isEnterprise())
                  .getCommits(login, repoId, branch, page)
            : RestProvider.getRepoService(isEnterprise())
                  .getCommits(login, repoId, branch, path, page);
    makeRestCall(observable, response -> {
      ...
      sendToView(view
                 -> view.onNotifyAdapter(
                     response != null ? response.getItems() : null, page));
    });
    return true;
  }
  ...
  public void onWorkOffline() {
    if (commits.isEmpty()) {
      manageDisposable(
          RxHelper
              .getObservable(Commit.getCommits(repoId, login).toObservable())
              .subscribe(
                  models
  ...
  @Override
  public void onBranchChanged(final @NonNull String branch) {
  ...
  @Override
  public void onItemClick(final int position, final View v, final Commit item) {
  ...
  private void getCommitCount(final @NonNull String branch) {
    manageDisposable(
        RxHelper
            .safeObservable(RxHelper.getObservable(
                RestProvider.getRepoService(isEnterprise())
