...
@Shortcut(id = "profile", icon = R.drawable.ic_app_shortcut_profile,
          shortLabelRes = R.string.profile, backStack = {MainActivity.class},
          rank = 4)
public class UserPagerActivity
    extends BaseActivity<UserPagerMvp.View, UserPagerPresenter>
    implements UserPagerMvp.View {
  ...
  public static void startActivity(final @NonNull Context context,
    ...
    context.startActivity(
        createIntent(context, login, isOrg, isEnterprise, index));
  }
  ...
  @Override
  public void onInitOrg(final boolean isMember) {
    hideProgress();
    FragmentsPagerAdapter adapter = new FragmentsPagerAdapter(
        getSupportFragmentManager(),
  ...
  public void onRepoFilterClicked() {
    if (isOrg) {
      ...
      OrgReposFragment fragment =
          ((OrgReposFragment)pager.getAdapter().instantiateItem(pager,
                                                                position));
      fragment.onRepoFilterClicked();
    } else {
      ProfileReposFragment fragment =
          ((ProfileReposFragment)pager.getAdapter().instantiateItem(pager, 2));
      fragment.onRepoFilterClicked();
    }
  }
  ...
  public boolean onPrepareOptionsMenu(final Menu menu) {
    Logger.e(getPresenter().isUserBlockedRequested(),
             getPresenter().isUserBlocked());
      ...
      if (login != null && !isOrg) {
        ...
        if (!username.equalsIgnoreCase(this.login)) {
          menu.findItem(R.id.block)
              .setIcon(getPresenter().isUserBlocked() ? R.drawable.ic_unlock
                                                      : R.drawable.ic_lock)
              .setTitle(getPresenter().isUserBlocked()
