...
class GistCommentsPresenter extends BasePresenter<GistCommentsMvp.View>
    implements GistCommentsMvp.Presenter {
  ...
  public boolean onCallApi(final int page, final @Nullable String parameter) {
    ...
    makeRestCall(
        RestProvider.getGistService(isEnterprise())
            .getGistComments(parameter, page),
        listResponse -> {
          lastPage = listResponse.getLast();
          if (getCurrentPage() == 1) {
            manageDisposable(
                Comment.saveForGist(listResponse.getItems(), parameter));
          }
          sendToView(
              view -> view.onNotifyAdapter(listResponse.getItems(), page));
        });
    return true;
  }
  ...
  public void onHandleDeletion(final @Nullable Bundle bundle) {
    if (bundle != null) {
      ...
      if (commId != 0 && gistId != null) {
        makeRestCall(RestProvider.getGistService(isEnterprise())
  ...
  @Override
  public void onWorkOffline(final @NonNull String gistId) {
    if (comments.isEmpty()) {
      manageDisposable(
          RxHelper.getObservable(Comment.getGistComments(gistId).toObservable())
              .subscribe(localComments
                         -> sendToView(
                             view -> view.onNotifyAdapter(localComments, 1))));
    } else {
  ...
  public void onHandleComment(final @NonNull String text,
    ...
    manageDisposable(
        RxHelper
            .getObservable(RestProvider.getGistService(isEnterprise())
                               .createGistComment(gistId, model))
            .doOnSubscribe(
                disposable -> sendToView(view -> view.showBlockingProgress(0)))
            .subscribe(comment
                       -> sendToView(view -> view.onAddNewComment(comment)),
                       throwable -> {
                         onError(throwable);
  ...
  public void onItemClick(final int position, final View v,
    ...
    if (v.getId() == R.id.toggle || v.getId() == R.id.toggleHolder) {
      ...
      popupMenu.getMenu()
          ...
          .setVisible(username.equalsIgnoreCase(item.getUser().getLogin()));
      popupMenu.getMenu().findItem(R.id.edit).setVisible(
          username.equalsIgnoreCase(item.getUser().getLogin()));
  ...
  public void onItemLongClick(final int position, final View v,
    ...
    if (v.getId() == R.id.toggle) {
      ...
      if (item.getUser() != null &&
          TextUtils.equals(item.getUser().getLogin(),
