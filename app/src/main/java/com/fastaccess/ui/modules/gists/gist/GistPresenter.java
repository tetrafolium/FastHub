...
class GistPresenter
    extends BasePresenter<GistMvp.View> implements GistMvp.Presenter {
  ...
  @Override
  public void checkStarring(final @NonNull String gistId) {
    makeRestCall(
        RestProvider.getGistService(isEnterprise()).checkGistStar(gistId),
        booleanResponse -> {
  ...
  public void onWorkOffline(final @NonNull String gistId) {
    if (gist == null) {
      manageDisposable(
          RxHelper.getObservable(Gist.getGist(gistId)).subscribe(gistsModel -> {
  ...
  public void callApi() {
    if (!InputHelper.isEmpty(gistId)) {
      ...
      makeRestCall(RestProvider.getGistService(isEnterprise()).getGist(gistId),
                   gistsModel -> {
                     this.gist = gistsModel;
                     sendToView(GistMvp.View::onSetupDetails);
                   });
