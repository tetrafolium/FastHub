...
public class CreateMilestonePresenter
    extends BasePresenter<CreateMilestoneMvp.View>
    implements CreateMilestoneMvp.Presenter {
  @Override
  public void
  onSubmit(final @Nullable String title, final @Nullable String dueOn,
           final @Nullable String description, final @NonNull String login,
    ...
    if (getView() != null) {
      ...
      if (!isEmptyTitle) {
        ...
        if (!InputHelper.isEmpty(dueOn)) {
          ...
          if (date != null)
            createMilestoneModel.setDueOn(ParseDateFormat.toGithubDate(date));
        }
        ...
        makeRestCall(
            RestProvider.getRepoService(isEnterprise())
                .createMilestone(login, repo, createMilestoneModel),
            milestoneModel -> {
