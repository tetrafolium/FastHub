...
class RepoPagerPresenter
    extends BasePresenter<RepoPagerMvp.View> implements RepoPagerMvp.Presenter {
  ...
  private void callApi(final int navTyp) {
    if (InputHelper.isEmpty(login) || InputHelper.isEmpty(repoId))
      return;
    makeRestCall(Observable.zip(RestProvider.getRepoService(isEnterprise())
                                    .getRepo(login(), repoId()),
                                RestProvider.getRepoService(isEnterprise())
                                    .isCollaborator(login, repoId,
                                                    Login.getUser().getLogin()),
                                (repo1, booleanResponse) -> {
                                  isCollaborator =
                                      booleanResponse.code() == 204;
                                  return repo1;
                                }),
                 repoModel -> {
                   this.repo = repoModel;
  ...
  @Override
  public void onUpdatePinnedEntry(final @NonNull String repoId,
                                  final @NonNull String login) {
    manageDisposable(PinnedRepos.updateEntry(login + "/" + repoId));
  }

  @Override
  public void onActivityCreate(final @NonNull String repoId,
                               final @NonNull String login, final int navTyp) {
    this.login = login;
    this.repoId = repoId;
    this.navTyp = navTyp;
  ...
  public boolean isRepoOwner() {
    if (getRepo() != null && getRepo().getOwner() != null) {
      return getRepo().getOwner().getLogin().equals(
                 Login.getUser().getLogin()) ||
          isCollaborator;
    }
    return false;
  ...
  public void onCheckWatching() {
    if (getRepo() != null) {
      ...
      manageDisposable(
          RxHelper
              .getObservable(RestProvider.getRepoService(isEnterprise())
                                 .isWatchingRepo(login, name))
              .doOnSubscribe(
                  disposable
              ...
              .doOnNext(subscriptionModel
                        -> sendToView(
                            view
                            -> view.onRepoWatched(
                                isWatched = subscriptionModel.isSubscribed())))
  ...
  public void onWorkOffline() {
    if (!InputHelper.isEmpty(login()) && !InputHelper.isEmpty(repoId())) {
      manageDisposable(
          RxHelper.getObservable(Repo.getRepo(repoId, login).toObservable())
              .subscribe(repoModel -> {
                repo = repoModel;
                if (repo != null) {
                  sendToView(view -> {
  ...
  @Override
  public void onModuleChanged(final @NonNull FragmentManager fragmentManager,
                              final @RepoPagerMvp.RepoNavigationType int type) {
    ...
    RepoCodePagerFragment codePagerView =
        (RepoCodePagerFragment)AppHelper.getFragmentByTag(
            fragmentManager, RepoCodePagerFragment.TAG);
    RepoIssuesPagerFragment repoIssuesPagerView =
        (RepoIssuesPagerFragment)AppHelper.getFragmentByTag(
            fragmentManager, RepoIssuesPagerFragment.TAG);
    RepoPullRequestPagerFragment pullRequestPagerView =
        (RepoPullRequestPagerFragment)AppHelper.getFragmentByTag(
            fragmentManager, RepoPullRequestPagerFragment.TAG);
    RepoProjectsFragmentPager projectsFragmentPager =
        (RepoProjectsFragmentPager)AppHelper.getFragmentByTag(
            fragmentManager, RepoProjectsFragmentPager.Companion.getTAG());
    if (getRepo() == null) {
      sendToView(RepoPagerMvp.View::onFinishActivity);
      return;
    }
    ...
    case RepoPagerMvp.CODE:
      if (codePagerView == null) {
        onAddAndHide(fragmentManager,
                     RepoCodePagerFragment.newInstance(
                         repoId(), login(), getRepo().getHtmlUrl(),
                         getRepo().getUrl(), getRepo().getDefaultBranch()),
                     currentVisible);
      } else {
    ...
    case RepoPagerMvp.ISSUES:
      ...
      if (repoIssuesPagerView == null) {
        ...
        onShowHideFragment(fragmentManager, repoIssuesPagerView,
                           currentVisible);
      }
      break;
    case RepoPagerMvp.PULL_REQUEST:
      if (pullRequestPagerView == null) {
        ...
        onShowHideFragment(fragmentManager, pullRequestPagerView,
                           currentVisible);
      }
      break;
    case RepoPagerMvp.PROJECTS:
      if (projectsFragmentPager == null) {
        onAddAndHide(
            fragmentManager,
            RepoProjectsFragmentPager.Companion.newInstance(login(), repoId()),
            currentVisible);
      } else {
        onShowHideFragment(fragmentManager, projectsFragmentPager,
                           currentVisible);
      }
      break;
    }
  ...
  @Override
  public void onShowHideFragment(final @NonNull FragmentManager fragmentManager,
                                 final @NonNull Fragment toShow,
  ...
  @Override
  public void onAddAndHide(final @NonNull FragmentManager fragmentManager,
                           final @NonNull Fragment toAdd,
                           final @NonNull Fragment toHide) {
  ...
  public void onDeleteRepo() {
    if (isRepoOwner()) {
      makeRestCall(
          RestProvider.getRepoService(isEnterprise()).deleteRepo(login, repoId),
          booleanResponse -> {
  ...
  @Override
  public void updatePinned(final int forks, final int stars,
                           final int watching) {
  ...
  @Override
  public void onMenuItemSelect(final @IdRes int id, final int position,
  ...
  @Override
  public void onMenuItemReselect(final @IdRes int id, final int position,
                                 final boolean fromUser) {}
