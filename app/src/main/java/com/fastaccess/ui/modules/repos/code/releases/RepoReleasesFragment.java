...
public class RepoReleasesFragment
    extends BaseFragment<RepoReleasesMvp.View, RepoReleasesPresenter>
    implements RepoReleasesMvp.View {
  ...
  public static RepoReleasesFragment newInstance(final @NonNull String repoId,
  ...
  @Override
  public void onNotifyAdapter(final @Nullable List<Release> items,
                              final int page) {
    hideProgress();
  ...
  @Override
  public void onDownload(final @NonNull Release item) {
    ...
    if (item.getAssets() != null && !item.getAssets().isEmpty()) {
      ArrayList<SimpleUrlsModel> mapped =
          Stream.of(item.getAssets())
              .filter(value
                      -> value != null && value.getBrowserDownloadUrl() != null)
              .map(assetsModel
                   -> new SimpleUrlsModel(
                       String.format("%s (%s)", assetsModel.getName(),
                                     assetsModel.getDownloadCount()),
  ...
  @Override
  public void onShowDetails(final @NonNull Release item) {
    if (!InputHelper.isEmpty(item.getBody())) {
      MessageDialogView
          .newInstance(!InputHelper.isEmpty(item.getName()) ? item.getName()
                                                            : item.getTagName(),
                       item.getBody(), true, false)
  ...
  @Override
  public void onItemSelected(final SimpleUrlsModel item) {
