...
class PullRequestPagerPresenter extends BasePresenter<PullRequestPagerMvp.View>
    implements PullRequestPagerMvp.Presenter {
  ...
  @Override
  public void onActivityCreated(final @Nullable Intent intent) {
    if (intent != null && intent.getExtras() != null) {
      ...
      if (pullRequest != null) {
        ...
        return;
      } else if (issueNumber > 0 && !InputHelper.isEmpty(login) &&
                 !InputHelper.isEmpty(repoId)) {
        callApi();
        return;
      }
    }
  ...
  public boolean isOwner() {
    if (getPullRequest() == null)
      return false;
    User userModel =
        getPullRequest() != null ? getPullRequest().getUser() : null;
    Login me = Login.getUser();
    PullsIssuesParser parser =
        PullsIssuesParser.getForIssue(getPullRequest().getHtmlUrl());
    return (userModel != null &&
            userModel.getLogin().equalsIgnoreCase(me.getLogin())) ||
  ...
  public boolean isMergeable() {
    return getPullRequest() != null && getPullRequest().isMergeable() &&
        !getPullRequest().isMerged();
  }
  ...
  @Override
  public void onHandleConfirmDialog(final @Nullable Bundle bundle) {
    if (bundle != null) {
  ...
  @Override
  public void onLockUnlockConversations(final String reason) {
    ...
    Observable<Response<Boolean>> observable = RxHelper.getObservable(
        model == null ? service.unlockIssue(login, repoId, issueNumber)
  ...
  public void onOpenCloseIssue() {
    if (getPullRequest() != null) {
      IssueRequestModel requestModel =
          IssueRequestModel.clone(getPullRequest(), true);
      manageDisposable(
          RxHelper
              .getObservable(RestProvider.getPullRequestService(isEnterprise())
                                 .editPullRequest(login, repoId, issueNumber,
                                                  requestModel))
              .doOnSubscribe(
                  disposable -> sendToView(view -> view.showProgress(0)))
              .subscribe(
                  issue
                  -> {
                    if (issue != null) {
                      sendToView(
                          ...
                          -> view.showSuccessIssueActionMsg(
                              getPullRequest().getState() == IssueState.open));
  ...
  @NonNull
  @Override
  public SpannableBuilder getMergeBy(final @NonNull PullRequest pullRequest,
                                     final @NonNull Context context) {
    return PullRequest.getMergeBy(pullRequest, context, false);
  }

  @Override
  public void onPutLabels(final @NonNull ArrayList<LabelModel> labels) {
    makeRestCall(
        RestProvider.getIssueService(isEnterprise())
            .putLabels(
                login, repoId, issueNumber,
                Stream.of(labels)
                    .filter(value -> value != null && value.getName() != null)
                    .map(LabelModel::getName)
                    .collect(Collectors.toList())),
        labelModels -> {
          sendToView(
              view -> updateTimeline(view, R.string.labels_added_successfully));
  ...
  @Override
  public void onPutMilestones(final @NonNull MilestoneModel milestone) {
    pullRequest.setMilestone(milestone);
    IssueRequestModel issueRequestModel =
        IssueRequestModel.clone(pullRequest, false);
    makeRestCall(
        RestProvider.getPullRequestService(isEnterprise())
            .editIssue(login, repoId, issueNumber, issueRequestModel),
        pr -> {
  ...
  @Override
  public void onPutAssignees(final @NonNull ArrayList<User> users,
    ...
    if (isAssignees) {
      assigneesRequestModel.setAssignees(
          assignees.isEmpty() ? Stream.of(pullRequest.getAssignees())
                                    .map(User::getLogin)
                                    .toList()
                              : assignees);
      makeRestCall(
          !assignees.isEmpty()
              ? RestProvider.getIssueService(isEnterprise())
                    .putAssignees(login, repoId, issueNumber,
                                  assigneesRequestModel)
              : RestProvider.getIssueService(isEnterprise())
                    .deleteAssignees(login, repoId, issueNumber,
                                     assigneesRequestModel),
          pullRequestResponse -> {
      ...
      makeRestCall(
          RestProvider.getPullRequestService(isEnterprise())
              .putReviewers(login, repoId, issueNumber, assigneesRequestModel),
          pullRequestResponse
          -> sendToView(view -> updateTimeline(view, R.string.reviewer_added)));
    }
  }
  ...
  public void onMerge(final @NonNull String msg,
    ...
    if (isMergeable() &&
        (isCollaborator() || isRepoOwner())) { // double the checking
      if (getPullRequest() == null || getPullRequest().getHead() == null ||
          getPullRequest().getHead().getSha() == null)
        return;
      ...
      manageDisposable(
          RxHelper
              .getObservable(RestProvider.getPullRequestService(isEnterprise())
                                 .mergePullRequest(login, repoId, issueNumber,
                                                   mergeRequestModel))
              .doOnSubscribe(
                  disposable -> sendToView(view -> view.showProgress(0)))
              .subscribe(
                  ...
                  -> {
                    if (mergeResponseModel.isMerged()) {
                      ...
                      sendToView(
                          view -> updateTimeline(view, R.string.success_merge));
                    } else {
                      sendToView(view
                                 -> view.showErrorMessage(
                                     mergeResponseModel.getMessage()));
                    }
                  },
                  throwable
                  -> sendToView(
                      view -> view.showErrorMessage(throwable.getMessage()))));
    }
  }
  ...
  @Override
  public void onUpdatePullRequest(final @NonNull PullRequest pullRequestModel) {
  ...
  @Override
  public void onAddComment(final @NonNull CommentRequestModel comment) {
  ...
  @Override
  public void onSubscribeOrMute(final boolean mute) {
    if (getPullRequest() == null)
      return;
    makeRestCall(
        mute
            ? RestProvider.getNotificationService(isEnterprise())
                  .subscribe(getPullRequest().getId(),
  ...
  private void callApi() {
    ...
    if (loggedInUser == null)
      return;
    makeRestCall(
        RxHelper.getObservable(Observable.zip(
            RestProvider.getPullRequestService(isEnterprise())
                .getPullRequest(login, repoId, issueNumber),
            RestProvider.getRepoService(isEnterprise())
                .isCollaborator(login, repoId, loggedInUser.getLogin()),
            RestProvider.getIssueService(isEnterprise())
                .getIssue(login, repoId, issueNumber),
  ...
  private void updateTimeline(final PullRequestPagerMvp.View view,
                              final int assignee_added) {
