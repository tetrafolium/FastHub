...
@Getter
public class IssueTimelinePresenter extends BasePresenter<IssueTimelineMvp.View>
    implements IssueTimelineMvp.Presenter {
    ...
    if (getView() != null) {
      ...
      if (item.getType() == TimelineModel.COMMENT) {
        if (v.getId() == R.id.commentMenu) {
          ...
          boolean isOwner =
              CommentsHelper.isOwner(username, issue.getLogin(),
        ...
        if (issueEventModel.getCommitUrl() != null) {
          ...
          FilterIssuesActivity.startActivity(
              v, issue.getLogin(), issue.getRepoId(), true, true,
              isEnterprise(),
        ...
        if (v.getId() == R.id.commentMenu) {
          ...
          String username = Login.getUser().getLogin();
          boolean isOwner =
              CommentsHelper.isOwner(username, item.getIssue().getLogin(),
                                     item.getIssue().getUser().getLogin()) ||
              isCollaborator;
          popupMenu.getMenu().findItem(R.id.edit).setVisible(isOwner);
          popupMenu.setOnMenuItemClickListener(item1 -> {
            if (getView() == null)
            ...
            if (item1.getItemId() == R.id.reply) {
              getView().onReply(item.getIssue().getUser(),
                                item.getIssue().getBody());
            } else if (item1.getItemId() == R.id.edit) {
              Activity activity = ActivityHelper.getActivity(v.getContext());
              if (activity == null)
                return false;
              CreateIssueActivity.startForResult(
                  activity, item.getIssue().getLogin(),
                  item.getIssue().getRepoId(), item.getIssue(), isEnterprise());
            } else if (item1.getItemId() == R.id.share) {
              ActivityHelper.shareUrl(v.getContext(),
                                      item.getIssue().getHtmlUrl());
            }
            return true;
          });
          popupMenu.show();
        } else {
          onHandleReaction(v.getId(), item.getIssue().getNumber(),
          ...
          if (type != null) {
            if (item.getType() == TimelineModel.HEADER) {
              getView().showReactionsPopup(type, login, repoId,
                                           item.getIssue().getNumber(), true);
            } else {
              getView().showReactionsPopup(type, login, repoId,
                                           item.getComment().getId(), false);
            }
          } else {
    ...
    if (bundle != null) {
      ...
      if (commId != 0) {
        ...
        makeRestCall(RestProvider.getIssueService(isEnterprise())
                         .deleteIssueComment(issue.getLogin(),
                                             issue.getRepoId(), commId),
  ...
  @Override
  public void
  onHandleReaction(final int viewId, final long id,
    ...
    Observable observable = getReactionsProvider().onHandleReaction(
        viewId, id, login, repoId, reactionType, isEnterprise());
    if (observable != null)
  ...
  @Override
  public boolean onCallApi(final int page, final @Nullable Issue parameter) {
    if (parameter == null) {
    ...
    Observable<List<TimelineModel>> observable =
        RestProvider.getIssueService(isEnterprise())
            ...
            .flatMap(response -> {
              ...
              return TimelineConverter.INSTANCE.convert(
  ...
  private void loadComment(final int page, final long commentId,
                           final String login, final String repoId,
    ...
    if (page == 1 && commentId > 0) {
      ...
      manageObservable(observable.doOnNext(
          timelineModel -> sendToView(view -> {
