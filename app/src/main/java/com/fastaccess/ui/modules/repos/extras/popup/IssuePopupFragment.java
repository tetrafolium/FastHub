...
public class IssuePopupFragment
    extends BaseMvpBottomSheetDialogFragment<IssuePopupMvp.View,
                                             IssuePopupPresenter>
  ...
  public static void showPopup(@NonNull FragmentManager manager,
                               @NonNull Issue issue) {
    IssuePopupFragment fragment = new IssuePopupFragment();
    PullsIssuesParser parser =
        PullsIssuesParser.getForIssue(issue.getHtmlUrl());
    if (parser == null) {
    ...
    fragment.setArguments(getBundle(
        parser.getLogin(), parser.getRepoId(), issue.getNumber(),
        issue.getTitle(), issue.getBody(), issue.getUser(), issue.getAssignee(),
        issue.getLabels(), issue.getMilestone(), !issue.isLocked()));
  ...
  public static void showPopup(@NonNull FragmentManager manager,
    ...
    if (parser == null)
      return;
    fragment.setArguments(getBundle(
        parser.getLogin(), parser.getRepoId(), pullRequest.getNumber(),
        pullRequest.getTitle(), pullRequest.getBody(), pullRequest.getUser(),
        pullRequest.getAssignee(), pullRequest.getLabels(),
        pullRequest.getMilestone(), !pullRequest.isLocked()));
    fragment.show(manager, "");
  }

  @NonNull
  private static Bundle
  getBundle(@NonNull String login, @NonNull String repoId, int number,
            @NonNull String title, @NonNull String body, @NonNull User user,
            @Nullable User assignee, @Nullable LabelListModel labels,
            @Nullable MilestoneModel milestone, boolean canComment) {
  ...
  void onSubmit() {
    ...
    if (!isEmpty) {
      ...
      getPresenter().onSubmit(
          getArguments().getString(BundleConstant.EXTRA_SEVEN),
          getArguments().getString(BundleConstant.EXTRA_EIGHT),
          getArguments().getInt(BundleConstant.ID),
          InputHelper.toString(comment));
    } else {
  ...
  public void onViewCreated(@NonNull View view,
    ...
    if (user != null) {
      ...
      avatarLayout.setUrl(user.getAvatarUrl(), user.getLogin(), false,
