...
class CommitCommentsPresenter extends BasePresenter<CommitCommentsMvp.View>
    implements CommitCommentsMvp.Presenter {
  ...
  public boolean onCallApi(final int page, final @Nullable String parameter) {
    ...
    makeRestCall(
        RestProvider.getRepoService(isEnterprise())
            .getCommitComments(login, repoId, sha, page)
        ...
        listResponse
        -> sendToView(view -> view.onNotifyAdapter(listResponse, page)));
    return true;
  }
  ...
  public void onWorkOffline() {
    if (comments.isEmpty()) {
      manageDisposable(
          RxHelper
              .getObservable(Comment.getCommitComments(repoId(), login(), sha)
                                 .toObservable())
              .flatMap(TimelineModel::construct)
              .subscribe(
                  models
                  -> sendToView(view -> view.onNotifyAdapter(models, 1))));
    } else {
      sendToView(CommitCommentsMvp.View::hideProgress);
    }
  }
    ...
    manageDisposable(
        RxHelper
            .getObservable(RestProvider.getRepoService(isEnterprise())
                               .postCommitComment(login, repoId, sha, model))
            .doOnSubscribe(
                disposable -> sendToView(view -> view.showBlockingProgress(0)))
            .subscribe(comment
                       -> sendToView(view -> view.addComment(comment)),
                       throwable -> {
                         onError(throwable);
                         sendToView(
                             CommitCommentsMvp.View::hideBlockingProgress);
                       }));
  }
  ...
  public void onItemClick(final int position, final View v,
    ...
    if (getView() != null) {
      ...
      if (v.getId() == R.id.commentMenu) {
        ...
        boolean isOwner = CommentsHelper.isOwner(username, login,
  ...
  public void onItemLongClick(final int position, final View v,
    ...
    if (v.getId() == R.id.commentMenu) {
      ...
      if (reactionTypes != null) {
        if (getView() != null)
          getView().showReactionsPopup(reactionTypes, login, repoId,
                                       timelineModel.getComment().getId());
      } else {
  ...
  private void onHandleReaction(final int viewId, final long id) {
    Observable observable = getReactionsProvider().onHandleReaction(
        viewId, id, login, repoId, ReactionsProvider.COMMIT, isEnterprise());
    if (observable != null)
