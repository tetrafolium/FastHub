...
class ProfileOverviewPresenter extends BasePresenter<ProfileOverviewMvp.View>
    implements ProfileOverviewMvp.Presenter {
  ...
  @Override
  public void onCheckFollowStatus(final @NonNull String login) {
    if (!TextUtils.equals(login, Login.getUser().getLogin())) {
      manageDisposable(
          RxHelper
              .getObservable(RestProvider.getUserService(isEnterprise())
  ...
  @Override
  public void onFollowButtonClicked(final @NonNull String login) {
    manageDisposable(
        RxHelper
            .getObservable(!isFollowing
                   ...
                   userModel -> {
                     ...
                     if (userModel != null) {
                       ...
                       if (userModel.getType() != null &&
                           userModel.getType().equalsIgnoreCase("user")) {
  ...
  private void loadPinnedRepos(final @NonNull String login) {
    ApolloCall<GetPinnedReposQuery.Data> apolloCall =
        ApolloProdivder.INSTANCE.getApollo(isEnterprise())
            .query(GetPinnedReposQuery.builder().login(login).build());
    manageDisposable(
        RxHelper.getObservable(Rx2Apollo.from(apolloCall))
            .filter(dataResponse -> !dataResponse.hasErrors())
            .flatMap(dataResponse -> {
              if (dataResponse.data() != null &&
                  dataResponse.data().user() != null) {
                return Observable.fromIterable(
                    dataResponse.data().user().pinnedRepositories().edges());
              }
  ...
  @Override
  public void onSendUserToView(final @Nullable User userModel) {
    sendToView(view -> view.onInitViews(userModel));
  }

  @Override
  public void onLoadContributionWidget(
      final @NonNull GitHubContributionsView gitHubContributionsView) {
    if (!isEnterprise()) {
      if (contributions == null || contributions.isEmpty()) {
        ...
        manageDisposable(
            RxHelper
                ...
                .flatMap(s
                         -> Observable.just(
                             new ContributionsProvider().getContributions(s)))
                .subscribe(lists -> {
                  contributions.clear();
  ...
  @NonNull
  @Override
  public ArrayList<ContributionsDay> getContributions() {
  ...
  private void
  loadContributions(final ArrayList<ContributionsDay> contributions,
                    final GitHubContributionsView gitHubContributionsView) {
    List<ContributionsDay> filter =
        gitHubContributionsView.getLastContributions(contributions);
    if (filter != null && contributions != null) {
      Observable<Bitmap> bitmapObservable = Observable.just(
          gitHubContributionsView.drawOnCanvas(filter, contributions));
      manageObservable(bitmapObservable.doOnNext(
          bitmap
          -> sendToView(view -> view.onInitContributions(bitmap != null))));
    }
  }

  private void loadOrgs() {
    boolean isMe = login.equalsIgnoreCase(
        Login.getUser() != null ? Login.getUser().getLogin() : "");
    manageDisposable(
        RxHelper
            .getObservable(isMe ? RestProvider.getOrgService(isEnterprise())
                                      .getMyOrganizations()
                                : RestProvider.getOrgService(isEnterprise())
                                      .getMyOrganizations(login))
            .subscribe(response -> {
