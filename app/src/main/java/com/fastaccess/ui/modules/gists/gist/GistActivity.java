...
public class GistActivity
    extends BaseActivity<GistMvp.View, GistPresenter> implements GistMvp.View {
  ...
  void onTitleClick() {
    if (getPresenter().getGist() != null &&
        !InputHelper.isEmpty(getPresenter().getGist().getDescription()))
      MessageDialogView
          .newInstance(getString(R.string.details),
                       getPresenter().getGist().getDescription(), false, true)
          .show(getSupportFragmentManager(), MessageDialogView.TAG);
  }

  @OnClick({R.id.startGist, R.id.forkGist, R.id.browser})
  public void onGistActions(final View view) {
    if (getPresenter().getGist() == null)
      return;
    ...
    switch (view.getId()) {
    case R.id.startGist:
      GithubActionService.startForGist(
          this, getPresenter().getGist().getGistId(),
          getPresenter().isStarred() ? GithubActionService.UNSTAR_GIST
                                     : GithubActionService.STAR_GIST,
          isEnterprise());
    ...
    case R.id.browser:
      ActivityHelper.startCustomTab(this,
                                    getPresenter().getGist().getHtmlUrl());
      break;
    }
  }
  ...
  void onEdit() {
    if (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked()) {
      if (getPresenter().getGist() != null)
        CreateGistActivity.start(this, getPresenter().getGist());
    } else {
  ...
  public boolean onOptionsItemSelected(final MenuItem item) {
    if (item.getItemId() == R.id.share) {
      if (getPresenter().getGist() != null)
        ActivityHelper.shareUrl(this, getPresenter().getGist().getHtmlUrl());
      return true;
    } else if (item.getItemId() == R.id.deleteGist) {
      MessageDialogView
          .newInstance(getString(R.string.delete_gist),
  ...
  public void onSuccessDeleted() {
    ...
    if (getPresenter().getGist() != null) {
      ...
      intent.putExtras(
          Bundler.start().put(BundleConstant.ITEM, gistsModel).end());
  ...
  @Override
  public void onGistStarred(final boolean isStarred) {
    startGist.setImageResource(isStarred ? R.drawable.ic_star_filled
                                         : R.drawable.ic_star);
  ...
  @Override
  public void onGistForked(final boolean isForked) {
  ...
  public void onSetupDetails() {
    ...
    String url = gistsModel.getOwner() != null
                     ? gistsModel.getOwner().getAvatarUrl()
                     : gistsModel.getUser() != null
                           ? gistsModel.getUser().getAvatarUrl()
                           : "";
    String login = gistsModel.getOwner() != null
                       ? gistsModel.getOwner().getLogin()
                       : gistsModel.getUser() != null
                             ? gistsModel.getUser().getLogin()
                             : "";
    avatarLayout.setUrl(url, login, false,
                        LinkParserHelper.isEnterprise(gistsModel.getHtmlUrl()));
    title.setText(gistsModel.getDisplayTitle(false, true));
    setTaskName(gistsModel.getDisplayTitle(false, true).toString());
    edit.setVisibility(Login.getUser().getLogin().equals(login) ? View.VISIBLE
                                                                : View.GONE);
    detailsIcon.setVisibility(
        InputHelper.isEmpty(gistsModel.getDescription()) ||
                !ViewHelper.isEllipsed(title)
            ? View.GONE
            : View.VISIBLE);
    if (gistsModel.getCreatedAt().before(gistsModel.getUpdatedAt())) {
      date.setText(String.format(
          "%s %s", ParseDateFormat.getTimeAgo(gistsModel.getCreatedAt()),
    ...
    pager.setAdapter(new FragmentsPagerAdapter(
        ...
        FragmentPagerAdapterModel.buildForGist(this, gistsModel)));
  ...
  @Override
  public void onUpdatePinIcon(final @NonNull Gist gist) {
    pinUnpin.setImageDrawable(
        PinnedGists.isPinned(gist.getGistId().hashCode())
