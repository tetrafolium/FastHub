...
public class AllNotificationsPresenter
    extends BasePresenter<AllNotificationsMvp.View>
    implements AllNotificationsMvp.Presenter {
  private final ArrayList<GroupedNotificationModel> notifications =
      new ArrayList<>();

  @Override
  public void onItemClick(final int position, final View v,
                          final GroupedNotificationModel model) {
    if (getView() == null)
      return;
    if (model.getType() == GroupedNotificationModel.ROW) {
      ...
      if (v.getId() == R.id.markAsRead) {
        ...
        if (item.getSubject() != null && item.getSubject().getUrl() != null) {
          ...
          if (getView() != null)
            getView().onClick(item.getSubject().getUrl());
        }
      }
    } else {
      ...
      if (v.getId() == R.id.markAsRead) {
        ...
        RepoPagerActivity.startRepoPager(v.getContext(),
                                         new NameParser(repo.getUrl()));
      }
    }
  }

  private void markAsRead(final int position, final View v,
                          final Notification item) {
    item.setUnread(false);
    manageDisposable(item.save(item));
    sendToView(view
               -> view.onUpdateReadState(new GroupedNotificationModel(item),
                                         position));
  ...
  public void onItemLongClick(final int position, final View v,
                              final GroupedNotificationModel item) {}
  ...
  public void onWorkOffline() {
    if (notifications.isEmpty()) {
      manageDisposable(
          RxHelper
              .getObservable(Notification.getAllNotifications().toObservable())
              .flatMap(notifications
                       -> Observable.just(
                           GroupedNotificationModel.construct(notifications)))
              .subscribe(
                  models -> sendToView(view -> view.onNotifyAdapter(models))));
    } else {
  ...
  public void onCallApi() {
    ...
    //        RestProvider.getNotificationService().getAllNotifications()
    //                .flatMap(response -> response.getItems() != null ?
    //                Observable.fromIterable(response.getItems()) :
    //                Observable.empty()) .filter(ObjectsCompat::nonNull)
    //                .flatMap(notification ->
    //                RestProvider.getNotificationService().isSubscribed(notification.getId())
    ...
    //                        })
    //                .toList();
    Observable<List<GroupedNotificationModel>> observable =
            ...
            .flatMap(response -> {
              ...
              if (response.getItems() != null &&
                ...
                return Observable.just(
                    GroupedNotificationModel.construct(response.getItems()));
              }
              return Observable.empty();
            });
    makeRestCall(
        observable.doOnComplete(() -> sendToView(BaseMvp.FAView::hideProgress)),
        response -> sendToView(view -> view.onNotifyAdapter(response)));
  }

  @Override
  public void onMarkAllAsRead(final
                              @NonNull List<GroupedNotificationModel> data) {
    manageDisposable(
        RxHelper.getObservable(Observable.fromIterable(data))
            .filter(group -> group.getType() == GroupedNotificationModel.ROW)
            .filter(group
                    -> group.getNotification() != null &&
                           group.getNotification().isUnread())
  ...
  @Override
  public void onMarkReadByRepo(final
                               @NonNull List<GroupedNotificationModel> data,
        ...
        RxHelper.getObservable(Observable.fromIterable(data))
            ...
            .filter(group
                    -> group.getNotification()
                           .getRepository()
                           .getFullName()
                           .equalsIgnoreCase(repo.getFullName()))
            .map(GroupedNotificationModel::getNotification)
            .subscribe(notification -> {
              notification.setUnread(false);
              manageDisposable(notification.save(notification));
