...
public class GistFilesListFragment
    extends BaseFragment<GistFilesListMvp.View, GistFilesListPresenter>
    implements GistFilesListMvp.View {
  ...
  public static GistFilesListFragment
  newInstance(@NonNull ArrayList<FilesListModel> files, boolean isOwner) {
  ...
  protected void onFragmentCreated(@NonNull View view,
    ...
    adapter = new GistFilesAdapter(getPresenter().getFiles(), getPresenter(),
                                   isOwner);
    recycler.setAdapter(adapter);
    if (getArguments() != null && savedInstanceState == null) {
      ArrayList<FilesListModel> filesListModel =
          getArguments().getParcelableArrayList(BundleConstant.ITEM);
  ...
  @Override
  public void onOpenFile(@NonNull FilesListModel item, int position) {
  ...
  @Override
  public void onDeleteFile(@NonNull FilesListModel item, int position) {
    MessageDialogView
  ...
  @Override
  public void onEditFile(@NonNull FilesListModel item, int position) {
    AddGistBottomSheetDialog.Companion.newInstance(item, position)
        .show(getChildFragmentManager(),
              AddGistBottomSheetDialog.Companion.getTAG());
  }

  @Override
  public void onInitFiles(@Nullable ArrayList<FilesListModel> filesListModel,
                          boolean isOwner) {
  ...
  public void onAddNewFile() {
    ...
    if (adapter.getItemCount() == 0 ||
        (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked())) {
      AddGistBottomSheetDialog.Companion.newInstance(null, -1).show(
  ...
  public void onFileAdded(@NonNull FilesListModel file, Integer position) {
    if (position == null || position == -1) {
      ...
      if (getPresenter().getFilesMap().get(current.getFilename()) != null) {
        FilesListModel toUpdate =
            getPresenter().getFilesMap().get(current.getFilename());
  ...
  private boolean canOpen(@NonNull FilesListModel item) {
    if (item.getRawUrl() == null)
      return false;
    if (item.getSize() > FileHelper.ONE_MB &&
        !MarkDownProvider.isImage(item.getRawUrl())) {
      MessageDialogView
          .newInstance(getString(R.string.big_file),
                       getString(R.string.big_file_description), false, true,
                       Bundler.start()
                           .put(BundleConstant.YES_NO_EXTRA, true)
                           .put(BundleConstant.EXTRA, item.getRawUrl())
                           .end())
