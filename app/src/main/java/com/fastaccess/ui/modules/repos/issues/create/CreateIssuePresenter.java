...
public class CreateIssuePresenter extends BasePresenter<CreateIssueMvp.View>
    implements CreateIssueMvp.Presenter {

  @com.evernote.android.state.State boolean isCollaborator;

  @Override
  public void checkAuthority(final @NonNull String login,
  ...
  @Override
  public void onSubmit(final @NonNull String title,
                       final @NonNull CharSequence description,
                       final @NonNull String login, final @NonNull String repo,
                       final @Nullable Issue issue,
                       final @Nullable PullRequest pullRequestModel,
                       final @Nullable ArrayList<LabelModel> labels,
                       final @Nullable MilestoneModel milestoneModel,
      ...
      if (issue == null && pullRequestModel == null) {
        ...
        if (isCollaborator) {
          if (labels != null && !labels.isEmpty()) {
            createIssue.setLabels(
                Stream.of(labels)
                    .map(LabelModel::getName)
                    .collect(Collectors.toCollection(ArrayList::new)));
          }
          if (users != null && !users.isEmpty()) {
            createIssue.setAssignees(
                Stream.of(users)
                    .map(User::getLogin)
                    .collect(Collectors.toCollection(ArrayList::new)));
          }
        ...
        makeRestCall(
            RestProvider.getIssueService(isEnterprise())
                .createIssue(login, repo, createIssue),
            issueModel
            -> {
              if (issueModel != null) {
        ...
        if (issue != null) {
          ...
          IssueRequestModel requestModel =
              IssueRequestModel.clone(issue, false);
          makeRestCall(
              RestProvider.getIssueService(isEnterprise())
                  .editIssue(login, repo, number, requestModel),
              issueModel
              -> {
                if (issueModel != null) {
                  sendToView(view -> view.onSuccessSubmission(issueModel));
        ...
        if (pullRequestModel != null) {
          ...
          IssueRequestModel requestModel =
              IssueRequestModel.clone(pullRequestModel, false);
          makeRestCall(
              RestProvider.getPullRequestService(isEnterprise())
                  .editPullRequest(login, repo, number, requestModel)
                  .flatMap(pullRequest1
                           -> RestProvider.getIssueService(isEnterprise())
                                  .getIssue(login, repo, number),
                           (pullRequest1, issueReaction)
                               -> { // hack to get reactions from issue api
                             if (issueReaction != null) {
                               pullRequest1.setReactions(
                                   issueReaction.getReactions());
                             }
                             return pullRequest1;
                           }),
              pr
  ...
  public void onCheckAppVersion() {
    makeRestCall(
        RestProvider.getRepoService(false).getLatestRelease("k0shk0sh",
                                                            "FastHub"),
        release
        -> {
          if (release != null) {
