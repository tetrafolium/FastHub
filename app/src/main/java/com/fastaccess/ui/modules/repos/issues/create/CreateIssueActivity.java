...
public class CreateIssueActivity
    extends BaseActivity<CreateIssueMvp.View, CreateIssuePresenter>
    implements CreateIssueMvp.View {
  ...
  public static void startForResult(final @NonNull Fragment fragment,
                                    final @NonNull String login,
                                    final @NonNull String repoId,
                                    final boolean isEnterprise) {
    Intent intent =
        new Intent(fragment.getContext(), CreateIssueActivity.class);
    intent.putExtras(Bundler.start()
                         .put(BundleConstant.EXTRA, login)
                         .put(BundleConstant.ID, repoId)
                         .put(BundleConstant.EXTRA_TWO,
                              login.equalsIgnoreCase("k0shk0sh") &&
                                  repoId.equalsIgnoreCase("FastHub"))
                         .put(BundleConstant.IS_ENTERPRISE, isEnterprise)
                         .end());
    View view = fragment.getActivity() != null
                    ? fragment.getActivity().findViewById(R.id.fab)
                    : null;
    if (view != null) {
      ActivityHelper.startReveal(fragment, intent, view,
  ...
  public static void startForResult(final @NonNull Activity activity,
                                    final @NonNull String login,
                                    final @NonNull String repoId,
                                    final @Nullable Issue issueModel,
                                    final boolean isEnterprise) {
    if (issueModel != null) {
  ...
  getIntent(final @NonNull Context context, final @NonNull String login,
            final @NonNull String repoId, final boolean isFeedback) {
  ...
  @NonNull
  public static Intent startForResult(final @NonNull Activity activity) {
  ...
  @Override
  public void onSetCode(final @NonNull CharSequence charSequence) {
    this.savedText = charSequence;
    MarkDownProvider.setMdText(description, InputHelper.toString(savedText));
  }

  @Override
  public void onTitleError(final boolean isEmptyTitle) {
    title.setError(isEmptyTitle ? getString(R.string.required_field) : null);
  }

  @Override
  public void onDescriptionError(final boolean isEmptyDesc) {
    description.setError(isEmptyDesc ? getString(R.string.required_field)
                                     : null);
  }

  @Override
  public void onSuccessSubmission(final Issue issueModel) {
    hideProgress();
    Intent intent = new Intent();
    intent.putExtras(
        Bundler.start().put(BundleConstant.ITEM, issueModel).end());
  ...
  @Override
  public void onSuccessSubmission(final PullRequest issueModel) {
    hideProgress();
    Intent intent = new Intent();
    intent.putExtras(
        Bundler.start().put(BundleConstant.ITEM, issueModel).end());
  ...
  public void onShowIssueMisc() {
    ...
    issueMiscLayout.setVisibility(getPresenter().isCollaborator() ? View.VISIBLE
                                                                  : View.GONE);
    // TODO
  }
  ...
  protected void onCreate(final Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      ...
      if (bundle.getParcelable(BundleConstant.ITEM) != null) {
        ...
        } else if (bundle.getParcelable(BundleConstant.ITEM) instanceof
                   PullRequest) {
      ...
      if (pullRequest != null) {
        ...
        if (pullRequest.getAssignees() != null) {
          ...
          onSelectedAssignees(new ArrayList<>(pullRequest.getAssignees()),
                              false);
        }
    ...
    if (isFeedback || ("k0shk0sh".equalsIgnoreCase(login) &&
                       repoId.equalsIgnoreCase("FastHub"))) {
      Toasty
          .info(App.getInstance(), getString(R.string.report_issue_warning),
                Toast.LENGTH_LONG)
          .show();
    ...
    setTaskName(login + "/" + repoId + " - " +
                (isFeedback ? getString(R.string.submit_feedback)
                            : getString(R.string.create_issue)));
  }
  ...
  public void onBackPressed() {
    if (InputHelper.isEmpty(title)) {
      ...
      MessageDialogView
          .newInstance(getString(R.string.close),
                       getString(R.string.unsaved_data_warning),
                       Bundler.start()
      ...
      intent.putExtras(
          Bundler.start()
              ...
              .put(BundleConstant.EXTRA_TYPE,
                   BundleConstant.ExtraType.FOR_RESULT_EXTRA)
              .put(BundleConstant.IS_ENTERPRISE, isEnterprise())
              .end());
      ActivityHelper.startReveal(this, intent, submit,
                                 BundleConstant.REQUEST_CODE);
      return true;
    }
    return false;
  ...
  @OnClick(R.id.submit)
  public void onClick() {
    getPresenter().onSubmit(InputHelper.toString(title), savedText, login,
                            repoId, issue, pullRequest, labelModels,
                            milestoneModel, users);
  }
  ...
  @Override
  public void onSelectedLabels(final
                               @NonNull ArrayList<LabelModel> labelModels) {
    ...
    for (int i = 0; i < labelModels.size(); i++) {
      ...
      if (i > 0) {
        builder.append(" ").append(" " + labelModel.getName() + " ",
                                   new LabelSpan(color));
      } else {
  ...
  public void onMilestoneSelected(final
    ...
    Logger.e(milestoneModel.getTitle(), milestoneModel.getDescription(),
             milestoneModel.getNumber());
