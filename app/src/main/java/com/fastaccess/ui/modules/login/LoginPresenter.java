...
public class LoginPresenter
    extends BasePresenter<LoginMvp.View> implements LoginMvp.Presenter {

  LoginPresenter() { RestProvider.clearHttpClient(); }
  ...
  public void onError(final @NonNull Throwable throwable) {
    if (RestProvider.getErrorCode(throwable) == 401 && throwable instanceof
                                                           HttpException) {
      ...
      if (response != null && response.headers() != null) {
        ...
        if (twoFaToken != null) {
          ...
          sendToView(
              view -> view.showMessage(R.string.error, R.string.failed_login));
          return;
        }
      }
    }
  ...
  @Override
  public void onTokenResponse(final @Nullable AccessTokenModel modelResponse) {
    if (modelResponse != null) {
      String token = modelResponse.getToken() != null
                         ? modelResponse.getToken()
                         : modelResponse.getAccessToken();
      if (!InputHelper.isEmpty(token)) {
        PrefGetter.setToken(token);
        makeRestCall(RestProvider.getUserService(false).getUser(),
                     this::onUserResponse);
        return;
      }
    }
  ...
  public Uri getAuthorizationUrl() {
    return new Uri.Builder()
        ...
        .appendQueryParameter("redirect_uri",
                              GithubConfigHelper.getRedirectUrl())
  ...
  @Override
  public void onHandleAuthIntent(final @Nullable Intent intent) {
    if (intent != null && intent.getData() != null) {
      ...
      if (uri.toString().startsWith(GithubConfigHelper.getRedirectUrl())) {
        ...
        if (!InputHelper.isEmpty(tokenCode)) {
          makeRestCall(LoginProvider.getLoginRestService().getAccessToken(
                           tokenCode, GithubConfigHelper.getClientId(),
  ...
  @Override
  public void onUserResponse(final @Nullable Login userModel) {
    if (userModel != null) {
      manageObservable(
          Login.onMultipleLogin(userModel, isEnterprise(), true)
              .doOnComplete(()
                                -> sendToView(view
  ...
  @Override
  public void
  login(final @NonNull String username, final @NonNull String password,
        final @Nullable String twoFactorCode, final boolean isBasicAuth,
    ...
    if ((!usernameIsEmpty && !passwordIsEmpty)) {
      try {
        ...
        if (isBasicAuth && !isEnterprise()) {
          ...
          authModel.setScopes(Arrays.asList("user", "repo", "gist",
          ...
          makeRestCall(
              LoginProvider.getLoginRestService(authToken, twoFactorCode, null)
                  .login(authModel),
              accessTokenModel -> {
                if (!InputHelper.isEmpty(twoFactorCode)) {
        ...
        sendToView(
            view
            -> view.showMessage("Error", "The app was about to crash!!(" +
                                             e.getMessage() + ")"));
      }
    }
  }

  private void accessTokenLogin(final @NonNull String password,
                                final @Nullable String endpoint,
                                final @Nullable String otp,
                                final @NonNull String authToken) {
    makeRestCall(LoginProvider.getLoginRestService(authToken, otp, endpoint)
                     .loginAccessToken(),
                 login -> {
                   if (!isEnterprise()) {
