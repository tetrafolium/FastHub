...
public class RepoFilesActivity extends BaseActivity {
  ...
  public static void startActivity(@NonNull Context context,
                                   @NonNull String url, boolean isEnterprise) {
    if (!InputHelper.isEmpty(url)) {
      Intent intent =
          ActivityHelper.editBundle(getIntent(context, url), isEnterprise);
  ...
  public static Intent getIntent(@NonNull Context context,
    ...
    if (uri.getPathSegments() != null && uri.getPathSegments().size() > 3) {
      ...
      if (uri.getPathSegments().get(0).equalsIgnoreCase("repos")) {
        ...
        startWithRepo = true;
      } else if (uri.getPathSegments().get(0).equalsIgnoreCase(
                     "repositories")) {
      ...
      for (int i = startWithRepo ? 4 : 3; i < uri.getPathSegments().size() - 1;
           i++) {
  ...
  protected void onCreate(@Nullable Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      ...
      String defaultBranch = Objects.toString(
          bundle.getString(BundleConstant.EXTRA_THREE), "master");
      getSupportFragmentManager()
          .beginTransaction()
          .add(R.id.fragmentContainer,
               RepoFilePathFragment.newInstance(login, repoId, path,
                                                defaultBranch, true),
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    if (item.getItemId() == android.R.id.home) {
      Intent intent = ActivityHelper.editBundle(
          RepoPagerActivity.createIntent(this, repoId, login), isEnterprise());
  ...
  public void onBackPressed() {
    RepoFilePathFragment filePathView =
        (RepoFilePathFragment)AppHelper.getFragmentByTag(
            getSupportFragmentManager(), "RepoFilePathFragment");
