...
class ProfileFollowingPresenter extends BasePresenter<ProfileFollowingMvp.View>
    implements ProfileFollowingMvp.Presenter {
  ...
  public boolean onCallApi(int page, @Nullable String parameter) {
    ...
    makeRestCall(
        RestProvider.getUserService(isEnterprise())
            .getFollowing(parameter, page),
        response -> {
          lastPage = response.getLast();
          if (getCurrentPage() == 1) {
            manageDisposable(
                User.saveUserFollowingList(response.getItems(), parameter));
          }
  ...
  public void onWorkOffline(@NonNull String login) {
    if (users.isEmpty()) {
      manageDisposable(
          RxHelper.getSingle(User.getUserFollowingList(login))
              .subscribe(
                  userModels
