...
public class LoginActivity extends BaseActivity<LoginMvp.View, LoginPresenter>
  ...
  @OnClick(R.id.browserLogin)
  void onOpenBrowser() {
    if (isEnterprise()) {
      MessageDialogView
          .newInstance(getString(R.string.warning),
                       getString(R.string.github_enterprise_reply), true,
                       Bundler.start().put("hide_buttons", true).end())
  ...
  @OnCheckedChanged(R.id.accessTokenCheckbox)
  void onCheckChanged(final boolean checked) {
    isBasicAuth = !checked;
    password.setHint(checked ? getString(R.string.access_token)
  ...
  @OnEditorAction(R.id.twoFactorEditText)
  public boolean onSend2FA() {
    doLogin();
    return true;
  }

  @OnEditorAction(R.id.endpointEditText)
  boolean onSendEndpoint() {
    doLogin();
    return true;
  }
  ...
  @Override
  public void onEmptyUserName(final boolean isEmpty) {
    username.setError(isEmpty ? getString(R.string.required_field) : null);
  }

  @Override
  public void onRequire2Fa() {
    Toasty.warning(App.getInstance(), getString(R.string.two_factors_otp_error))
        .show();
  ...
  @Override
  public void onEmptyPassword(final boolean isEmpty) {
    password.setError(isEmpty ? getString(R.string.required_field) : null);
  }

  @Override
  public void onEmptyEndpoint(final boolean isEmpty) {
    endpoint.setError(isEmpty ? getString(R.string.required_field) : null);
  }

  @Override
  public void onSuccessfullyLoggedIn(final boolean extraLogin) {
    checkPurchases(() -> {
      hideProgress();
      onRestartApp();
    });
                ...
                if (purchases != null && !purchases.isEmpty()) {
                  for (Purchase purchase : purchases) {
                    ...
                    if (!InputHelper.isEmpty(sku)) {
                      DonateActivity.Companion.enableProduct(sku,
                                                             App.getInstance());
                    }
                  }
                }
  ...
  private void doLogin() {
    if (progress.getVisibility() == View.GONE) {
      getPresenter().login(InputHelper.toString(username),
                           ...
                           InputHelper.toString(twoFactor), isBasicAuth,
                           InputHelper.toString(endpoint));
    }
  }
}
