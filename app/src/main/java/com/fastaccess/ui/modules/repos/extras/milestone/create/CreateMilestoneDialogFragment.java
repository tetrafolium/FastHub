...
public class CreateMilestoneDialogFragment
    extends BaseDialogFragment<CreateMilestoneMvp.View,
                               CreateMilestonePresenter>
    implements CreateMilestoneMvp.View, DatePickerCallback {

  public static final String TAG =
      CreateMilestoneDialogFragment.class.getSimpleName();
  ...
  public static CreateMilestoneDialogFragment
  newInstance(final @NonNull String login, final @NonNull String repo) {
    CreateMilestoneDialogFragment fragment =
        new CreateMilestoneDialogFragment();
  ...
  public void onAttach(final @NotNull Context context) {
    ...
    if (getParentFragment() instanceof CreateMilestoneMvp.OnMilestoneAdded) {
      onMilestoneAdded =
          (CreateMilestoneMvp.OnMilestoneAdded)getParentFragment();
    } else {
  ...
  protected void onFragmentCreated(final @NonNull View view,
    ...
    toolbar.setOnMenuItemClickListener(item -> {
      getPresenter().onSubmit(InputHelper.toString(title),
                              InputHelper.toString(dueOnEditText),
                              InputHelper.toString(description), login, repo);
      return true;
    });
  }
  ...
  boolean onTouch(final MotionEvent event) {
    if (event.getAction() == MotionEvent.ACTION_UP) {
      DatePickerFragmentDialog.newInstance().show(getChildFragmentManager(),
  ...
  @Override
  public void onDateSet(final long date) {
    if (date > 0) {
  ...
  @Override
  public void onShowTitleError(final boolean isError) {
    title.setError(isError ? getString(R.string.required_field) : null);
  }

  @Override
  public void onMilestoneAdded(final @NonNull MilestoneModel milestoneModel) {
    hideProgress();
