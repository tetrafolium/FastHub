...
public class SettingsCategoryFragment extends PreferenceFragmentCompat
    implements Preference.OnPreferenceChangeListener,
               NotificationSoundMvp.NotificationSoundListener {

  public static final String TAG =
      SettingsCategoryFragment.class.getSimpleName();
  ...
  public void onCreatePreferences(final Bundle savedInstanceState,
    ...
    default:
      Toast
          .makeText(App.getInstance(), "You reached the impossible :'(",
                    Toast.LENGTH_SHORT)
          .show();
    }
  }
    ...
    if (preference.getKey().equalsIgnoreCase("notificationEnabled")) {
      if ((boolean)newValue) {
        ...
        NotificationSchedulerJobTask.scheduleJob(
            App.getInstance(), PrefGetter.getNotificationTaskDuration(), true);
      } else {
      ...
      if (newValue.toString().equalsIgnoreCase(appColor))
        return true;
      Toasty
          .warning(App.getInstance(), getString(R.string.change_theme_warning),
                   Toast.LENGTH_LONG)
          .show();
      callback.onThemeChanged();
      return true;
    ...
    if (requestCode == PERMISSION_REQUEST_CODE) {
      if (permissions[0].equals(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
        if (grantResults[0] == PackageManager.PERMISSION_GRANTED) {
          ...
          PrefHelper.set(
              "backed_up",
              new SimpleDateFormat("MM/dd", Locale.ENGLISH).format(new Date()));
          findPreference("backup").setSummary(
              getString(R.string.backup_summary, getString(R.string.now)));
          Toasty.success(App.getInstance(), getString(R.string.backed_up))
              .show();
        } else {
          Toasty.error(App.getInstance(), getString(R.string.permission_failed))
              .show();
        }
      } else if (permissions[0].equals(
    ...
    if (resultCode == RESULT_OK) {
      if (requestCode == RESTORE_REQUEST_CODE) {
        ...
        if (notificationSoundPath != null &&
            notificationSoundPath.isVisible()) {
  ...
  @Override
  public void onSoundSelected(final Uri uri) {
    PrefGetter.setNotificationSound(uri);
    if (notificationSoundPath != null && notificationSoundPath.isVisible())
      notificationSoundPath.setSummary(
          FileHelper.getRingtoneName(getContext(), uri));
  }
  ...
  private void addBackup() {
    ...
    findPreference("backup").setOnPreferenceClickListener((Preference
                                                               preference) -> {
      if (ContextCompat.checkSelfPermission(
        ...
        try {
          ...
          try (FileOutputStream outputStream = new FileOutputStream(backup)) {
            try (OutputStreamWriter myOutWriter =
                     new OutputStreamWriter(outputStream)) {
              myOutWriter.append(json);
            }
          }
    ...
    if (PrefHelper.getString("backed_up") != null) {
      findPreference("backup").setSummary(
          SpannableBuilder.builder()
              .append(getString(R.string.backup_summary,
                                PrefHelper.getString("backed_up")))
              .append("\n")
  ...
  private void addBehaviour() {
    ...
    findPreference("clear_image_cache")
        .setOnPreferenceClickListener(preference -> {
          final Glide glide = Glide.get(App.getInstance());
          disposable.add(RxHelper
  ...
  private void addNotifications() {
    ...
    notificationSoundPath.setSummary(FileHelper.getRingtoneName(
        getContext(), PrefGetter.getNotificationSound()));
    notificationSoundPath.setOnPreferenceClickListener(preference -> {
      NotificationSoundBottomSheet.Companion
          .newInstance(FileHelper.getRingtoneName(
              getContext(), PrefGetter.getNotificationSound()))
  ...
  private void restoreData(final Intent data) {
    ...
    if (!InputHelper.isEmpty(json)) {
      try {
        ...
        Map<String, ?> savedPref =
            gson.fromJson(json.toString(), typeOfHashMap);
        ...
        Toasty
            .error(App.getInstance(), getString(R.string.error),
                   Toast.LENGTH_SHORT)
            .show();
      }
    }
