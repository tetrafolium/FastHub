...
public class MainPresenter
    extends BasePresenter<MainMvp.View> implements MainMvp.Presenter {
  ...
  MainPresenter() {
    ...
    manageDisposable(
        RxHelper
            .getObservable(
                RestProvider.getUserService(isEnterprise()).getUser())
            .flatMap(login -> {
            ...
            .flatMapSingle(notificationPageable -> {
              if (notificationPageable != null &&
                  (notificationPageable.getItems() != null &&
                   !notificationPageable.getItems().isEmpty())) {
                return Notification.saveAsSingle(
                    notificationPageable.getItems());
              } else {
  ...
  @Override
  public boolean canBackPress(final @NonNull DrawerLayout drawerLayout) {
  ...
  public void onModuleChanged(final @NonNull FragmentManager fragmentManager,
                              final @MainMvp.NavigationType int type) {
    ...
    case MainMvp.ISSUES:
      if (issuesView == null) {
        onAddAndHide(fragmentManager, MyIssuesPagerFragment.newInstance(),
                     currentVisible);
      } else {
