...
class OrgTeamPresenter
    extends BasePresenter<OrgTeamMvp.View> implements OrgTeamMvp.Presenter {
  ...
  public boolean onCallApi(int page, @Nullable String parameter) {
    ...
    makeRestCall(
        RestProvider.getOrgService(isEnterprise()).getOrgTeams(parameter, page),
        response -> {
  ...
  @Override
  public void onItemClick(int position, View v, TeamsModel item) {
    Logger.e(item.getUrl());
    TeamPagerActivity.startActivity(v.getContext(), item.getId(),
                                    item.getName());
  }

  @Override
  public void onItemLongClick(int position, View v, TeamsModel item) {}
}
