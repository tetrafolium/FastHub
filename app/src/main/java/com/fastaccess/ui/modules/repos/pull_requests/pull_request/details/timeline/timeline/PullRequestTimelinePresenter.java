...
public class PullRequestTimelinePresenter
    extends BasePresenter<PullRequestTimelineMvp.View>
    implements PullRequestTimelineMvp.Presenter {
    ...
    if (pullRequest != null) {
      if (item.getType() == TimelineModel.COMMENT) {
        if (v.getId() == R.id.commentMenu) {
          ...
          boolean isOwner =
              CommentsHelper.isOwner(username, pullRequest.getLogin(),
                                     item.getComment().getUser().getLogin()) ||
              isCollaborator;
          ...
          popupMenu.setOnMenuItemClickListener(item1 -> {
            ...
            if (item1.getItemId() == R.id.delete) {
              ...
              getView().onReply(item.getComment().getUser(),
                                item.getComment().getBodyHtml());
            } else if (item1.getItemId() == R.id.edit) {
              getView().onEditComment(item.getComment());
            } else if (item1.getItemId() == R.id.share) {
              ActivityHelper.shareUrl(v.getContext(),
                                      item.getComment().getHtmlUrl());
            }
            return true;
          });
          popupMenu.show();
        ...
        if (v.getId() == R.id.commentMenu) {
          ...
          popupMenu.inflate(R.menu.comments_menu);
          String username = Login.getUser().getLogin();
          boolean isOwner = CommentsHelper.isOwner(
                                username, item.getPullRequest().getLogin(),
          ...
          popupMenu.setOnMenuItemClickListener(item1 -> {
            ...
            if (item1.getItemId() == R.id.reply) {
              getView().onReply(item.getPullRequest().getUser(),
                                item.getPullRequest().getBodyHtml());
            } else if (item1.getItemId() == R.id.edit) {
              Activity activity = ActivityHelper.getActivity(v.getContext());
              if (activity == null)
                return false;
              CreateIssueActivity.startForResult(
                  activity, item.getPullRequest().getLogin(),
                  item.getPullRequest().getRepoId(), item.getPullRequest(),
                  isEnterprise());
            } else if (item1.getItemId() == R.id.share) {
              ActivityHelper.shareUrl(v.getContext(),
                                      item.getPullRequest().getHtmlUrl());
            }
            return true;
          });
          popupMenu.show();
        } else {
          onHandleReaction(v.getId(), item.getPullRequest().getNumber(),
  ...
  public void onItemLongClick(final int position, final View v,
    ...
    if (getView() == null || getView().getPullRequest() == null)
      return;
    if (item.getType() == TimelineModel.COMMENT ||
        item.getType() == TimelineModel.HEADER) {
      if (v.getId() == R.id.commentMenu &&
          item.getType() == TimelineModel.COMMENT) {
        ...
        if (!InputHelper.isEmpty(login) && !InputHelper.isEmpty(repoId)) {
          ...
          if (type != null) {
            if (item.getType() == TimelineModel.HEADER) {
              getView().showReactionsPopup(type, login, repoId,
                                           item.getPullRequest().getNumber(),
                                           ReactionsProvider.HEADER);
            } else {
              getView().showReactionsPopup(type, login, repoId,
  ...
  @Override
  public void onHandleDeletion(final @Nullable Bundle bundle) {
    if (getView() == null || getView().getPullRequest() == null)
      return;
    if (bundle != null) {
      PullRequest pullRequest = getView().getPullRequest();
      ...
      if (commId != 0 && !isReviewComment) {
        makeRestCall(RestProvider.getIssueService(isEnterprise())
                         .deleteIssueComment(login, repoId, commId),
        ...
        makeRestCall(RestProvider.getReviewService(isEnterprise())
                         .deleteComment(login, repoId, commId),
                     booleanResponse -> sendToView(view -> {
                       if (booleanResponse.code() == 204) {
                         view.onRemoveReviewComment(groupPosition,
                                                    commentPosition);
                       } else {
                         view.showMessage(R.string.error,
                                          R.string.error_deleting_comment);
  ...
  @Override
  public void
  onHandleReaction(final int vId, final long idOrNumber,
    ...
    String login = pullRequest.getLogin();
    String repoId = pullRequest.getRepoId();
    Observable observable = getReactionsProvider().onHandleReaction(
        vId, idOrNumber, login, repoId, reactionType, isEnterprise());
  ...
  public boolean isMerged(final PullRequest pullRequest) {
    return pullRequest != null &&
        (pullRequest.isMerged() ||
         !InputHelper.isEmpty(pullRequest.getMergedAt()));
  }
  ...
  @Override
  public void onHandleComment(final @NonNull String text,
  ...
  @Override
  public boolean isPreviouslyReacted(final long commentId, final int vId) {
  ...
  @Override
  public void onClick(final int groupPosition, final int commentPosition,
                      final @NonNull View v,
                      final @NonNull ReviewCommentModel comment) {
    if (getView() == null || getView().getPullRequest() == null)
      return;
    if (v.getId() == R.id.commentMenu) {
      PopupMenu popupMenu = new PopupMenu(v.getContext(), v);
      ...
      String username = Login.getUser().getLogin();
      boolean isOwner = CommentsHelper.isOwner(
                            username, getView().getPullRequest().getLogin(),
      ...
      popupMenu.setOnMenuItemClickListener(item1 -> {
        if (getView() == null)
          return false;
        if (item1.getItemId() == R.id.delete) {
          getView().onShowReviewDeleteMsg(comment.getId(), groupPosition,
                                          commentPosition);
        } else if (item1.getItemId() == R.id.reply) {
          EditReviewCommentModel model = new EditReviewCommentModel();
          ...
          getView().onReplyOrCreateReview(comment.getUser(),
                                          comment.getBodyHtml(), groupPosition,
                                          commentPosition, model);
        } else if (item1.getItemId() == R.id.edit) {
          getView().onEditReviewComment(comment, groupPosition,
                                        commentPosition);
        } else if (item1.getItemId() == R.id.share) {
      ...
      onHandleReaction(v.getId(), comment.getId(),
                       ReactionsProvider.REVIEW_COMMENT);
    }
  }

  @Override
  public void onLongClick(final int groupPosition, final int commentPosition,
                          final @NonNull View v,
                          final @NonNull ReviewCommentModel model) {
    if (getView() == null || getView().getPullRequest() == null)
      return;
    PullRequest pullRequest = getView().getPullRequest();
    String login = pullRequest.getLogin();
    ...
    if (!InputHelper.isEmpty(login) && !InputHelper.isEmpty(repoId)) {
      ReactionTypes type = ReactionTypes.get(v.getId());
      if (type != null) {
        getView().showReactionsPopup(type, login, repoId, model.getId(),
                                     ReactionsProvider.REVIEW_COMMENT);
      } else {
  ...
  @Override
  public boolean onCallApi(final int page,
                           final @Nullable PullRequest parameter) {
    if (parameter == null) {
    ...
    if (parameter.getHead() != null) {
      Observable<List<TimelineModel>> observable = Observable.zip(
          RestProvider.getIssueService(isEnterprise())
          ...
          RestProvider.getPullRequestService(isEnterprise())
              .getPullStatus(login, repoId, parameter.getHead().getSha())
              .onErrorReturn(
                  throwable
                  -> RestProvider.getPullRequestService(isEnterprise())
                         .getPullStatus(login, repoId,
                                        parameter.getBase().getSha())
                         .blockingFirst(new PullRequestStatusModel())),
          (response, comments, status) -> {
            if (response != null) {
              lastPage = response.getLast();
              List<TimelineModel> models = TimelineConverter.INSTANCE.convert(
                  response.getItems(), comments);
      ...
      makeRestCall(
          observable,
          timeline -> sendToView(view -> view.onNotifyAdapter(timeline, page)));
      return true;
    }
    return false;
  }
  //
  //    @Nullable private Observable<PullRequestTimelineModel>
  //    getTimelineObservable(Response<PullRequestTimelineQuery.Data> response,
  //                                                                                 boolean isMergeable) {
  //        if (!response.hasErrors()) {
  //            PullRequestTimelineQuery.Data data = response.data();
  //            if (data != null) {
  //                PullRequestTimelineQuery.Repository repo =
  //                data.repository(); PullRequestTimelineQuery.PullRequest
  //                pullRequest = repo != null ? repo.pullRequest() : null; if
  //                (pullRequest != null) {
  //                    PullRequestTimelineQuery.Timeline timeline =
  //                    pullRequest.timeline(); lastPage =
  //                    timeline.pageInfo().hasNextPage() ? Integer.MAX_VALUE :
  //                    0; pages.clear(); ArrayList<PullRequestTimelineModel>
  //                    models = new ArrayList<>();
  //                    PullRequestTimelineQuery.PullRequestCommits
  //                    pullRequestCommits = pullRequest.pullRequestCommits();
  //                    List<PullRequestTimelineQuery.PullRequestCommit> commits
  //                    = pullRequestCommits.pullRequestCommit(); if (commits !=
  //                    null && !commits.isEmpty() && page <= 1) {
  //                        PullRequestTimelineQuery.Status status =
  //                        commits.get(0).commit().status(); if (status !=
  //                        null) {
  //                            models.add(new PullRequestTimelineModel(status,
  //                            isMergeable));
  //                        }
  //                    }
  //                    List<PullRequestTimelineQuery.Edge> edges =
  //                    timeline.edges(); if (edges != null) {
  //                        Stream.of(edges).forEachIndexed((i, edge) ->
  //                        pages.append(i, edge.cursor()));
  //                    }
  //                    List<PullRequestTimelineQuery.Node> nodes =
  //                    timeline.nodes(); if (nodes != null) {
  //                        for (PullRequestTimelineQuery.Node node : nodes) {
  //                            models.add(new PullRequestTimelineModel(node));
  //                        }
  //                    }
  //                    return
  //                    RxHelper.getObservable(Observable.fromIterable(models));
  //                }
  //            }
  //        }
  //        return null;
  //    }
  //
  //    @NonNull private PullRequestTimelineQuery getTimelineBuilder(@NonNull
  //    String login, @NonNull String repoId, int number, int page) {
  ...
  //    }
  //
  //    private void loadEverything(@NonNull String login, @NonNull String
  //    repoId, int number,
  //                                @NonNull String sha, boolean isMergeable,
  //                                int page) {
  //        PullRequestTimelineQuery query = getTimelineBuilder(login, repoId,
  //        number, page); ApolloCall<PullRequestTimelineQuery.Data> apolloCall
  //        = App.getInstance().getApolloClient().query(query);
  //        Observable<PullRequestTimelineModel> observable =
  //        Rx2Apollo.from(apolloCall)
  //                .flatMap(response -> {
  //                    Observable<PullRequestTimelineModel> models =
  //                    getTimelineObservable(response, isMergeable); return
  //                    models != null ? models :
  //                    RxHelper.getObservable(Observable.fromIterable(new
  //                    ArrayList<>()));
  //                });
  //        makeRestCall(observable.toList().toObservable(),
  //                pullRequestTimelineModels -> sendToView(view ->
  //                view.onNotifyAdapter(pullRequestTimelineModels, page)));
  //    }
}
