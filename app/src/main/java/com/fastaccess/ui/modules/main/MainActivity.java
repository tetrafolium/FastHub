...
public class MainActivity
    extends BaseActivity<MainMvp.View, MainPresenter> implements MainMvp.View {
  ...
  protected void onNewIntent(final Intent intent) {
    ...
    if (intent != null && intent.getExtras() != null) {
      boolean recreate =
          intent.getExtras().getBoolean(BundleConstant.YES_NO_EXTRA);
      if (recreate)
        recreate();
    }
  }
  ...
  public boolean onOptionsItemSelected(final @NonNull MenuItem item) {
    if (item.getItemId() == android.R.id.home) {
      ...
      ViewHelper.tintDrawable(
          item.setIcon(R.drawable.ic_notifications_none).getIcon(),
  ...
  public boolean onPrepareOptionsMenu(final Menu menu) {
    if (isLoggedIn() && Notification.hasUnreadNotifications()) {
      ...
      ViewHelper.tintDrawable(menu.findItem(R.id.notifications)
                                  .setIcon(R.drawable.ic_notifications_none)
                                  .getIcon(),
                              ViewHelper.getIconColor(this));
    }
  ...
  @Override
  public void onNavigationChanged(final @MainMvp.NavigationType int navType) {
  ...
  public void onScrollTop(final int index) {
    ...
    if (index == 0) {
      FeedsFragment homeView =
          (FeedsFragment)getFragmentByTag(fragmentManager, FeedsFragment.TAG);
      if (homeView != null) {
        homeView.onScrollTop(index);
      }
    } else if (index == 1) {
      MyIssuesPagerFragment issuesView =
          (MyIssuesPagerFragment)getFragmentByTag(fragmentManager,
                                                  MyIssuesPagerFragment.TAG);
      if (issuesView != null) {
        issuesView.onScrollTop(index);
      }
    } else if (index == 2) {
      MyPullsPagerFragment pullRequestView =
          (MyPullsPagerFragment)getFragmentByTag(fragmentManager,
  ...
  @Shortcut(id = "myIssues", icon = R.drawable.ic_app_shortcut_issues,
            shortLabelRes = R.string.issues, rank = 2, action = "myIssues")
  public void
  myIssues() {} // do nothing

  @Shortcut(id = "myPulls", icon = R.drawable.ic_app_shortcut_pull_requests,
            shortLabelRes = R.string.pull_requests, rank = 3,
            action = "myPulls")
  public void
  myPulls() {} // do nothing

  private void onInit(final @Nullable Bundle savedInstanceState) {
    if (isLoggedIn()) {
      if (savedInstanceState == null) {
        ...
        if (getIntent().getAction() != null) {
          if (getIntent().getAction().equalsIgnoreCase("myPulls")) {
            ...
            getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.container, MyPullsPagerFragment.newInstance(),
                         MyPullsPagerFragment.TAG)
                .commit();
            ...
            getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.container, MyIssuesPagerFragment.newInstance(),
                         MyIssuesPagerFragment.TAG)
                .commit();
        ...
        if (attachFeeds) {
          getSupportFragmentManager()
              .beginTransaction()
              .replace(R.id.container, FeedsFragment.newInstance(null),
                       FeedsFragment.TAG)
              .commit();
        }
      }
