...
public class RepoFilePathFragment
    extends BaseFragment<RepoFilePathMvp.View, RepoFilePathPresenter>
    implements RepoFilePathMvp.View {
  ...
  public static RepoFilePathFragment
  newInstance(@NonNull String login, @NonNull String repoId,
              @Nullable String path, @NonNull String defaultBranch) {
    return newInstance(login, repoId, path, defaultBranch, false);
  }

  public static RepoFilePathFragment newInstance(@NonNull String login,
  ...
  void onAddFile() {
    if (PrefGetter.isProEnabled() || PrefGetter.isAllFeaturesUnlocked()) {
      RepoFile repoFile = !adapter.isEmpty()
                              ? adapter.getItem(adapter.getItemCount() - 1)
                              : null;
      EditRepoFileModel fileModel = new EditRepoFileModel(
          getPresenter().login, getPresenter().repoId,
          repoFile != null ? repoFile.getPath() : "", ref,
          repoFile != null ? repoFile.getSha() : "", null, null, false);
    ...
    if (ActivityHelper.checkAndRequestReadWritePermission(getActivity())) {
      MessageDialogView
          .newInstance(
              ...
              Bundler.start().put(BundleConstant.YES_NO_EXTRA, true).end())
  ...
  void onSearchClicked() {
    startActivity(SearchFileActivity.createIntent(
        getContext(), getPresenter().getLogin(), getPresenter().getRepoId(),
        isEnterprise()));
  }
  ...
  void onBackClicked() {
    if (adapter.getItemCount() > 0) {
      adapter.clear();
      getRepoFilesView().onSetData(getPresenter().getLogin(),
                                   getPresenter().getRepoId(), "", ref, false,
                                   null);
    }
  }
  ...
  void onBranchesClicked() {
    BranchesPagerFragment.Companion
        .newInstance(getPresenter().login, getPresenter().repoId)
        .show(getChildFragmentManager(), "BranchesFragment");
  }

  @Override
  public void onNotifyAdapter(@Nullable List<RepoFile> items, int page) {
    hideProgress();
  ...
  @Override
  public void onItemClicked(@NonNull RepoFile model, int position) {
    if (getRepoFilesView().isRefreshing())
      return;
    getRepoFilesView().onSetData(
        getPresenter().getLogin(), getPresenter().getRepoId(),
  ...
  @Override
  public void onAppendPath(@NonNull RepoFile model) {
    getRepoFilesView().onSetData(
        getPresenter().getLogin(), getPresenter().getRepoId(),
  ...
  protected void onFragmentCreated(@NonNull View view,
    ...
    } else if (getPresenter().getPaths().isEmpty() &&
  ...
  public void setUserVisibleHint(boolean isVisibleToUser) {
    ...
    if (isSafe() && getRepoFilesView() != null)
      getRepoFilesView().onHiddenChanged(!isVisibleToUser);
  }
  ...
  public RepoFilesFragment getRepoFilesView() {
    if (repoFilesView == null) {
      repoFilesView =
          (RepoFilesFragment)getChildFragmentManager().findFragmentById(
              R.id.filesFragment);
    }
  ...
  public void onScrollTop(int index) {
    ...
    if (repoFilesView != null)
      repoFilesView.onScrollTop(index);
  }

  @Override
  public void onBranchSelected(@NonNull BranchesModel branch) {
    ref = branch.getName();
    branches.setText(ref);
    getRepoFilesView().onSetData(getPresenter().getLogin(),
                                 getPresenter().getRepoId(), "", ref, true,
                                 null);
    onBackClicked();
  }
  ...
  private void showReload() { hideProgress(); }
