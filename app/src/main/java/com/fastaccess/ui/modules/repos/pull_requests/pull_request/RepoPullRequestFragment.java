...
public class RepoPullRequestFragment
    extends BaseFragment<RepoPullRequestMvp.View, RepoPullRequestPresenter>
    implements RepoPullRequestMvp.View {
  ...
  public static RepoPullRequestFragment
  newInstance(@NonNull String repoId, @NonNull String login,
  ...
  @Override
  public void onNotifyAdapter(@Nullable List<PullRequest> items, int page) {
    hideProgress();
  ...
  @Override
  public void onUpdateCount(int totalCount) {
    if (tabsBadgeListener != null)
      tabsBadgeListener.onSetBadge(
          getPresenter().getIssueState() == IssueState.open ? 0 : 1,
          totalCount);
  }

  @Override
  public void onOpenPullRequest(@NonNull PullsIssuesParser parser) {
    Intent intent = PullRequestPagerActivity.createIntent(
        getContext(), parser.getRepoId(), parser.getLogin(), parser.getNumber(),
        false, isEnterprise());
    startActivityForResult(intent,
                           RepoPullRequestMvp.PULL_REQUEST_REQUEST_CODE);
  }

  @Override
  public void onShowPullRequestPopup(@NonNull PullRequest item) {
  ...
  private IssueState getIssueState() {
    return (
        (IssueState)getArguments().getSerializable(BundleConstant.EXTRA_TWO));
  }
