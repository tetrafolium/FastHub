...
class CommitPagerPresenter extends BasePresenter<CommitPagerMvp.View>
    implements CommitPagerMvp.Presenter {
  ...
  public void onActivityCreated(final @Nullable Intent intent) {
    if (intent != null && intent.getExtras() != null) {
      ...
      if (commitModel != null) {
        ...
        return;
      } else if (!InputHelper.isEmpty(sha) && !InputHelper.isEmpty(login) &&
  ...
  @Override
  public void onWorkOffline(final @NonNull String sha,
                            final @NonNull String repoId,
                            final @NonNull String login) {
    manageDisposable(
        RxHelper.getObservable(Commit.getCommit(sha, repoId, login))
            .subscribe(commit -> {
              commitModel = commit;
              sendToView(CommitPagerMvp.View::onSetup);
            }));
