...
public class ReactionsDialogPresenter
    extends BasePresenter<ReactionsDialogMvp.View>
    implements ReactionsDialogMvp.Presenter {
  private int page;
  ...
  @ReactionsProvider.ReactionType
  ...
  public void onFragmentCreated(final @Nullable Bundle bundle) {
    if (bundle != null) {
      ...
      reactionType =
          (ReactionTypes)bundle.getSerializable(BundleConstant.EXTRA_TYPE);
  ...
  public boolean onCallApi(final int page, final @Nullable Object parameter) {
    ...
    if (page > lastPage || lastPage == 0 ||
        (login == null || repoId == null || reactionType == null)) {
    ...
    case ReactionsProvider.COMMENT:
      observable = RestProvider.getReactionsService(isEnterprise())
                       .getIssueCommentReaction(
                           login, repoId, id, reactionType.getContent(), page);
      break;
    case ReactionsProvider.COMMIT:
      observable = RestProvider.getReactionsService(isEnterprise())
                       .getCommitReaction(login, repoId, id,
                                          reactionType.getContent(), page);
      break;
    case ReactionsProvider.HEADER:
      observable = RestProvider.getReactionsService(isEnterprise())
                       .getIssueReaction(login, repoId, id,
                                         reactionType.getContent(), page);
      break;
  ...
  ReactionTypes getReactionType() { return reactionType; }
}
