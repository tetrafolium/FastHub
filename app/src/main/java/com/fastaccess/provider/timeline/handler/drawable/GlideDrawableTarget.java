...
class GlideDrawableTarget extends SimpleTarget<Drawable> {
  ...
  GlideDrawableTarget(final UrlDrawable urlDrawable,
                      final WeakReference<TextView> container,
                      final int width) {
  ...
  @Override
  public void
  onResourceReady(final @NonNull Drawable resource,
                  final @Nullable Transition<? super Drawable> transition) {
    if (container != null && container.get() != null) {
      ...
      textView.post(() -> {
        ...
        if (resource instanceof GifDrawable) {
          urlDrawable.setCallback(
              (Drawable.Callback)textView.getTag(R.id.drawable_callback));
