...
public class ReactionsProvider {
  ...
  @Nullable
  public Observable
  onHandleReaction(final @IdRes int viewId, final long idOrNumber,
                   final @Nullable String login, final @Nullable String repoId,
                   final @ReactionType int reactionType,
      ...
      if (!isPreviouslyReacted(idOrNumber, viewId)) {
        ...
        if (reactionTypes != null) {
            ...
            observable =
                RestProvider.getReactionsService(isEnterprise)
                    .postIssueCommentReaction(
                        new PostReactionModel(reactionTypes.getPostContent()),
                        login, repoId, idOrNumber);
            break;
          case HEADER:
            observable =
                RestProvider.getReactionsService(isEnterprise)
          ...
          return RxHelper.safeObservable(observable)
              .doOnNext(
                  response -> getReactionsMap().put(idOrNumber, response));
        }
      } else {
        ReactionsModel reactionsModel = getReactionsMap().get(idOrNumber);
        if (reactionsModel != null) {
          return RxHelper
              .safeObservable(RestProvider.getReactionsService(isEnterprise)
                                  .delete(reactionsModel.getId()))
  ...
  public boolean isPreviouslyReacted(final long idOrNumber,
                                     final @IdRes int vId) {
    ReactionsModel reactionsModel = getReactionsMap().get(idOrNumber);
    if (reactionsModel == null ||
        InputHelper.isEmpty(reactionsModel.getContent())) {
      return false;
    }
    ReactionTypes type = ReactionTypes.get(vId);
    return type != null &&
        (type.getContent().equals(reactionsModel.getContent()) ||
         type.getPostContent().equals(reactionsModel.getContent()));
  }
