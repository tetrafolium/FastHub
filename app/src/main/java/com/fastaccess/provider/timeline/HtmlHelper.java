...
public class HtmlHelper {
  ...
  public static void htmlIntoTextView(final @NonNull TextView textView,
                                      final @NonNull String html,
                                      final int width) {
  ...
  private static void registerClickEvent(final @NonNull TextView textView) {
    BetterLinkMovementExtended betterLinkMovementMethod =
        BetterLinkMovementExtended.linkifyHtml(textView);
    ...
    betterLinkMovementMethod.setOnLinkLongClickListener((view, url) -> {
      ...
      menu.setOnMenuItemClickListener(menuItem -> {
        ...
        case R.id.open_new_window:
          SchemeParser.launchUri(view.getContext(), Uri.parse(url), false,
                                 true);
          return true;
        default:
          return false;
  ...
  private static HtmlSpanner initHtml(final @NonNull TextView textView,
                                      final int width) {
    @PrefGetter.ThemeType int theme = PrefGetter.getThemeType();
    @ColorInt int windowBackground = getWindowBackground(theme);
    Drawable checked = ContextCompat.getDrawable(textView.getContext(),
                                                 R.drawable.ic_checkbox_small);
    Drawable unchecked = ContextCompat.getDrawable(
        textView.getContext(), R.drawable.ic_checkbox_empty_small);
    HtmlSpanner mySpanner = new HtmlSpanner();
    mySpanner.setStripExtraWhiteSpace(true);
    mySpanner.registerHandler("pre",
                              new PreTagHandler(windowBackground, true, theme));
    mySpanner.registerHandler(
        "code", new PreTagHandler(windowBackground, false, theme));
    ...
    mySpanner.registerHandler("hr",
                              new HrHandler(windowBackground, width, false));
  ...
  @ColorInt
  public static int getWindowBackground(final @PrefGetter.ThemeType int theme) {
  ...
  private static final String TOGGLE_START =
      "<span class=\"email-hidden-toggle\">";

  private static final String TOGGLE_END = "</span>";

  private static final String REPLY_START =
  ...
  private static void strip(final StringBuilder input, final String prefix,
                            final String suffix) {
  ...
  private static boolean replace(final StringBuilder input, final String from,
                                 final String to) {
