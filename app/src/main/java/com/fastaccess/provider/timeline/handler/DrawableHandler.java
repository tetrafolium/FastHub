  ...
  public void handleTagNode(TagNode node, SpannableStringBuilder builder,
    ...
    if (!InputHelper.isEmpty(src)) {
      if (!PrefGetter.isAutoImageDisabled()) {
        ...
        builder.setSpan(new ImageSpan(imageGetter.getDrawable(src)), start,
                        builder.length(), SPAN_EXCLUSIVE_EXCLUSIVE);
        builder.append("\n");
      } else {
        builder.append(SpannableBuilder.builder().clickable(
            "Image", v -> SchemeParser.launchUri(v.getContext(), src)));
        builder.append("\n");
      }
    }
  }
