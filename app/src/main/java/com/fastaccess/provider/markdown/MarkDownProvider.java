...
public class MarkDownProvider {
  ...
  private static final String[] MARKDOWN_EXTENSIONS = {
      ...
      ".mkd", ".mkdown", ".ron",  ".rst",   "adoc"};

  private static final String[] ARCHIVE_EXTENSIONS = {
      ".zip",     ".7z",   ".rar",      ".tar.gz", ".tgz", ".tar.Z",
      ".tar.bz2", ".tbz2", ".tar.lzma", ".tlz",    ".apk", ".jar",
      ".dmg",     ".pdf",  ".ico",      ".docx",   ".doc", ".xlsx",
      ".hwp",     ".pptx", ".show",     ".mp3",    ".ogg", ".ipynb"};

  private MarkDownProvider() {}

  public static void setMdText(final @NonNull TextView textView,
  ...
  public static void setMdText(final @NonNull TextView textView,
                               final String markdown, final int width) {
    if (!InputHelper.isEmpty(markdown)) {
      render(textView, markdown, width);
    }
  }

  protected static void render(final @NonNull TextView textView,
                               final String markdown, final int width) {
    List<Extension> extensions = Arrays.asList(
        StrikethroughExtension.create(), AutolinkExtension.create(),
        TablesExtension.create(), InsExtension.create(),
        EmojiExtension.create(), MentionExtension.create(),
        YamlFrontMatterExtension.create());
    Parser parser = Parser.builder().extensions(extensions).build();
    try {
      Node node = parser.parse(markdown);
      String rendered =
          HtmlRenderer.builder().extensions(extensions).build().render(node);
      HtmlHelper.htmlIntoTextView(
          textView, rendered,
          (width - (textView.getPaddingStart() + textView.getPaddingEnd())));
  ...
  public static void stripMdText(final @NonNull TextView textView,
                                 final String markdown) {
    if (!InputHelper.isEmpty(markdown)) {
  ...
  @NonNull
  public static String stripMdText(final String markdown) {
    if (!InputHelper.isEmpty(markdown)) {
      Parser parser = Parser.builder().build();
      Node node = parser.parse(markdown);
  ...
  public static void addList(final @NonNull EditText editText,
  ...
  public static void addHeader(final @NonNull EditText editText,
                               final int level) {
    String source = editText.getText().toString();
    int selectionStart = editText.getSelectionStart();
    int selectionEnd = editText.getSelectionEnd();
  ...
  public static void addPhoto(final @NonNull EditText editText,
                              final @NonNull String title,
                              final @NonNull String link) {
    String result = "![" + InputHelper.toString(title) + "](" +
                    InputHelper.toString(link) + ")";
    insertAtCursor(editText, result);
  }

  public static void addLink(final @NonNull EditText editText,
                             final @NonNull String title,
                             final @NonNull String link) {
    String result = "[" + InputHelper.toString(title) + "](" +
                    InputHelper.toString(link) + ")";
  ...
  private static boolean hasNewLine(final @NonNull String source,
  ...
  public static void insertAtCursor(final @NonNull EditText editText,
    ...
    if (start >= 0 && end > 0 && start != end) {
      ...
      int index =
          editText.getSelectionStart() >= 0 ? editText.getSelectionStart() : 0;
