...
public class GithubResponseConverter extends Converter.Factory {
  ...
  responseBodyConverter(final Type type, final Annotation[] annotations,
  ...
  requestBodyConverter(final Type type, final Annotation[] parameterAnnotations,
                       final Annotation[] methodAnnotations,
                       final Retrofit retrofit) {
    return GsonConverterFactory.create(gson).requestBodyConverter(
        type, parameterAnnotations, methodAnnotations, retrofit);
  }

  public static class StringResponseConverter
      implements Converter<ResponseBody, String> {
    @Override
    public String convert(final @NonNull ResponseBody value)
        throws IOException {
      return value.string();
    }
  }
