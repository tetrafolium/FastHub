...
public class SchemeParser {
  ...
  public static void launchUri(final @NonNull Context context,
                               final @NonNull String url) {
    launchUri(context, Uri.parse(url), false);
  }

  public static void launchUri(final @NonNull Context context,
                               final @NonNull Uri data) {
    launchUri(context, data, false);
  }

  public static void launchUri(final @NonNull Context context,
                               final @NonNull Uri data,
                               final boolean showRepoBtn) {
  ...
  @Nullable
  private static Intent convert(final @NonNull Context context, final Uri data,
                                final boolean showRepoBtn) {
    if (data == null)
      return null;
    if (InputHelper.isEmpty(data.getHost()) ||
        InputHelper.isEmpty(data.getScheme())) {
    ...
    if (data.getPathSegments() != null && !data.getPathSegments().isEmpty()) {
      if (IGNORED_LIST.contains(data.getPathSegments().get(0)))
        return null;
  ...
  @Nullable
  private static Intent getIntentForURI(final @NonNull Context context,
                                        final @NonNull Uri data,
                                        final boolean showRepoBtn) {
    String authority = data.getAuthority();
    boolean isEnterprise = PrefGetter.isEnterprise() &&
                           LinkParserHelper.isEnterprise(
                               authority == null ? data.toString() : authority);
    if (HOST_GISTS.equals(data.getHost()) ||
        "gist".equalsIgnoreCase(data.getPathSegments().get(0))) {
      String extension = MimeTypeMap.getFileExtensionFromUrl(data.toString());
      if (!InputHelper.isEmpty(extension) &&
          !MarkDownProvider.isArchive(data.getLastPathSegment())) {
      ...
      if (MarkDownProvider.isArchive(data.toString()))
        return null;
      if (TextUtils.equals(authority, HOST_DEFAULT) ||
          TextUtils.equals(authority, RAW_AUTHORITY) ||
        ...
        Intent pullRequestIntent =
            getPullRequestIntent(context, data, showRepoBtn);
        ...
        Optional<Intent> intentOptional = returnNonNull(
            trending, projects, search, userIntent, repoIssues, repoPulls,
            pullRequestIntent, label, commit, commits, createIssueIntent,
            issueIntent, releasesIntent, repoIntent, repoWikiIntent, blob);
        Optional<Intent> empty = Optional.empty();
        if (intentOptional != null && intentOptional.isPresent() &&
            intentOptional != empty) {
  ...
  private static boolean getInvitationIntent(final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    return (segments != null && segments.size() == 3) &&
        "invitations".equalsIgnoreCase(uri.getLastPathSegment());
  }

  @Nullable
  private static Intent getPullRequestIntent(final @NonNull Context context,
    ...
    if (segments.size() > 3) {
      ...
      } else if (("pull".equals(segments.get(3)) ||
                  "pulls".equals(segments.get(3))) &&
                 segments.size() > 4) {
    ...
    if (issueNumber < 1)
      return null;
    return PullRequestPagerActivity.createIntent(
  ...
  @Nullable
  private static Intent getIssueIntent(final @NonNull Context context,
                                       final @NonNull Uri uri,
                                       final boolean showRepoBtn) {
    List<String> segments = uri.getPathSegments();
    if (segments == null || segments.size() < 3)
    ...
    if (segments.size() > 3) {
      if (segments.get(2).equalsIgnoreCase("issues")) {
        ...
        repo = segments.get(1);
        number = segments.get(3);
      } else if (segments.get(3).equalsIgnoreCase("issues") &&
                 segments.size() > 4) {
        owner = segments.get(1);
        repo = segments.get(2);
        number = segments.get(4);
    ...
    if (issueNumber < 1)
      return null;
    return IssuePagerActivity.createIntent(
        context, repo, owner, issueNumber, showRepoBtn,
        LinkParserHelper.isEnterprise(uri.toString()),
        commentId == null ? 0 : commentId);
  }
  ...
  @Nullable
  private static Intent getLabel(final @NonNull Context context,
                                 final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments == null || segments.size() < 3)
      return null;
    ...
    if ("labels".equalsIgnoreCase(lastPath)) {
      return FilterIssuesActivity.getIntent(
          context, owner, repoName, "label:\"" + segments.get(3) + "\"");
    }
    return null;
  }

  @Nullable
  private static Intent getRepo(final @NonNull Context context,
                                final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments == null || segments.size() < 2 || segments.size() > 3)
      return null;
    String owner = segments.get(0);
    String repoName = segments.get(1);
  ...
  @Nullable
  private static Intent getRepoProject(final @NonNull Context context,
                                       final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments == null || segments.size() < 3)
      return null;
    ...
    if (segments.size() == 3 && "projects".equalsIgnoreCase(segments.get(2))) {
      return RepoPagerActivity.createIntent(context, repoName, owner,
                                            RepoPagerMvp.PROJECTS);
    } else if (segments.size() == 4 &&
               "projects".equalsIgnoreCase(segments.get(2))) {
      try {
        int projectId = Integer.parseInt(segments.get(segments.size() - 1));
        if (projectId > 0) {
          return ProjectPagerActivity.Companion.getIntent(
              context, owner, repoName, projectId,
              LinkParserHelper.isEnterprise(uri.toString()));
        }
      } catch (Exception ignored) {
      }
  ...
  @Nullable
  private static Intent getWiki(final @NonNull Context context,
                                final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments == null || segments.size() < 3)
      return null;
    if ("wiki".equalsIgnoreCase(segments.get(2))) {
      String owner = segments.get(0);
      String repoName = segments.get(1);
      return WikiActivity.Companion.getWiki(
          context, repoName, owner,
          "wiki".equalsIgnoreCase(uri.getLastPathSegment())
              ? null
   ...
   */
  @Nullable
  private static Intent getGeneralRepo(final @NonNull Context context,
                                       final @NonNull Uri uri) {
    // TODO parse deeper links to their associate views. meantime fallback to
    // repoPage
    ...
    boolean isEnterprise =
        ...
        Uri.parse(LinkParserHelper.getEndpoint(PrefGetter.getEnterpriseUrl()))
            .getAuthority()
            .equalsIgnoreCase(uri.getAuthority());
    if (uri.getAuthority().equals(HOST_DEFAULT) ||
        uri.getAuthority().equals(API_AUTHORITY) || isEnterprise) {
      List<String> segments = uri.getPathSegments();
  ...
  @Nullable
  private static Intent getCommits(final @NonNull Context context,
                                   final @NonNull Uri uri,
                                   final boolean showRepoBtn) {
    List<String> segments = Stream.of(uri.getPathSegments())
                                .filter(value
                                        -> !value.equalsIgnoreCase("api") ||
                                               !value.equalsIgnoreCase("v3"))
                                .toList();
    if (segments == null || segments.isEmpty() || segments.size() < 3)
      return null;
    String login = null;
    String repoId = null;
    String sha = null;
    ...
    if (login != null && sha != null && repoId != null) {
      return CommitPagerActivity.createIntent(context, repoId, login, sha,
                                              showRepoBtn);
    }
    return null;
  }
  ...
  @Nullable
  private static Intent getCommit(final @NonNull Context context,
                                  final @NonNull Uri uri,
                                  final boolean showRepoBtn) {
    List<String> segments = Stream.of(uri.getPathSegments())
                                .filter(value
                                        -> !value.equalsIgnoreCase("api") ||
                                               !value.equalsIgnoreCase("v3"))
                                .toList();
    if (segments.size() < 3 || !"commit".equals(segments.get(2)))
      return null;
  ...
  @Nullable
  private static String getGistId(final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    ...
    if (TextUtils.isDigitsOnly(gistId))
      return gistId;
    else if (gistId.matches("[a-fA-F0-9]+"))
      return gistId;
    else
      return null;
  }
  ...
  @Nullable
  private static Intent getUser(final @NonNull Context context,
                                final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments != null && !segments.isEmpty() && segments.size() == 1) {
      return UserPagerActivity.createIntent(context, segments.get(0));
    } else if (segments != null && !segments.isEmpty() && segments.size() > 1 &&
               segments.get(0).equalsIgnoreCase("orgs")) {
  ...
  @Nullable
  private static Intent getBlob(final @NonNull Context context,
                                final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
  ...
  @Nullable
  private static Intent getRepoIssueIntent(final @NonNull Context context,
                                           final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments != null && segments.size() == 3 &&
        uri.getLastPathSegment().equalsIgnoreCase("issues")) {
      String owner = segments.get(0);
      String repo = segments.get(1);
      Uri encoded =
          Uri.parse(uri.toString().replace("utf8=%E2%9C%93&amp;", ""));
      ...
      return RepoPagerActivity.createIntent(context, repo, owner,
                                            RepoPagerMvp.ISSUES);
    }
    return null;
  }
  ...
  @Nullable
  private static Intent getRepoPullRequestIntent(final @NonNull Context context,
                                                 final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments != null && segments.size() == 3 &&
        uri.getLastPathSegment().equalsIgnoreCase("pulls")) {
      String owner = segments.get(0);
      String repo = segments.get(1);
      Uri encoded =
          Uri.parse(uri.toString().replace("utf8=%E2%9C%93&amp;", ""));
  ...
  @Nullable
  private static Intent getReleases(final @NonNull Context context,
                                    final @NonNull Uri uri,
      ...
      if (uri.getPathSegments().get(2).equals("releases")) {
        ...
        if (tag != null && !repo.equalsIgnoreCase(tag)) {
          if (TextUtils.isDigitsOnly(tag)) {
            return ReleasesListActivity.getIntent(
                context, owner, repo, InputHelper.toLong(tag), isEnterprise);
          } else {
        ...
        return ReleasesListActivity.getIntent(context, owner, repo);
                 ...
                 segments.get(3).equalsIgnoreCase("releases")) {
        String owner = segments.get(1);
  ...
  @Nullable
  private static Intent getTrending(final @NonNull Context context,
                                    final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (segments != null && !segments.isEmpty()) {
      if (uri.getPathSegments().get(0).equals("trending")) {
        ...
        return TrendingActivity.Companion.getTrendingIntent(context, lang,
                                                            query);
      }
      return null;
    }
   ...
   */
  @Nullable
  private static Intent getCreateIssueIntent(final @NonNull Context context,
                                             final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
    if (uri.getLastPathSegment() == null)
      return null;
    if (segments == null || segments.size() < 3 ||
        !uri.getLastPathSegment().equalsIgnoreCase("new"))
      return null;
    if ("issues".equals(segments.get(2))) {
      String owner = segments.get(0);
      String repo = segments.get(1);
      boolean isFeedback =
          "k0shk0sh/FastHub".equalsIgnoreCase(owner + "/" + repo);
  ...
  @Nullable
  private static Intent getGistFile(final @NonNull Context context,
                                    final @NonNull Uri uri) {
  ...
  @Nullable
  private static Intent getSearchIntent(final @NonNull Context context,
                                        final @NonNull Uri uri) {
    List<String> segments = uri.getPathSegments();
