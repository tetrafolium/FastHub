...
public class LinkParserHelper {
  ...
  static final List<String> IGNORED_LIST =
      Arrays.asList("notifications", "settings", "blog", "explore", "dashboard",
                    "repositories", "logout", "sessions", "site", "security",
                    "contact", "about", "logos", "login", "pricing", "");

  @SafeVarargs
  static <T> Optional<T> returnNonNull(final @NonNull T... t) {
    return Stream.of(t).filter(ObjectsCompat::nonNull).findFirst();
  }

  @NonNull
  static Uri getBlobBuilder(final @NonNull Uri uri) {
    boolean isSvg = "svg".equalsIgnoreCase(
  ...
  public static boolean isEnterprise(final @Nullable String url) {
    ...
    url = url.toLowerCase();
    return url.equalsIgnoreCase(enterpriseUrl) ||
        url.startsWith(enterpriseUrl) ||
        url.startsWith(getEndpoint(enterpriseUrl)) ||
  ...
  public static String stripScheme(final @NonNull String url) {
    try {
      ...
      return !InputHelper.isEmpty(uri.getAuthority()) ? uri.getAuthority()
                                                      : url;
  ...
  @NonNull
  public static String getEndpoint(final @NonNull String url) {
  ...
  @NonNull
  private static String getEnterpriseUrl(final @NonNull String url) {
  ...
  public static String getEnterpriseGistUrl(final @NonNull String url,
                                            final boolean isEnterprise) {
    if (isEnterprise) {
      Uri uri = Uri.parse(url);
      boolean isGist = uri == null || uri.getPathSegments() == null
                           ? url.contains("gist/")
                           : uri.getPathSegments().get(0).equals("gist");
      if (isGist) {
  ...
  @Nullable
  public static String getGistId(final @NonNull Uri uri) {
    String gistId = null;
