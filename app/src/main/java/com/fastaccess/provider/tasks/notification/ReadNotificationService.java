...
public class ReadNotificationService extends IntentService {
  ...
  public static Intent start(final @NonNull Context context, final long id,
  ...
  public static void unSubscribe(final @NonNull Context context,
                                 final long id) {
    Intent intent = new Intent(context.getApplicationContext(),
                               ReadNotificationService.class);
    intent.putExtras(Bundler.start()
  ...
  protected void onHandleIntent(final @Nullable Intent intent) {
    if (intent != null && intent.getExtras() != null) {
      ...
      if (type == READ_SINGLE) {
        ...
        openNotification(bundle.getLong(BundleConstant.ID),
  ...
  private void unSubscribeFromThread(final long id) {
    RestProvider.getNotificationService(PrefGetter.isEnterprise())
        ...
        .flatMap(notification1
                 -> Observable.create(subscriber -> markSingleAsRead(id)))
        .subscribe(booleanResponse -> cancel(id), throwable -> cancel(id));
  }

  private void openNotification(final long id, final @Nullable String url,
    ...
    if (id > 0 && url != null) {
      ...
      if (!readOnly) {
        SchemeParser.launchUri(getApplicationContext(), Uri.parse(url), true,
                               true);
      }
    }
  }
