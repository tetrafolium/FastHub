...
public abstract class AbstractSearchHistory implements Parcelable {
  ...
  public Single<SearchHistory> save(final SearchHistory entity) {
    return RxHelper.getSingle(
        App.getInstance()
            ...
            .single()
            .flatMap(
                integer -> App.getInstance().getDataStore().insert(entity)));
  }
  ...
  public AbstractSearchHistory(final String text) { this.text = text; }

  protected AbstractSearchHistory(final Parcel in) {
    this.text = in.readString();
  }

  public static final Creator<SearchHistory> CREATOR =
      new Creator<SearchHistory>() {
        @Override
        public SearchHistory createFromParcel(final Parcel source) {
          return new SearchHistory(source);
        }

        @Override
        public SearchHistory[] newArray(final int size) {
