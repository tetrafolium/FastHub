...
public class PayloadModel implements Parcelable {
  ...
  protected PayloadModel(final Parcel in) {
    ...
    this.download =
        in.readParcelable(ReleasesAssetsModel.class.getClassLoader());
  ...
  public static final Creator<PayloadModel> CREATOR =
      new Creator<PayloadModel>() {
        @Override
        public PayloadModel createFromParcel(final Parcel source) {
          return new PayloadModel(source);
        }

        @Override
        public PayloadModel[] newArray(final int size) {
