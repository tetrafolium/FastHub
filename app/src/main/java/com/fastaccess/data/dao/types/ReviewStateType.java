...
public enum ReviewStateType {
  ...
  ReviewStateType(final @StringRes int stringRes,
  ...
  @Nullable
  public static ReviewStateType getType(final @NonNull String state) {
    return Stream.of(values())
        .filter(
            value
            -> value.name().toLowerCase().equalsIgnoreCase(state.toLowerCase()))
        .findFirst()
        .orElse(null);
  }
}
