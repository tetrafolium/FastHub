...
public abstract class AbstractPinnedRepos implements Parcelable {
  ...
  @Nullable
  public static PinnedRepos get(final long id) {
    return App.getInstance()
        .getDataStore()
        .select(PinnedRepos.class)
        .where(ID.eq(id))
        .get()
        .firstOrNull();
  }
  ...
  @Nullable
  public static PinnedRepos get(final @NonNull String repoFullName) {
    return App.getInstance()
        .getDataStore()
        .toBlocking()
        .select(PinnedRepos.class)
        .where(REPO_FULL_NAME.eq(repoFullName)
                   .and(LOGIN.eq(Login.getUser().getLogin()))
  ...
  @NonNull
  public static Disposable updateEntry(final @NonNull String repoFullName) {
    return RxHelper
        .getObservable(Observable.fromPublisher(e -> {
    ...
    RxHelper
        .getObservable(Observable.fromPublisher(e -> {
          try {
            ...
            BlockingEntityStore<Persistable> reactiveEntityStore =
                App.getInstance().getDataStore().toBlocking();
            List<PinnedRepos> pinnedRepos =
                reactiveEntityStore.select(PinnedRepos.class)
                    .where(LOGIN.isNull())
                    .get()
                    .toList();
            if (pinnedRepos != null) {
