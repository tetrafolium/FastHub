...
public abstract class AbstractLogin implements Parcelable {
  ...
  public Observable<Login> update(final Login login) {
    return RxHelper.safeObservable(
        App.getInstance().getDataStore().update(login).toObservable());
  }
  ...
  public static Login getUser(final @NonNull String login) {
    return App.getInstance()
        .getDataStore()
        .select(Login.class)
        .where(Login.LOGIN.eq(login).and(Login.TOKEN.notNull()))
        .get()
        .firstOrNull();
  }
  ...
  public static void logout() {
    ...
    App.getInstance()
        .getDataStore()
        .toBlocking()
        .delete(PinnedRepos.class)
        .where(PinnedRepos.LOGIN.eq(login.getLogin()))
        .get()
        .value();
  ...
  public static boolean hasNormalLogin() {
    return App.getInstance()
               .getDataStore()
               .count(Login.class)
               .where(Login.IS_ENTERPRISE.eq(false).or(
                   Login.IS_ENTERPRISE.isNull()))
               .get()
               .value() > 0;
  }

  public static Observable<Boolean> onMultipleLogin(final
                                                    @NonNull Login userModel,
    ...
    return Observable.fromPublisher(s -> {
      ...
      if (isNew) {
        ...
        userModel.setToken(isEnterprise ? PrefGetter.getEnterpriseToken()
                                        : PrefGetter.getToken());
        userModel.setOtpCode(isEnterprise ? PrefGetter.getEnterpriseOtpCode()
                                          : PrefGetter.getOtpCode());
        userModel.setEnterpriseUrl(isEnterprise ? PrefGetter.getEnterpriseUrl()
                                                : null);
        App.getInstance()
            .getDataStore()
            .toBlocking()
