...
public class CommitFileChanges implements Parcelable {
  ...
  constructToObservable(@Nullable ArrayList<CommitFileModel> files) {
  ...
  construct(@Nullable List<CommitFileModel> files) {
    if (files == null || files.isEmpty()) {
  ...
  @NonNull
  private static CommitFileChanges getCommitFileChanges(CommitFileModel m) {
