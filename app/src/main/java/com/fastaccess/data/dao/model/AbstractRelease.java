...
public abstract class AbstractRelease implements Parcelable {
  ...
  public static Disposable save(final @NonNull List<Release> models,
        ...
        .getSingle(Single.fromPublisher(s -> {
          try {
            ...
            dataSource.delete(Release.class)
                .where(Release.REPO_ID.equal(repoId).and(
                    Release.LOGIN.equal(login)))
                .get()
                .value();
            if (!models.isEmpty()) {
              for (Release releasesModel : models) {
                dataSource.delete(Release.class)
                    .where(Release.ID.eq(releasesModel.getId()))
                    .get()
                    .value();
  ...
  public static Completable delete(final @NonNull String repoId,
                                   final @NonNull String login) {
  ...
  public static Single<List<Release>> get(final @NonNull String repoId,
                                          final @NonNull String login) {
    return App.getInstance()
        .getDataStore()
        .select(Release.class)
  ...
  public static final Creator<Release> CREATOR = new Creator<Release>() {
    @Override
    public Release createFromParcel(final Parcel source) {
      return new Release(source);
    }

    @Override
    public Release[] newArray(final int size) {
