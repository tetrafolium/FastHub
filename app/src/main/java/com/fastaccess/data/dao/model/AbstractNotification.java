...
public abstract class AbstractNotification implements Parcelable {
    ...
    return RxHelper
        .getSingle(Single.fromPublisher(s -> {
          try {
            ...
            dataStore.delete(Notification.class)
    ...
    return RxHelper
        .getSingle(Single.fromPublisher(s -> {
          try {
            BlockingEntityStore<Persistable> dataStore =
                App.getInstance().getDataStore().toBlocking();
            Notification current = dataStore.select(Notification.class)
  ...
  public static Disposable
  save(final @androidx.annotation.Nullable List<Notification> models) {
  ...
  public static Single<Boolean>
  saveAsSingle(final @androidx.annotation.Nullable List<Notification> models) {
    if (models == null || models.isEmpty()) {
  ...
  public boolean equals(final Object o) {
    ...
    return repository != null && that.repository != null &&
        repository.getId() == that.repository.getId();
  }
  ...
  protected AbstractNotification(final Parcel in) {
    ...
    this.subject =
        in.readParcelable(NotificationSubjectModel.class.getClassLoader());
    int tmpReason = in.readInt();
    this.reason =
        tmpReason == -1 ? null : NotificationReason.values()[tmpReason];
  ...
  public static final Creator<Notification> CREATOR =
      new Creator<Notification>() {
        @Override
        public Notification createFromParcel(final Parcel source) {
          return new Notification(source);
        }

        @Override
        public Notification[] newArray(final int size) {
