...
public class GroupedNotificationModel {
  ...
  @NonNull
  public static List<GroupedNotificationModel>
  construct(final @Nullable List<Notification> items) {
    List<GroupedNotificationModel> models = new ArrayList<>();
    if (items == null || items.isEmpty())
      return models;
    Map<Repo, List<Notification>> grouped =
        Stream.of(items)
            .filter(value -> !value.isUnread())
            .collect(Collectors.groupingBy(
                Notification::getRepository, LinkedHashMap::new,
                Collectors.mapping(o -> o, toList())));
    Stream.of(grouped)
        .filter(repoListEntry
                -> repoListEntry.getValue() != null &&
                       !repoListEntry.getValue().isEmpty())
        .forEach(repoListEntry -> {
          ...
          Stream.of(notifications)
              .sorted(
                  (o1, o2) -> o2.getUpdatedAt().compareTo(o1.getUpdatedAt()))
              .forEach(
                  notification
  ...
  @NonNull
  public static List<GroupedNotificationModel>
  onlyNotifications(final @Nullable List<Notification> items) {
    if (items == null || items.isEmpty())
  ...
  public boolean equals(final Object o) {
    ...
    return notification != null && model.getNotification() != null &&
        notification.getId() == (model.notification.getId());
  }

  @Override
  public int hashCode() {
    return notification != null ? InputHelper.getSafeIntId(notification.getId())
                                : 0;
  }
}
