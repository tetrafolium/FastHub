...
public abstract class AbstractIssue implements Parcelable {
  ...
  public static Disposable save(final @NonNull List<Issue> models,
          ...
          try {
            ...
            if (!models.isEmpty()) {
              for (Issue issueModel : models) {
                dataSource.delete(Issue.class)
                    .where(Issue.ID.eq(issueModel.getId()))
                    .get()
                    .value();
  ...
  public static Single<List<Issue>> getIssues(final @NonNull String repoId,
                                              final @NonNull String login,
                                              final
  ...
  public static Observable<Issue>
  getIssueByNumber(final int number, final String repoId, final String login) {
    return App.getInstance()
        .getDataStore()
        .select(Issue.class)
        .where(
            NUMBER.equal(number).and(REPO_ID.eq(repoId)).and(LOGIN.eq(login)))
        .get()
        .observable();
  }
  ...
  public static final Creator<Issue> CREATOR = new Creator<Issue>() {
    @Override
    public Issue createFromParcel(final Parcel source) {
      return new Issue(source);
    }

    @Override
    public Issue[] newArray(final int size) {
