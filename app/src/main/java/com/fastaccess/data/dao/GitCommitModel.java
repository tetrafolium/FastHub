...
public class GitCommitModel implements Parcelable {
  ...
  public static final Creator<GitCommitModel> CREATOR =
      new Creator<GitCommitModel>() {
        @Override
        public GitCommitModel createFromParcel(final Parcel source) {
          return new GitCommitModel(source);
        }

        @Override
        public GitCommitModel[] newArray(final int size) {
  ...
  public String toString() {
    if (message != null) {
      return (sha != null && sha.length() > 7 ? sha.substring(0, 7) + " - "
                                              : "") +
          message.split(System.lineSeparator())[0];
