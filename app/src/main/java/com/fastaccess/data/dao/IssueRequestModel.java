...
public class IssueRequestModel implements Parcelable {
  ...
  public static IssueRequestModel clone(final @NonNull Issue issue,
                                        final boolean toClose) {
    IssueRequestModel model = new IssueRequestModel();
    if (issue.getLabels() != null) {
      model.setLabels(Stream.of(issue.getLabels())
                          .filter(value -> value.getName() != null)
                          .map(LabelModel::getName)
                          .collect(Collectors.toList()));
    }
    model.setAssignee(
        issue.getAssignee() != null ? issue.getAssignee().getLogin() : null);
    model.setBody(issue.getBody());
    model.setMilestone(
        issue.getMilestone() != null ? issue.getMilestone().getNumber() : null);
    model.setState(toClose ? issue.getState() == IssueState.closed
                                 ? IssueState.open
                                 : IssueState.closed
                           : issue.getState());
  ...
  public static IssueRequestModel clone(final @NonNull PullRequest issue,
                                        final boolean toClose) {
    IssueRequestModel model = new IssueRequestModel();
    if (issue.getLabels() != null) {
      model.setLabels(Stream.of(issue.getLabels())
    ...
    model.setBase(issue.getBase() != null ? issue.getBase().getRef()
                                          : "master");
    model.setAssignee(
        issue.getAssignee() != null ? issue.getAssignee().getLogin() : null);
    model.setBody(issue.getBody());
  ...
  public static final Creator<IssueRequestModel> CREATOR =
      new Creator<IssueRequestModel>() {
        @Override
        public IssueRequestModel createFromParcel(final Parcel source) {
          return new IssueRequestModel(source);
        }

        @Override
        public IssueRequestModel[] newArray(final int size) {
