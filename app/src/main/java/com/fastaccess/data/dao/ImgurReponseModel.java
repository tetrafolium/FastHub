...
public class ImgurReponseModel implements Parcelable {
  ...
  private ImgurReponseModel(final Parcel in) {
    this.success = in.readByte() != 0;
    this.status = in.readInt();
    this.data = in.readParcelable(ImgurImage.class.getClassLoader());
  }

  public static final Parcelable.Creator<ImgurReponseModel> CREATOR =
      new Parcelable.Creator<ImgurReponseModel>() {
        @Override
        public ImgurReponseModel createFromParcel(final Parcel source) {
          return new ImgurReponseModel(source);
        }

        @Override
        public ImgurReponseModel[] newArray(final int size) {
  ...
  public static class ImgurImage implements Parcelable {
    ...
    private ImgurImage(final Parcel in) {
    ...
    public static final Creator<ImgurImage> CREATOR =
        new Creator<ImgurImage>() {
          @Override
          public ImgurImage createFromParcel(final Parcel source) {
            return new ImgurImage(source);
          }

          @Override
          public ImgurImage[] newArray(final int size) {
