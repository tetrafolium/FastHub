...
public abstract class AbstractCommit implements Parcelable {
  ...
  @Convert(GitCommitConverter.class)
  ...
  public static Disposable save(final @NonNull List<Commit> models,
  ...
  public static Single<List<Commit>> getCommits(final @NonNull String repoId,
                                                final @NonNull String login) {
  ...
  public static Observable<Commit> getCommit(final @NonNull String sha,
                                             final @NonNull String repoId,
                                             final @NonNull String login) {
    return App.getInstance()
        .getDataStore()
        .select(Commit.class)
        .where(REPO_ID.eq(repoId).and(LOGIN.eq(login)).and(SHA.eq(sha)))
        .limit(1)
        .get()
        .observable();
  }
  ...
  public static final Creator<Commit> CREATOR = new Creator<Commit>() {
    @Override
    public Commit createFromParcel(final Parcel source) {
      return new Commit(source);
    }

    @Override
    public Commit[] newArray(final int size) {
