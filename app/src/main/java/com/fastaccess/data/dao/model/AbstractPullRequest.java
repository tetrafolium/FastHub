...
public abstract class AbstractPullRequest implements Parcelable {
  ...
  public Single<PullRequest> save(final PullRequest entity) {
    return RxHelper.getSingle(
        App.getInstance()
            ...
            .single()
            .flatMap(
                observer -> App.getInstance().getDataStore().insert(entity)));
  }

  public static Disposable save(final @NonNull List<PullRequest> models,
                                final @NonNull String repoId,
                                final @NonNull String login) {
    return RxHelper
        .getSingle(Single.fromPublisher(s -> {
          try {
            BlockingEntityStore<Persistable> dataSource =
            ...
            if (!models.isEmpty()) {
              for (PullRequest pullRequest : models) {
                dataSource.delete(PullRequest.class)
                    .where(PullRequest.ID.eq(pullRequest.getId()))
                    .get()
                    .value();
  ...
  public static Single<List<PullRequest>>
  getPullRequests(final @NonNull String repoId, final @NonNull String login,
  ...
  public static Observable<PullRequest>
  getPullRequestByNumber(final int number, final @NonNull String repoId,
                         final @NonNull String login) {
    return App.getInstance()
        .getDataStore()
        .select(PullRequest.class)
  ...
  getMergeBy(final @NonNull PullRequest pullRequest,
             final @NonNull Context context, final boolean showRepoName) {
    boolean isMerge =
        pullRequest.isMerged() || !InputHelper.isEmpty(pullRequest.mergedAt);
    if (isMerge) {
      User merger = pullRequest.getMergedBy();
      SpannableBuilder builder = SpannableBuilder.builder();
      if (showRepoName) {
        PullsIssuesParser parser =
            PullsIssuesParser.getForPullRequest(pullRequest.getHtmlUrl());
        if (parser != null)
      ...
      if (pullRequest.getState() == IssueState.open &&
          pullRequest.getHead() != null && pullRequest.getBase() != null) {
        return builder.append(context.getString(R.string.want_to_merge))
            .append(" ")
        ...
        return builder.bold(status.toLowerCase())
            .append(" ")
  ...
  public static final Creator<PullRequest> CREATOR =
      new Creator<PullRequest>() {
        @Override
        public PullRequest createFromParcel(final Parcel source) {
          return new PullRequest(source);
        }

        @Override
        public PullRequest[] newArray(final int size) {
