...
public abstract class AbstractEvent implements Parcelable {
  ...
  save(final @androidx.annotation.Nullable List<Event> events,
       final @androidx.annotation.Nullable String user) {
    return RxHelper
        .getSingle(Single.fromPublisher(s -> {
          try {
            ...
            dataSource.delete(Event.class)
                .where(
                    Event.LOGIN.isNull().or(Event.LOGIN.eq(login.getLogin())))
                .get()
                .value();
            if (events != null && !events.isEmpty() &&
                TextUtils.equals(login.getLogin(), user)) {
  ...
  @NonNull
  public static Single<List<Event>> getEvents(final @NonNull String login) {
    return RxHelper.getSingle(App.getInstance()
                                  .getDataStore()
                                  .select(Event.class)
