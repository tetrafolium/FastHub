...
public class ActivityHelper {
  ...
  @Nullable
  public static Activity getActivity(final @Nullable Context content) {
    if (content == null)
      return null;
    else if (content instanceof Activity)
      return (Activity)content;
    else if (content instanceof ContextWrapper)
      return getActivity(((ContextWrapper)content).getBaseContext());
    return null;
  }

  public static void startCustomTab(final @NonNull Activity context,
  ...
  public static void openChooser(final @NonNull Context context,
                                 final @NonNull Uri url) {
    openChooser(context, url, false);
  }

  private static void openChooser(final @NonNull Context context,
                                  final @NonNull Uri url,
                                  final boolean fromCustomTab) {
  ...
  @SafeVarargs
  public static void start(final @NonNull Activity activity, final Class cl,
  ...
  public static void start(final @NonNull Activity activity,
                           final Intent intent,
                           final @NonNull View sharedElement) {
    ActivityOptionsCompat options =
        ActivityOptionsCompat.makeSceneTransitionAnimation(
            activity, sharedElement,
            ViewHelper.getTransitionName(sharedElement));
    activity.startActivity(intent, options.toBundle());
  }

  public static void startReveal(final @NonNull Activity activity,
                                 final Intent intent,
                                 final @NonNull View sharedElement,
    ...
    if (!PrefGetter.isAppAnimationDisabled()) {
      ActivityOptionsCompat options =
          ActivityOptionsCompat.makeClipRevealAnimation(
              sharedElement, sharedElement.getWidth() / 2,
              sharedElement.getHeight() / 2, sharedElement.getWidth(),
  ...
  public static void startReveal(final @NonNull Fragment fragment,
                                 final Intent intent,
                                 final @NonNull View sharedElement,
                                 final int requestCode) {
    if (!PrefGetter.isAppAnimationDisabled()) {
  ...
  @SafeVarargs
  public static void
  start(final @NonNull Activity activity, final @NonNull Intent intent,
  ...
  public static void shareUrl(final @NonNull Context context,
                              final @NonNull String url) {
    Activity activity = getActivity(context);
    if (activity == null)
      throw new IllegalArgumentException(
          "Context given is not an instance of activity " +
  ...
  @Nullable
  public static Fragment getVisibleFragment(final
                                            @NonNull FragmentManager manager) {
    List<Fragment> fragments = manager.getFragments();
    if (fragments != null && !fragments.isEmpty()) {
      for (Fragment fragment : fragments) {
        if (fragment != null && fragment.isVisible() &&
            !(fragment instanceof MainDrawerFragment ||
              fragment instanceof AccountDrawerFragment)) {
          return fragment;
        }
      }
    }
  ...
  private static boolean isPermissionGranted(final @NonNull Context context,
                                             final @NonNull String permission) {
    return ActivityCompat.checkSelfPermission(context, permission) ==
        PackageManager.PERMISSION_GRANTED;
  }

  private static boolean isExplanationNeeded(final @NonNull Activity context,
                                             final
  ...
  isReadWritePermissionIsGranted(final @NonNull Context context) {
  ...
  private static void requestReadWritePermission(final
  ...
  checkAndRequestReadWritePermission(final @NonNull Activity activity) {
    if (!isReadWritePermissionIsGranted(activity)) {
      requestReadWritePermission(activity);
      return false;
    } else if (isExplanationNeeded(activity,
                                   Manifest.permission.READ_EXTERNAL_STORAGE) ||
               isExplanationNeeded(
  ...
  private static Intent chooserIntent(final @NonNull Context context,
  ...
  public static void activateLinkInterceptorActivity(final Context context,
                                                     final boolean activate) {
    final PackageManager pm = context.getPackageManager();
    final int flag = activate ? PackageManager.COMPONENT_ENABLED_STATE_ENABLED
                              : PackageManager.COMPONENT_ENABLED_STATE_DISABLED;
    pm.setComponentEnabledSetting(
        new ComponentName(context, LinksParserActivity.class), flag,
        PackageManager.DONT_KILL_APP);
  }

  public static Intent editBundle(final @NonNull Intent intent,
