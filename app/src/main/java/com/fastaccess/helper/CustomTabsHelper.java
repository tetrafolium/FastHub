...
class CustomTabsHelper {
   ...
   * effort to return a valid package name. <p> This is <strong>not</strong>
   * threadsafe.
   *
   * @param context
  ...
  static String getPackageNameToUse(final Context context) {
    ...
    // Now packagesSupportingCustomTabs contains all apps that can handle both
    // VIEW intents and service calls.
   ...
   */
  private static boolean hasSpecializedHandlerIntents(final Context context,
                                                      final Intent intent) {
    try {
      PackageManager pm = context.getPackageManager();
